...
@dataclass
class PharmaCustomerEntity:
    """Entity for pharma customer"""
    id: str
    data: Dict[str, Any]
...
class PharmaCustomerInMemoryRepository(BasePharmaRepository[PharmaCustomerEntity]):
    """In-memory repository for pharma customer"""

    def __init__(self):
        self._storage: Dict[str, PharmaCustomerEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: PharmaCustomerEntity) -> PharmaCustomerEntity:
    ...
    async def read(self, id: str) -> Optional[PharmaCustomerEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: PharmaCustomerEntity) -> PharmaCustomerEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[PharmaCustomerEntity]:
    ...
    async def find_by(self, **kwargs) -> List[PharmaCustomerEntity]:
...
class PharmaCustomerCachedRepository(BasePharmaRepository[PharmaCustomerEntity]):
    """Cached repository wrapper for pharma customer"""
...
class PharmaCustomerUnitOfWork:
    """Unit of Work pattern for pharma customer"""

    def __init__(self, repository: BasePharmaRepository):
        self._repository = repository
        self._new: List[PharmaCustomerEntity] = []
        self._dirty: List[PharmaCustomerEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: PharmaCustomerEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: PharmaCustomerEntity) -> None:
