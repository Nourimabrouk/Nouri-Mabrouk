...
@dataclass
class PharmaInventoryEntity:
    """Entity for pharma inventory"""
    id: str
    data: Dict[str, Any]
...
class PharmaInventoryInMemoryRepository(BasePharmaRepository[PharmaInventoryEntity]):
    """In-memory repository for pharma inventory"""

    def __init__(self):
        self._storage: Dict[str, PharmaInventoryEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: PharmaInventoryEntity) -> PharmaInventoryEntity:
    ...
    async def read(self, id: str) -> Optional[PharmaInventoryEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: PharmaInventoryEntity) -> PharmaInventoryEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[PharmaInventoryEntity]:
    ...
    async def find_by(self, **kwargs) -> List[PharmaInventoryEntity]:
...
class PharmaInventoryCachedRepository(BasePharmaRepository[PharmaInventoryEntity]):
    """Cached repository wrapper for pharma inventory"""
...
class PharmaInventoryUnitOfWork:
    """Unit of Work pattern for pharma inventory"""

    def __init__(self, repository: BasePharmaRepository):
        self._repository = repository
        self._new: List[PharmaInventoryEntity] = []
        self._dirty: List[PharmaInventoryEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: PharmaInventoryEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: PharmaInventoryEntity) -> None:
