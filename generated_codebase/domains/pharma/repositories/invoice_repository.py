...
@dataclass
class PharmaInvoiceEntity:
    """Entity for pharma invoice"""
    id: str
    data: Dict[str, Any]
...
class PharmaInvoiceInMemoryRepository(BasePharmaRepository[PharmaInvoiceEntity]):
    """In-memory repository for pharma invoice"""

    def __init__(self):
        self._storage: Dict[str, PharmaInvoiceEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: PharmaInvoiceEntity) -> PharmaInvoiceEntity:
    ...
    async def read(self, id: str) -> Optional[PharmaInvoiceEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: PharmaInvoiceEntity) -> PharmaInvoiceEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[PharmaInvoiceEntity]:
    ...
    async def find_by(self, **kwargs) -> List[PharmaInvoiceEntity]:
...
class PharmaInvoiceCachedRepository(BasePharmaRepository[PharmaInvoiceEntity]):
    """Cached repository wrapper for pharma invoice"""
...
class PharmaInvoiceUnitOfWork:
    """Unit of Work pattern for pharma invoice"""

    def __init__(self, repository: BasePharmaRepository):
        self._repository = repository
        self._new: List[PharmaInvoiceEntity] = []
        self._dirty: List[PharmaInvoiceEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: PharmaInvoiceEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: PharmaInvoiceEntity) -> None:
