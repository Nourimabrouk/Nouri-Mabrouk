...
@dataclass
class PharmaSupplierEntity:
    """Entity for pharma supplier"""
    id: str
    data: Dict[str, Any]
...
class PharmaSupplierInMemoryRepository(BasePharmaRepository[PharmaSupplierEntity]):
    """In-memory repository for pharma supplier"""

    def __init__(self):
        self._storage: Dict[str, PharmaSupplierEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: PharmaSupplierEntity) -> PharmaSupplierEntity:
    ...
    async def read(self, id: str) -> Optional[PharmaSupplierEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: PharmaSupplierEntity) -> PharmaSupplierEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[PharmaSupplierEntity]:
    ...
    async def find_by(self, **kwargs) -> List[PharmaSupplierEntity]:
...
class PharmaSupplierCachedRepository(BasePharmaRepository[PharmaSupplierEntity]):
    """Cached repository wrapper for pharma supplier"""
...
class PharmaSupplierUnitOfWork:
    """Unit of Work pattern for pharma supplier"""

    def __init__(self, repository: BasePharmaRepository):
        self._repository = repository
        self._new: List[PharmaSupplierEntity] = []
        self._dirty: List[PharmaSupplierEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: PharmaSupplierEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: PharmaSupplierEntity) -> None:
