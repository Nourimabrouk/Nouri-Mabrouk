...
@dataclass
class PharmaPaymentEntity:
    """Entity for pharma payment"""
    id: str
    data: Dict[str, Any]
...
class PharmaPaymentInMemoryRepository(BasePharmaRepository[PharmaPaymentEntity]):
    """In-memory repository for pharma payment"""

    def __init__(self):
        self._storage: Dict[str, PharmaPaymentEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: PharmaPaymentEntity) -> PharmaPaymentEntity:
    ...
    async def read(self, id: str) -> Optional[PharmaPaymentEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: PharmaPaymentEntity) -> PharmaPaymentEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[PharmaPaymentEntity]:
    ...
    async def find_by(self, **kwargs) -> List[PharmaPaymentEntity]:
...
class PharmaPaymentCachedRepository(BasePharmaRepository[PharmaPaymentEntity]):
    """Cached repository wrapper for pharma payment"""
...
class PharmaPaymentUnitOfWork:
    """Unit of Work pattern for pharma payment"""

    def __init__(self, repository: BasePharmaRepository):
        self._repository = repository
        self._new: List[PharmaPaymentEntity] = []
        self._dirty: List[PharmaPaymentEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: PharmaPaymentEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: PharmaPaymentEntity) -> None:
