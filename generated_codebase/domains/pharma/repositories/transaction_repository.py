...
@dataclass
class PharmaTransactionEntity:
    """Entity for pharma transaction"""
    id: str
    data: Dict[str, Any]
...
class BasePharmaRepository(ABC, Generic[T]):
    """Base repository for pharma"""
...
class PharmaTransactionInMemoryRepository(BasePharmaRepository[PharmaTransactionEntity]):
    """In-memory repository for pharma transaction"""

    def __init__(self):
        self._storage: Dict[str, PharmaTransactionEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: PharmaTransactionEntity) -> PharmaTransactionEntity:
    ...
    async def read(self, id: str) -> Optional[PharmaTransactionEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: PharmaTransactionEntity) -> PharmaTransactionEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[PharmaTransactionEntity]:
    ...
    async def find_by(self, **kwargs) -> List[PharmaTransactionEntity]:
...
class PharmaTransactionCachedRepository(BasePharmaRepository[PharmaTransactionEntity]):
    """Cached repository wrapper for pharma transaction"""

    def __init__(self, base_repo: BasePharmaRepository, cache_ttl: int = 300):
...
class PharmaTransactionUnitOfWork:
    """Unit of Work pattern for pharma transaction"""

    def __init__(self, repository: BasePharmaRepository):
        self._repository = repository
        self._new: List[PharmaTransactionEntity] = []
        self._dirty: List[PharmaTransactionEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: PharmaTransactionEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: PharmaTransactionEntity) -> None:
