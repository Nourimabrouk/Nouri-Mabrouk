...
@dataclass
class PharmaTicketEntity:
    """Entity for pharma ticket"""
    id: str
    data: Dict[str, Any]
...
class PharmaTicketInMemoryRepository(BasePharmaRepository[PharmaTicketEntity]):
    """In-memory repository for pharma ticket"""

    def __init__(self):
        self._storage: Dict[str, PharmaTicketEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: PharmaTicketEntity) -> PharmaTicketEntity:
    ...
    async def read(self, id: str) -> Optional[PharmaTicketEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: PharmaTicketEntity) -> PharmaTicketEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[PharmaTicketEntity]:
    ...
    async def find_by(self, **kwargs) -> List[PharmaTicketEntity]:
...
class PharmaTicketCachedRepository(BasePharmaRepository[PharmaTicketEntity]):
    """Cached repository wrapper for pharma ticket"""
...
class PharmaTicketUnitOfWork:
    """Unit of Work pattern for pharma ticket"""

    def __init__(self, repository: BasePharmaRepository):
        self._repository = repository
        self._new: List[PharmaTicketEntity] = []
        self._dirty: List[PharmaTicketEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: PharmaTicketEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: PharmaTicketEntity) -> None:
