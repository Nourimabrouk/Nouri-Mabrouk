...
@dataclass
class PharmaOrderEntity:
    """Entity for pharma order"""
    id: str
    data: Dict[str, Any]
...
class PharmaOrderInMemoryRepository(BasePharmaRepository[PharmaOrderEntity]):
    """In-memory repository for pharma order"""

    def __init__(self):
        self._storage: Dict[str, PharmaOrderEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: PharmaOrderEntity) -> PharmaOrderEntity:
    ...
    async def read(self, id: str) -> Optional[PharmaOrderEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: PharmaOrderEntity) -> PharmaOrderEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[PharmaOrderEntity]:
    ...
    async def find_by(self, **kwargs) -> List[PharmaOrderEntity]:
...
class PharmaOrderCachedRepository(BasePharmaRepository[PharmaOrderEntity]):
    """Cached repository wrapper for pharma order"""
...
class PharmaOrderUnitOfWork:
    """Unit of Work pattern for pharma order"""

    def __init__(self, repository: BasePharmaRepository):
        self._repository = repository
        self._new: List[PharmaOrderEntity] = []
        self._dirty: List[PharmaOrderEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: PharmaOrderEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: PharmaOrderEntity) -> None:
