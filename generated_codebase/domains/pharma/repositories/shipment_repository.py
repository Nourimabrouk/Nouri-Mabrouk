...
@dataclass
class PharmaShipmentEntity:
    """Entity for pharma shipment"""
    id: str
    data: Dict[str, Any]
...
class PharmaShipmentInMemoryRepository(BasePharmaRepository[PharmaShipmentEntity]):
    """In-memory repository for pharma shipment"""

    def __init__(self):
        self._storage: Dict[str, PharmaShipmentEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: PharmaShipmentEntity) -> PharmaShipmentEntity:
    ...
    async def read(self, id: str) -> Optional[PharmaShipmentEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: PharmaShipmentEntity) -> PharmaShipmentEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[PharmaShipmentEntity]:
    ...
    async def find_by(self, **kwargs) -> List[PharmaShipmentEntity]:
...
class PharmaShipmentCachedRepository(BasePharmaRepository[PharmaShipmentEntity]):
    """Cached repository wrapper for pharma shipment"""
...
class PharmaShipmentUnitOfWork:
    """Unit of Work pattern for pharma shipment"""

    def __init__(self, repository: BasePharmaRepository):
        self._repository = repository
        self._new: List[PharmaShipmentEntity] = []
        self._dirty: List[PharmaShipmentEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: PharmaShipmentEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: PharmaShipmentEntity) -> None:
