...
@dataclass
class PharmaEmployeeEntity:
    """Entity for pharma employee"""
    id: str
    data: Dict[str, Any]
...
class PharmaEmployeeInMemoryRepository(BasePharmaRepository[PharmaEmployeeEntity]):
    """In-memory repository for pharma employee"""

    def __init__(self):
        self._storage: Dict[str, PharmaEmployeeEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: PharmaEmployeeEntity) -> PharmaEmployeeEntity:
    ...
    async def read(self, id: str) -> Optional[PharmaEmployeeEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: PharmaEmployeeEntity) -> PharmaEmployeeEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[PharmaEmployeeEntity]:
    ...
    async def find_by(self, **kwargs) -> List[PharmaEmployeeEntity]:
...
class PharmaEmployeeCachedRepository(BasePharmaRepository[PharmaEmployeeEntity]):
    """Cached repository wrapper for pharma employee"""
...
class PharmaEmployeeUnitOfWork:
    """Unit of Work pattern for pharma employee"""

    def __init__(self, repository: BasePharmaRepository):
        self._repository = repository
        self._new: List[PharmaEmployeeEntity] = []
        self._dirty: List[PharmaEmployeeEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: PharmaEmployeeEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: PharmaEmployeeEntity) -> None:
