...
@dataclass
class PharmaReportEntity:
    """Entity for pharma report"""
    id: str
    data: Dict[str, Any]
...
class PharmaReportInMemoryRepository(BasePharmaRepository[PharmaReportEntity]):
    """In-memory repository for pharma report"""

    def __init__(self):
        self._storage: Dict[str, PharmaReportEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: PharmaReportEntity) -> PharmaReportEntity:
    ...
    async def read(self, id: str) -> Optional[PharmaReportEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: PharmaReportEntity) -> PharmaReportEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[PharmaReportEntity]:
    ...
    async def find_by(self, **kwargs) -> List[PharmaReportEntity]:
...
class PharmaReportCachedRepository(BasePharmaRepository[PharmaReportEntity]):
    """Cached repository wrapper for pharma report"""
...
class PharmaReportUnitOfWork:
    """Unit of Work pattern for pharma report"""

    def __init__(self, repository: BasePharmaRepository):
        self._repository = repository
        self._new: List[PharmaReportEntity] = []
        self._dirty: List[PharmaReportEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: PharmaReportEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: PharmaReportEntity) -> None:
