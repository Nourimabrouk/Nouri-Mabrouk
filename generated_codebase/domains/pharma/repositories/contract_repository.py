...
@dataclass
class PharmaContractEntity:
    """Entity for pharma contract"""
    id: str
    data: Dict[str, Any]
...
class PharmaContractInMemoryRepository(BasePharmaRepository[PharmaContractEntity]):
    """In-memory repository for pharma contract"""

    def __init__(self):
        self._storage: Dict[str, PharmaContractEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: PharmaContractEntity) -> PharmaContractEntity:
    ...
    async def read(self, id: str) -> Optional[PharmaContractEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: PharmaContractEntity) -> PharmaContractEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[PharmaContractEntity]:
    ...
    async def find_by(self, **kwargs) -> List[PharmaContractEntity]:
...
class PharmaContractCachedRepository(BasePharmaRepository[PharmaContractEntity]):
    """Cached repository wrapper for pharma contract"""
...
class PharmaContractUnitOfWork:
    """Unit of Work pattern for pharma contract"""

    def __init__(self, repository: BasePharmaRepository):
        self._repository = repository
        self._new: List[PharmaContractEntity] = []
        self._dirty: List[PharmaContractEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: PharmaContractEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: PharmaContractEntity) -> None:
