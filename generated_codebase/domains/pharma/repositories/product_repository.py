...
@dataclass
class PharmaProductEntity:
    """Entity for pharma product"""
    id: str
    data: Dict[str, Any]
...
class PharmaProductInMemoryRepository(BasePharmaRepository[PharmaProductEntity]):
    """In-memory repository for pharma product"""

    def __init__(self):
        self._storage: Dict[str, PharmaProductEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: PharmaProductEntity) -> PharmaProductEntity:
    ...
    async def read(self, id: str) -> Optional[PharmaProductEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: PharmaProductEntity) -> PharmaProductEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[PharmaProductEntity]:
    ...
    async def find_by(self, **kwargs) -> List[PharmaProductEntity]:
...
class PharmaProductCachedRepository(BasePharmaRepository[PharmaProductEntity]):
    """Cached repository wrapper for pharma product"""
...
class PharmaProductUnitOfWork:
    """Unit of Work pattern for pharma product"""

    def __init__(self, repository: BasePharmaRepository):
        self._repository = repository
        self._new: List[PharmaProductEntity] = []
        self._dirty: List[PharmaProductEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: PharmaProductEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: PharmaProductEntity) -> None:
