"""
Healthcare Customer Repository
...
@dataclass
class HealthcareCustomerEntity:
    """Entity for healthcare customer"""
    id: str
    data: Dict[str, Any]
...
class HealthcareCustomerInMemoryRepository(BaseHealthcareRepository[HealthcareCustomerEntity]):
    """In-memory repository for healthcare customer"""

    def __init__(self):
        self._storage: Dict[str, HealthcareCustomerEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: HealthcareCustomerEntity) -> HealthcareCustomerEntity:
    ...
    async def read(self, id: str) -> Optional[HealthcareCustomerEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: HealthcareCustomerEntity) -> HealthcareCustomerEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[HealthcareCustomerEntity]:
    ...
    async def find_by(self, **kwargs) -> List[HealthcareCustomerEntity]:
...
class HealthcareCustomerCachedRepository(BaseHealthcareRepository[HealthcareCustomerEntity]):
    """Cached repository wrapper for healthcare customer"""
...
class HealthcareCustomerUnitOfWork:
    """Unit of Work pattern for healthcare customer"""

    def __init__(self, repository: BaseHealthcareRepository):
        self._repository = repository
        self._new: List[HealthcareCustomerEntity] = []
        self._dirty: List[HealthcareCustomerEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: HealthcareCustomerEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: HealthcareCustomerEntity) -> None:
