...
@dataclass
class HealthcareOrderEntity:
    """Entity for healthcare order"""
    id: str
    data: Dict[str, Any]
...
class HealthcareOrderInMemoryRepository(BaseHealthcareRepository[HealthcareOrderEntity]):
    """In-memory repository for healthcare order"""

    def __init__(self):
        self._storage: Dict[str, HealthcareOrderEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: HealthcareOrderEntity) -> HealthcareOrderEntity:
    ...
    async def read(self, id: str) -> Optional[HealthcareOrderEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: HealthcareOrderEntity) -> HealthcareOrderEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[HealthcareOrderEntity]:
    ...
    async def find_by(self, **kwargs) -> List[HealthcareOrderEntity]:
...
class HealthcareOrderCachedRepository(BaseHealthcareRepository[HealthcareOrderEntity]):
    """Cached repository wrapper for healthcare order"""
...
class HealthcareOrderUnitOfWork:
    """Unit of Work pattern for healthcare order"""

    def __init__(self, repository: BaseHealthcareRepository):
        self._repository = repository
        self._new: List[HealthcareOrderEntity] = []
        self._dirty: List[HealthcareOrderEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: HealthcareOrderEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: HealthcareOrderEntity) -> None:
