"""
Healthcare Appointment Repository
...
@dataclass
class HealthcareAppointmentEntity:
    """Entity for healthcare appointment"""
    id: str
    data: Dict[str, Any]
...
class BaseHealthcareRepository(ABC, Generic[T]):
    """Base repository for healthcare"""
...
class HealthcareAppointmentInMemoryRepository(BaseHealthcareRepository[HealthcareAppointmentEntity]):
    """In-memory repository for healthcare appointment"""

    def __init__(self):
        self._storage: Dict[str, HealthcareAppointmentEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: HealthcareAppointmentEntity) -> HealthcareAppointmentEntity:
    ...
    async def read(self, id: str) -> Optional[HealthcareAppointmentEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: HealthcareAppointmentEntity) -> HealthcareAppointmentEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[HealthcareAppointmentEntity]:
    ...
    async def find_by(self, **kwargs) -> List[HealthcareAppointmentEntity]:
...
class HealthcareAppointmentCachedRepository(BaseHealthcareRepository[HealthcareAppointmentEntity]):
    """Cached repository wrapper for healthcare appointment"""

    def __init__(self, base_repo: BaseHealthcareRepository, cache_ttl: int = 300):
...
class HealthcareAppointmentUnitOfWork:
    """Unit of Work pattern for healthcare appointment"""

    def __init__(self, repository: BaseHealthcareRepository):
        self._repository = repository
        self._new: List[HealthcareAppointmentEntity] = []
        self._dirty: List[HealthcareAppointmentEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: HealthcareAppointmentEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: HealthcareAppointmentEntity) -> None:
