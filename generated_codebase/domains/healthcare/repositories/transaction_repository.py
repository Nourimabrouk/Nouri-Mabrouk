...
@dataclass
class HealthcareTransactionEntity:
    """Entity for healthcare transaction"""
    id: str
    data: Dict[str, Any]
...
class HealthcareTransactionInMemoryRepository(BaseHealthcareRepository[HealthcareTransactionEntity]):
    """In-memory repository for healthcare transaction"""

    def __init__(self):
        self._storage: Dict[str, HealthcareTransactionEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: HealthcareTransactionEntity) -> HealthcareTransactionEntity:
    ...
    async def read(self, id: str) -> Optional[HealthcareTransactionEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: HealthcareTransactionEntity) -> HealthcareTransactionEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[HealthcareTransactionEntity]:
    ...
    async def find_by(self, **kwargs) -> List[HealthcareTransactionEntity]:
...
class HealthcareTransactionCachedRepository(BaseHealthcareRepository[HealthcareTransactionEntity]):
    """Cached repository wrapper for healthcare transaction"""
...
class HealthcareTransactionUnitOfWork:
    """Unit of Work pattern for healthcare transaction"""

    def __init__(self, repository: BaseHealthcareRepository):
        self._repository = repository
        self._new: List[HealthcareTransactionEntity] = []
        self._dirty: List[HealthcareTransactionEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: HealthcareTransactionEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: HealthcareTransactionEntity) -> None:
