...
@dataclass
class FinanceTicketEntity:
    """Entity for finance ticket"""
    id: str
    data: Dict[str, Any]
...
class FinanceTicketInMemoryRepository(BaseFinanceRepository[FinanceTicketEntity]):
    """In-memory repository for finance ticket"""

    def __init__(self):
        self._storage: Dict[str, FinanceTicketEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: FinanceTicketEntity) -> FinanceTicketEntity:
    ...
    async def read(self, id: str) -> Optional[FinanceTicketEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: FinanceTicketEntity) -> FinanceTicketEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[FinanceTicketEntity]:
    ...
    async def find_by(self, **kwargs) -> List[FinanceTicketEntity]:
...
class FinanceTicketCachedRepository(BaseFinanceRepository[FinanceTicketEntity]):
    """Cached repository wrapper for finance ticket"""
...
class FinanceTicketUnitOfWork:
    """Unit of Work pattern for finance ticket"""

    def __init__(self, repository: BaseFinanceRepository):
        self._repository = repository
        self._new: List[FinanceTicketEntity] = []
        self._dirty: List[FinanceTicketEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: FinanceTicketEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: FinanceTicketEntity) -> None:
