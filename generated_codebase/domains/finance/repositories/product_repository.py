...
class FinanceProductInMemoryRepository(BaseFinanceRepository[FinanceProductEntity]):
    """In-memory repository for finance product"""

    def __init__(self):
        self._storage: Dict[str, FinanceProductEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: FinanceProductEntity) -> FinanceProductEntity:
    ...
    async def read(self, id: str) -> Optional[FinanceProductEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: FinanceProductEntity) -> FinanceProductEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[FinanceProductEntity]:
    ...
    async def find_by(self, **kwargs) -> List[FinanceProductEntity]:
...
class FinanceProductCachedRepository(BaseFinanceRepository[FinanceProductEntity]):
    """Cached repository wrapper for finance product"""
...
class FinanceProductUnitOfWork:
    """Unit of Work pattern for finance product"""

    def __init__(self, repository: BaseFinanceRepository):
        self._repository = repository
        self._new: List[FinanceProductEntity] = []
        self._dirty: List[FinanceProductEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: FinanceProductEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: FinanceProductEntity) -> None:
