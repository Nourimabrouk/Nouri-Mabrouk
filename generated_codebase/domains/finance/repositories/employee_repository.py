...
@dataclass
class FinanceEmployeeEntity:
    """Entity for finance employee"""
    id: str
    data: Dict[str, Any]
...
class FinanceEmployeeInMemoryRepository(BaseFinanceRepository[FinanceEmployeeEntity]):
    """In-memory repository for finance employee"""

    def __init__(self):
        self._storage: Dict[str, FinanceEmployeeEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: FinanceEmployeeEntity) -> FinanceEmployeeEntity:
    ...
    async def read(self, id: str) -> Optional[FinanceEmployeeEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: FinanceEmployeeEntity) -> FinanceEmployeeEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[FinanceEmployeeEntity]:
    ...
    async def find_by(self, **kwargs) -> List[FinanceEmployeeEntity]:
...
class FinanceEmployeeCachedRepository(BaseFinanceRepository[FinanceEmployeeEntity]):
    """Cached repository wrapper for finance employee"""
...
class FinanceEmployeeUnitOfWork:
    """Unit of Work pattern for finance employee"""

    def __init__(self, repository: BaseFinanceRepository):
        self._repository = repository
        self._new: List[FinanceEmployeeEntity] = []
        self._dirty: List[FinanceEmployeeEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: FinanceEmployeeEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: FinanceEmployeeEntity) -> None:
