...
@dataclass
class FinanceContractEntity:
    """Entity for finance contract"""
    id: str
    data: Dict[str, Any]
...
class FinanceContractInMemoryRepository(BaseFinanceRepository[FinanceContractEntity]):
    """In-memory repository for finance contract"""

    def __init__(self):
        self._storage: Dict[str, FinanceContractEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: FinanceContractEntity) -> FinanceContractEntity:
    ...
    async def read(self, id: str) -> Optional[FinanceContractEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: FinanceContractEntity) -> FinanceContractEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[FinanceContractEntity]:
    ...
    async def find_by(self, **kwargs) -> List[FinanceContractEntity]:
...
class FinanceContractCachedRepository(BaseFinanceRepository[FinanceContractEntity]):
    """Cached repository wrapper for finance contract"""
...
class FinanceContractUnitOfWork:
    """Unit of Work pattern for finance contract"""

    def __init__(self, repository: BaseFinanceRepository):
        self._repository = repository
        self._new: List[FinanceContractEntity] = []
        self._dirty: List[FinanceContractEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: FinanceContractEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: FinanceContractEntity) -> None:
