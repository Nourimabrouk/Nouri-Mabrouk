...
class FinanceTransactionEntity:
    """Entity for finance transaction"""
    id: str
    data: Dict[str, Any]
...
class FinanceTransactionInMemoryRepository(BaseFinanceRepository[FinanceTransactionEntity]):
    """In-memory repository for finance transaction"""

    def __init__(self):
        self._storage: Dict[str, FinanceTransactionEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: FinanceTransactionEntity) -> FinanceTransactionEntity:
    ...
    async def read(self, id: str) -> Optional[FinanceTransactionEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: FinanceTransactionEntity) -> FinanceTransactionEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[FinanceTransactionEntity]:
    ...
    async def find_by(self, **kwargs) -> List[FinanceTransactionEntity]:
...
class FinanceTransactionCachedRepository(BaseFinanceRepository[FinanceTransactionEntity]):
    """Cached repository wrapper for finance transaction"""
...
class FinanceTransactionUnitOfWork:
    """Unit of Work pattern for finance transaction"""

    def __init__(self, repository: BaseFinanceRepository):
        self._repository = repository
        self._new: List[FinanceTransactionEntity] = []
        self._dirty: List[FinanceTransactionEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: FinanceTransactionEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: FinanceTransactionEntity) -> None:
