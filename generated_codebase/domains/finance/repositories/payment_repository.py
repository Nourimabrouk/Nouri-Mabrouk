...
@dataclass
class FinancePaymentEntity:
    """Entity for finance payment"""
    id: str
    data: Dict[str, Any]
...
class FinancePaymentInMemoryRepository(BaseFinanceRepository[FinancePaymentEntity]):
    """In-memory repository for finance payment"""

    def __init__(self):
        self._storage: Dict[str, FinancePaymentEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: FinancePaymentEntity) -> FinancePaymentEntity:
    ...
    async def read(self, id: str) -> Optional[FinancePaymentEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: FinancePaymentEntity) -> FinancePaymentEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[FinancePaymentEntity]:
    ...
    async def find_by(self, **kwargs) -> List[FinancePaymentEntity]:
...
class FinancePaymentCachedRepository(BaseFinanceRepository[FinancePaymentEntity]):
    """Cached repository wrapper for finance payment"""
...
class FinancePaymentUnitOfWork:
    """Unit of Work pattern for finance payment"""

    def __init__(self, repository: BaseFinanceRepository):
        self._repository = repository
        self._new: List[FinancePaymentEntity] = []
        self._dirty: List[FinancePaymentEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: FinancePaymentEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: FinancePaymentEntity) -> None:
