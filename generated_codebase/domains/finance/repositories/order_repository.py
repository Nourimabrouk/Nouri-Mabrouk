...
@dataclass
class FinanceOrderEntity:
    """Entity for finance order"""
    id: str
    data: Dict[str, Any]
...
class FinanceOrderInMemoryRepository(BaseFinanceRepository[FinanceOrderEntity]):
    """In-memory repository for finance order"""

    def __init__(self):
        self._storage: Dict[str, FinanceOrderEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: FinanceOrderEntity) -> FinanceOrderEntity:
    ...
    async def read(self, id: str) -> Optional[FinanceOrderEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: FinanceOrderEntity) -> FinanceOrderEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[FinanceOrderEntity]:
    ...
    async def find_by(self, **kwargs) -> List[FinanceOrderEntity]:
...
class FinanceOrderCachedRepository(BaseFinanceRepository[FinanceOrderEntity]):
    """Cached repository wrapper for finance order"""
...
class FinanceOrderUnitOfWork:
    """Unit of Work pattern for finance order"""

    def __init__(self, repository: BaseFinanceRepository):
        self._repository = repository
        self._new: List[FinanceOrderEntity] = []
        self._dirty: List[FinanceOrderEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: FinanceOrderEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: FinanceOrderEntity) -> None:
