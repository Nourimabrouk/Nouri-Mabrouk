"""
Finance Appointment Repository
...
@dataclass
class FinanceAppointmentEntity:
    """Entity for finance appointment"""
    id: str
    data: Dict[str, Any]
...
class BaseFinanceRepository(ABC, Generic[T]):
    """Base repository for finance"""
...
class FinanceAppointmentInMemoryRepository(BaseFinanceRepository[FinanceAppointmentEntity]):
    """In-memory repository for finance appointment"""

    def __init__(self):
        self._storage: Dict[str, FinanceAppointmentEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: FinanceAppointmentEntity) -> FinanceAppointmentEntity:
    ...
    async def read(self, id: str) -> Optional[FinanceAppointmentEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: FinanceAppointmentEntity) -> FinanceAppointmentEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[FinanceAppointmentEntity]:
    ...
    async def find_by(self, **kwargs) -> List[FinanceAppointmentEntity]:
...
class FinanceAppointmentCachedRepository(BaseFinanceRepository[FinanceAppointmentEntity]):
    """Cached repository wrapper for finance appointment"""

    def __init__(self, base_repo: BaseFinanceRepository, cache_ttl: int = 300):
...
class FinanceAppointmentUnitOfWork:
    """Unit of Work pattern for finance appointment"""

    def __init__(self, repository: BaseFinanceRepository):
        self._repository = repository
        self._new: List[FinanceAppointmentEntity] = []
        self._dirty: List[FinanceAppointmentEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: FinanceAppointmentEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: FinanceAppointmentEntity) -> None:
