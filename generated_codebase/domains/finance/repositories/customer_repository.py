...
@dataclass
class FinanceCustomerEntity:
    """Entity for finance customer"""
    id: str
    data: Dict[str, Any]
...
class FinanceCustomerInMemoryRepository(BaseFinanceRepository[FinanceCustomerEntity]):
    """In-memory repository for finance customer"""

    def __init__(self):
        self._storage: Dict[str, FinanceCustomerEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: FinanceCustomerEntity) -> FinanceCustomerEntity:
    ...
    async def read(self, id: str) -> Optional[FinanceCustomerEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: FinanceCustomerEntity) -> FinanceCustomerEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[FinanceCustomerEntity]:
    ...
    async def find_by(self, **kwargs) -> List[FinanceCustomerEntity]:
...
class FinanceCustomerCachedRepository(BaseFinanceRepository[FinanceCustomerEntity]):
    """Cached repository wrapper for finance customer"""
...
class FinanceCustomerUnitOfWork:
    """Unit of Work pattern for finance customer"""

    def __init__(self, repository: BaseFinanceRepository):
        self._repository = repository
        self._new: List[FinanceCustomerEntity] = []
        self._dirty: List[FinanceCustomerEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: FinanceCustomerEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: FinanceCustomerEntity) -> None:
