...
@dataclass
class FinanceShipmentEntity:
    """Entity for finance shipment"""
    id: str
    data: Dict[str, Any]
...
class FinanceShipmentInMemoryRepository(BaseFinanceRepository[FinanceShipmentEntity]):
    """In-memory repository for finance shipment"""

    def __init__(self):
        self._storage: Dict[str, FinanceShipmentEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: FinanceShipmentEntity) -> FinanceShipmentEntity:
    ...
    async def read(self, id: str) -> Optional[FinanceShipmentEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: FinanceShipmentEntity) -> FinanceShipmentEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[FinanceShipmentEntity]:
    ...
    async def find_by(self, **kwargs) -> List[FinanceShipmentEntity]:
...
class FinanceShipmentCachedRepository(BaseFinanceRepository[FinanceShipmentEntity]):
    """Cached repository wrapper for finance shipment"""
...
class FinanceShipmentUnitOfWork:
    """Unit of Work pattern for finance shipment"""

    def __init__(self, repository: BaseFinanceRepository):
        self._repository = repository
        self._new: List[FinanceShipmentEntity] = []
        self._dirty: List[FinanceShipmentEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: FinanceShipmentEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: FinanceShipmentEntity) -> None:
