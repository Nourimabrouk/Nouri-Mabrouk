...
@dataclass
class FinanceSupplierEntity:
    """Entity for finance supplier"""
    id: str
    data: Dict[str, Any]
...
class FinanceSupplierInMemoryRepository(BaseFinanceRepository[FinanceSupplierEntity]):
    """In-memory repository for finance supplier"""

    def __init__(self):
        self._storage: Dict[str, FinanceSupplierEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: FinanceSupplierEntity) -> FinanceSupplierEntity:
    ...
    async def read(self, id: str) -> Optional[FinanceSupplierEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: FinanceSupplierEntity) -> FinanceSupplierEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[FinanceSupplierEntity]:
    ...
    async def find_by(self, **kwargs) -> List[FinanceSupplierEntity]:
...
class FinanceSupplierCachedRepository(BaseFinanceRepository[FinanceSupplierEntity]):
    """Cached repository wrapper for finance supplier"""
...
class FinanceSupplierUnitOfWork:
    """Unit of Work pattern for finance supplier"""

    def __init__(self, repository: BaseFinanceRepository):
        self._repository = repository
        self._new: List[FinanceSupplierEntity] = []
        self._dirty: List[FinanceSupplierEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: FinanceSupplierEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: FinanceSupplierEntity) -> None:
