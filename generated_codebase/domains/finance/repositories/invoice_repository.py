...
@dataclass
class FinanceInvoiceEntity:
    """Entity for finance invoice"""
    id: str
    data: Dict[str, Any]
...
class FinanceInvoiceInMemoryRepository(BaseFinanceRepository[FinanceInvoiceEntity]):
    """In-memory repository for finance invoice"""

    def __init__(self):
        self._storage: Dict[str, FinanceInvoiceEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: FinanceInvoiceEntity) -> FinanceInvoiceEntity:
    ...
    async def read(self, id: str) -> Optional[FinanceInvoiceEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: FinanceInvoiceEntity) -> FinanceInvoiceEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[FinanceInvoiceEntity]:
    ...
    async def find_by(self, **kwargs) -> List[FinanceInvoiceEntity]:
...
class FinanceInvoiceCachedRepository(BaseFinanceRepository[FinanceInvoiceEntity]):
    """Cached repository wrapper for finance invoice"""
...
class FinanceInvoiceUnitOfWork:
    """Unit of Work pattern for finance invoice"""

    def __init__(self, repository: BaseFinanceRepository):
        self._repository = repository
        self._new: List[FinanceInvoiceEntity] = []
        self._dirty: List[FinanceInvoiceEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: FinanceInvoiceEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: FinanceInvoiceEntity) -> None:
