"""
Government Transaction Repository
...
@dataclass
class GovernmentTransactionEntity:
    """Entity for government transaction"""
    id: str
    data: Dict[str, Any]
...
class GovernmentTransactionInMemoryRepository(BaseGovernmentRepository[GovernmentTransactionEntity]):
    """In-memory repository for government transaction"""

    def __init__(self):
        self._storage: Dict[str, GovernmentTransactionEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: GovernmentTransactionEntity) -> GovernmentTransactionEntity:
    ...
    async def read(self, id: str) -> Optional[GovernmentTransactionEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: GovernmentTransactionEntity) -> GovernmentTransactionEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[GovernmentTransactionEntity]:
    ...
    async def find_by(self, **kwargs) -> List[GovernmentTransactionEntity]:
...
class GovernmentTransactionCachedRepository(BaseGovernmentRepository[GovernmentTransactionEntity]):
    """Cached repository wrapper for government transaction"""
...
class GovernmentTransactionUnitOfWork:
    """Unit of Work pattern for government transaction"""

    def __init__(self, repository: BaseGovernmentRepository):
        self._repository = repository
        self._new: List[GovernmentTransactionEntity] = []
        self._dirty: List[GovernmentTransactionEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: GovernmentTransactionEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: GovernmentTransactionEntity) -> None:
