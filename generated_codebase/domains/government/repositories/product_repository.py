...
@dataclass
class GovernmentProductEntity:
    """Entity for government product"""
    id: str
    data: Dict[str, Any]
...
class GovernmentProductInMemoryRepository(BaseGovernmentRepository[GovernmentProductEntity]):
    """In-memory repository for government product"""

    def __init__(self):
        self._storage: Dict[str, GovernmentProductEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: GovernmentProductEntity) -> GovernmentProductEntity:
    ...
    async def read(self, id: str) -> Optional[GovernmentProductEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: GovernmentProductEntity) -> GovernmentProductEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[GovernmentProductEntity]:
    ...
    async def find_by(self, **kwargs) -> List[GovernmentProductEntity]:
...
class GovernmentProductCachedRepository(BaseGovernmentRepository[GovernmentProductEntity]):
    """Cached repository wrapper for government product"""
...
class GovernmentProductUnitOfWork:
    """Unit of Work pattern for government product"""

    def __init__(self, repository: BaseGovernmentRepository):
        self._repository = repository
        self._new: List[GovernmentProductEntity] = []
        self._dirty: List[GovernmentProductEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: GovernmentProductEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: GovernmentProductEntity) -> None:
