"""
Government Appointment Repository
...
@dataclass
class GovernmentAppointmentEntity:
    """Entity for government appointment"""
    id: str
    data: Dict[str, Any]
...
class BaseGovernmentRepository(ABC, Generic[T]):
    """Base repository for government"""
...
class GovernmentAppointmentInMemoryRepository(BaseGovernmentRepository[GovernmentAppointmentEntity]):
    """In-memory repository for government appointment"""

    def __init__(self):
        self._storage: Dict[str, GovernmentAppointmentEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: GovernmentAppointmentEntity) -> GovernmentAppointmentEntity:
    ...
    async def read(self, id: str) -> Optional[GovernmentAppointmentEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: GovernmentAppointmentEntity) -> GovernmentAppointmentEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[GovernmentAppointmentEntity]:
    ...
    async def find_by(self, **kwargs) -> List[GovernmentAppointmentEntity]:
...
class GovernmentAppointmentCachedRepository(BaseGovernmentRepository[GovernmentAppointmentEntity]):
    """Cached repository wrapper for government appointment"""

    def __init__(self, base_repo: BaseGovernmentRepository, cache_ttl: int = 300):
...
class GovernmentAppointmentUnitOfWork:
    """Unit of Work pattern for government appointment"""

    def __init__(self, repository: BaseGovernmentRepository):
        self._repository = repository
        self._new: List[GovernmentAppointmentEntity] = []
        self._dirty: List[GovernmentAppointmentEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: GovernmentAppointmentEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: GovernmentAppointmentEntity) -> None:
