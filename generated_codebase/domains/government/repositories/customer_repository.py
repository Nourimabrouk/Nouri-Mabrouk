"""
Government Customer Repository
...
@dataclass
class GovernmentCustomerEntity:
    """Entity for government customer"""
    id: str
    data: Dict[str, Any]
...
class GovernmentCustomerInMemoryRepository(BaseGovernmentRepository[GovernmentCustomerEntity]):
    """In-memory repository for government customer"""

    def __init__(self):
        self._storage: Dict[str, GovernmentCustomerEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: GovernmentCustomerEntity) -> GovernmentCustomerEntity:
    ...
    async def read(self, id: str) -> Optional[GovernmentCustomerEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: GovernmentCustomerEntity) -> GovernmentCustomerEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[GovernmentCustomerEntity]:
    ...
    async def find_by(self, **kwargs) -> List[GovernmentCustomerEntity]:
...
class GovernmentCustomerCachedRepository(BaseGovernmentRepository[GovernmentCustomerEntity]):
    """Cached repository wrapper for government customer"""
...
class GovernmentCustomerUnitOfWork:
    """Unit of Work pattern for government customer"""

    def __init__(self, repository: BaseGovernmentRepository):
        self._repository = repository
        self._new: List[GovernmentCustomerEntity] = []
        self._dirty: List[GovernmentCustomerEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: GovernmentCustomerEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: GovernmentCustomerEntity) -> None:
