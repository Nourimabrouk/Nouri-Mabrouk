"""
Government Employee Repository
...
@dataclass
class GovernmentEmployeeEntity:
    """Entity for government employee"""
    id: str
    data: Dict[str, Any]
...
class GovernmentEmployeeInMemoryRepository(BaseGovernmentRepository[GovernmentEmployeeEntity]):
    """In-memory repository for government employee"""

    def __init__(self):
        self._storage: Dict[str, GovernmentEmployeeEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: GovernmentEmployeeEntity) -> GovernmentEmployeeEntity:
    ...
    async def read(self, id: str) -> Optional[GovernmentEmployeeEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: GovernmentEmployeeEntity) -> GovernmentEmployeeEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[GovernmentEmployeeEntity]:
    ...
    async def find_by(self, **kwargs) -> List[GovernmentEmployeeEntity]:
...
class GovernmentEmployeeCachedRepository(BaseGovernmentRepository[GovernmentEmployeeEntity]):
    """Cached repository wrapper for government employee"""
...
class GovernmentEmployeeUnitOfWork:
    """Unit of Work pattern for government employee"""

    def __init__(self, repository: BaseGovernmentRepository):
        self._repository = repository
        self._new: List[GovernmentEmployeeEntity] = []
        self._dirty: List[GovernmentEmployeeEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: GovernmentEmployeeEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: GovernmentEmployeeEntity) -> None:
