...
@dataclass
class GovernmentPaymentEntity:
    """Entity for government payment"""
    id: str
    data: Dict[str, Any]
...
class GovernmentPaymentInMemoryRepository(BaseGovernmentRepository[GovernmentPaymentEntity]):
    """In-memory repository for government payment"""

    def __init__(self):
        self._storage: Dict[str, GovernmentPaymentEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: GovernmentPaymentEntity) -> GovernmentPaymentEntity:
    ...
    async def read(self, id: str) -> Optional[GovernmentPaymentEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: GovernmentPaymentEntity) -> GovernmentPaymentEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[GovernmentPaymentEntity]:
    ...
    async def find_by(self, **kwargs) -> List[GovernmentPaymentEntity]:
...
class GovernmentPaymentCachedRepository(BaseGovernmentRepository[GovernmentPaymentEntity]):
    """Cached repository wrapper for government payment"""
...
class GovernmentPaymentUnitOfWork:
    """Unit of Work pattern for government payment"""

    def __init__(self, repository: BaseGovernmentRepository):
        self._repository = repository
        self._new: List[GovernmentPaymentEntity] = []
        self._dirty: List[GovernmentPaymentEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: GovernmentPaymentEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: GovernmentPaymentEntity) -> None:
