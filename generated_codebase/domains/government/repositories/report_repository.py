...
@dataclass
class GovernmentReportEntity:
    """Entity for government report"""
    id: str
    data: Dict[str, Any]
...
class GovernmentReportInMemoryRepository(BaseGovernmentRepository[GovernmentReportEntity]):
    """In-memory repository for government report"""

    def __init__(self):
        self._storage: Dict[str, GovernmentReportEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: GovernmentReportEntity) -> GovernmentReportEntity:
    ...
    async def read(self, id: str) -> Optional[GovernmentReportEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: GovernmentReportEntity) -> GovernmentReportEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[GovernmentReportEntity]:
    ...
    async def find_by(self, **kwargs) -> List[GovernmentReportEntity]:
...
class GovernmentReportCachedRepository(BaseGovernmentRepository[GovernmentReportEntity]):
    """Cached repository wrapper for government report"""
...
class GovernmentReportUnitOfWork:
    """Unit of Work pattern for government report"""

    def __init__(self, repository: BaseGovernmentRepository):
        self._repository = repository
        self._new: List[GovernmentReportEntity] = []
        self._dirty: List[GovernmentReportEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: GovernmentReportEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: GovernmentReportEntity) -> None:
