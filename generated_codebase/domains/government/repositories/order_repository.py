...
@dataclass
class GovernmentOrderEntity:
    """Entity for government order"""
    id: str
    data: Dict[str, Any]
...
class GovernmentOrderInMemoryRepository(BaseGovernmentRepository[GovernmentOrderEntity]):
    """In-memory repository for government order"""

    def __init__(self):
        self._storage: Dict[str, GovernmentOrderEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: GovernmentOrderEntity) -> GovernmentOrderEntity:
    ...
    async def read(self, id: str) -> Optional[GovernmentOrderEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: GovernmentOrderEntity) -> GovernmentOrderEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[GovernmentOrderEntity]:
    ...
    async def find_by(self, **kwargs) -> List[GovernmentOrderEntity]:
...
class GovernmentOrderCachedRepository(BaseGovernmentRepository[GovernmentOrderEntity]):
    """Cached repository wrapper for government order"""
...
class GovernmentOrderUnitOfWork:
    """Unit of Work pattern for government order"""

    def __init__(self, repository: BaseGovernmentRepository):
        self._repository = repository
        self._new: List[GovernmentOrderEntity] = []
        self._dirty: List[GovernmentOrderEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: GovernmentOrderEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: GovernmentOrderEntity) -> None:
