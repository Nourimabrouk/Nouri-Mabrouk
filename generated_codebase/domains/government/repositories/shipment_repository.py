"""
Government Shipment Repository
...
@dataclass
class GovernmentShipmentEntity:
    """Entity for government shipment"""
    id: str
    data: Dict[str, Any]
...
class GovernmentShipmentInMemoryRepository(BaseGovernmentRepository[GovernmentShipmentEntity]):
    """In-memory repository for government shipment"""

    def __init__(self):
        self._storage: Dict[str, GovernmentShipmentEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: GovernmentShipmentEntity) -> GovernmentShipmentEntity:
    ...
    async def read(self, id: str) -> Optional[GovernmentShipmentEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: GovernmentShipmentEntity) -> GovernmentShipmentEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[GovernmentShipmentEntity]:
    ...
    async def find_by(self, **kwargs) -> List[GovernmentShipmentEntity]:
...
class GovernmentShipmentCachedRepository(BaseGovernmentRepository[GovernmentShipmentEntity]):
    """Cached repository wrapper for government shipment"""
...
class GovernmentShipmentUnitOfWork:
    """Unit of Work pattern for government shipment"""

    def __init__(self, repository: BaseGovernmentRepository):
        self._repository = repository
        self._new: List[GovernmentShipmentEntity] = []
        self._dirty: List[GovernmentShipmentEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: GovernmentShipmentEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: GovernmentShipmentEntity) -> None:
