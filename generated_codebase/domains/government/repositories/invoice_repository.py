...
@dataclass
class GovernmentInvoiceEntity:
    """Entity for government invoice"""
    id: str
    data: Dict[str, Any]
...
class GovernmentInvoiceInMemoryRepository(BaseGovernmentRepository[GovernmentInvoiceEntity]):
    """In-memory repository for government invoice"""

    def __init__(self):
        self._storage: Dict[str, GovernmentInvoiceEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: GovernmentInvoiceEntity) -> GovernmentInvoiceEntity:
    ...
    async def read(self, id: str) -> Optional[GovernmentInvoiceEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: GovernmentInvoiceEntity) -> GovernmentInvoiceEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[GovernmentInvoiceEntity]:
    ...
    async def find_by(self, **kwargs) -> List[GovernmentInvoiceEntity]:
...
class GovernmentInvoiceCachedRepository(BaseGovernmentRepository[GovernmentInvoiceEntity]):
    """Cached repository wrapper for government invoice"""
...
class GovernmentInvoiceUnitOfWork:
    """Unit of Work pattern for government invoice"""

    def __init__(self, repository: BaseGovernmentRepository):
        self._repository = repository
        self._new: List[GovernmentInvoiceEntity] = []
        self._dirty: List[GovernmentInvoiceEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: GovernmentInvoiceEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: GovernmentInvoiceEntity) -> None:
