"""
Government Inventory Repository
...
@dataclass
class GovernmentInventoryEntity:
    """Entity for government inventory"""
    id: str
    data: Dict[str, Any]
...
class GovernmentInventoryInMemoryRepository(BaseGovernmentRepository[GovernmentInventoryEntity]):
    """In-memory repository for government inventory"""

    def __init__(self):
        self._storage: Dict[str, GovernmentInventoryEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: GovernmentInventoryEntity) -> GovernmentInventoryEntity:
    ...
    async def read(self, id: str) -> Optional[GovernmentInventoryEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: GovernmentInventoryEntity) -> GovernmentInventoryEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[GovernmentInventoryEntity]:
    ...
    async def find_by(self, **kwargs) -> List[GovernmentInventoryEntity]:
...
class GovernmentInventoryCachedRepository(BaseGovernmentRepository[GovernmentInventoryEntity]):
    """Cached repository wrapper for government inventory"""
...
class GovernmentInventoryUnitOfWork:
    """Unit of Work pattern for government inventory"""

    def __init__(self, repository: BaseGovernmentRepository):
        self._repository = repository
        self._new: List[GovernmentInventoryEntity] = []
        self._dirty: List[GovernmentInventoryEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: GovernmentInventoryEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: GovernmentInventoryEntity) -> None:
