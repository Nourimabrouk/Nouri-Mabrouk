"""
Government Assessment Repository
...
@dataclass
class GovernmentAssessmentEntity:
    """Entity for government assessment"""
    id: str
    data: Dict[str, Any]
...
class GovernmentAssessmentInMemoryRepository(BaseGovernmentRepository[GovernmentAssessmentEntity]):
    """In-memory repository for government assessment"""

    def __init__(self):
        self._storage: Dict[str, GovernmentAssessmentEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: GovernmentAssessmentEntity) -> GovernmentAssessmentEntity:
    ...
    async def read(self, id: str) -> Optional[GovernmentAssessmentEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: GovernmentAssessmentEntity) -> GovernmentAssessmentEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[GovernmentAssessmentEntity]:
    ...
    async def find_by(self, **kwargs) -> List[GovernmentAssessmentEntity]:
...
class GovernmentAssessmentCachedRepository(BaseGovernmentRepository[GovernmentAssessmentEntity]):
    """Cached repository wrapper for government assessment"""
...
class GovernmentAssessmentUnitOfWork:
    """Unit of Work pattern for government assessment"""

    def __init__(self, repository: BaseGovernmentRepository):
        self._repository = repository
        self._new: List[GovernmentAssessmentEntity] = []
        self._dirty: List[GovernmentAssessmentEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: GovernmentAssessmentEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: GovernmentAssessmentEntity) -> None:
