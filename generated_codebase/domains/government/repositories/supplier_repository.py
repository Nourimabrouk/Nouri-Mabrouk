"""
Government Supplier Repository
...
@dataclass
class GovernmentSupplierEntity:
    """Entity for government supplier"""
    id: str
    data: Dict[str, Any]
...
class GovernmentSupplierInMemoryRepository(BaseGovernmentRepository[GovernmentSupplierEntity]):
    """In-memory repository for government supplier"""

    def __init__(self):
        self._storage: Dict[str, GovernmentSupplierEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: GovernmentSupplierEntity) -> GovernmentSupplierEntity:
    ...
    async def read(self, id: str) -> Optional[GovernmentSupplierEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: GovernmentSupplierEntity) -> GovernmentSupplierEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[GovernmentSupplierEntity]:
    ...
    async def find_by(self, **kwargs) -> List[GovernmentSupplierEntity]:
...
class GovernmentSupplierCachedRepository(BaseGovernmentRepository[GovernmentSupplierEntity]):
    """Cached repository wrapper for government supplier"""
...
class GovernmentSupplierUnitOfWork:
    """Unit of Work pattern for government supplier"""

    def __init__(self, repository: BaseGovernmentRepository):
        self._repository = repository
        self._new: List[GovernmentSupplierEntity] = []
        self._dirty: List[GovernmentSupplierEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: GovernmentSupplierEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: GovernmentSupplierEntity) -> None:
