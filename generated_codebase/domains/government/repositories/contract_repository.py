...
@dataclass
class GovernmentContractEntity:
    """Entity for government contract"""
    id: str
    data: Dict[str, Any]
...
class GovernmentContractInMemoryRepository(BaseGovernmentRepository[GovernmentContractEntity]):
    """In-memory repository for government contract"""

    def __init__(self):
        self._storage: Dict[str, GovernmentContractEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: GovernmentContractEntity) -> GovernmentContractEntity:
    ...
    async def read(self, id: str) -> Optional[GovernmentContractEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: GovernmentContractEntity) -> GovernmentContractEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[GovernmentContractEntity]:
    ...
    async def find_by(self, **kwargs) -> List[GovernmentContractEntity]:
...
class GovernmentContractCachedRepository(BaseGovernmentRepository[GovernmentContractEntity]):
    """Cached repository wrapper for government contract"""
...
class GovernmentContractUnitOfWork:
    """Unit of Work pattern for government contract"""

    def __init__(self, repository: BaseGovernmentRepository):
        self._repository = repository
        self._new: List[GovernmentContractEntity] = []
        self._dirty: List[GovernmentContractEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: GovernmentContractEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: GovernmentContractEntity) -> None:
