...
@dataclass
class GovernmentTicketEntity:
    """Entity for government ticket"""
    id: str
    data: Dict[str, Any]
...
class GovernmentTicketInMemoryRepository(BaseGovernmentRepository[GovernmentTicketEntity]):
    """In-memory repository for government ticket"""

    def __init__(self):
        self._storage: Dict[str, GovernmentTicketEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: GovernmentTicketEntity) -> GovernmentTicketEntity:
    ...
    async def read(self, id: str) -> Optional[GovernmentTicketEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: GovernmentTicketEntity) -> GovernmentTicketEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[GovernmentTicketEntity]:
    ...
    async def find_by(self, **kwargs) -> List[GovernmentTicketEntity]:
...
class GovernmentTicketCachedRepository(BaseGovernmentRepository[GovernmentTicketEntity]):
    """Cached repository wrapper for government ticket"""
...
class GovernmentTicketUnitOfWork:
    """Unit of Work pattern for government ticket"""

    def __init__(self, repository: BaseGovernmentRepository):
        self._repository = repository
        self._new: List[GovernmentTicketEntity] = []
        self._dirty: List[GovernmentTicketEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: GovernmentTicketEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: GovernmentTicketEntity) -> None:
