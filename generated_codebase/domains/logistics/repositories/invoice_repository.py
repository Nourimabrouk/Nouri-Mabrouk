...
@dataclass
class LogisticsInvoiceEntity:
    """Entity for logistics invoice"""
    id: str
    data: Dict[str, Any]
...
class LogisticsInvoiceInMemoryRepository(BaseLogisticsRepository[LogisticsInvoiceEntity]):
    """In-memory repository for logistics invoice"""

    def __init__(self):
        self._storage: Dict[str, LogisticsInvoiceEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: LogisticsInvoiceEntity) -> LogisticsInvoiceEntity:
    ...
    async def read(self, id: str) -> Optional[LogisticsInvoiceEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: LogisticsInvoiceEntity) -> LogisticsInvoiceEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[LogisticsInvoiceEntity]:
    ...
    async def find_by(self, **kwargs) -> List[LogisticsInvoiceEntity]:
...
class LogisticsInvoiceCachedRepository(BaseLogisticsRepository[LogisticsInvoiceEntity]):
    """Cached repository wrapper for logistics invoice"""
...
class LogisticsInvoiceUnitOfWork:
    """Unit of Work pattern for logistics invoice"""

    def __init__(self, repository: BaseLogisticsRepository):
        self._repository = repository
        self._new: List[LogisticsInvoiceEntity] = []
        self._dirty: List[LogisticsInvoiceEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: LogisticsInvoiceEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: LogisticsInvoiceEntity) -> None:
