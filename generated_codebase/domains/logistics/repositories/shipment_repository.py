...
@dataclass
class LogisticsShipmentEntity:
    """Entity for logistics shipment"""
    id: str
    data: Dict[str, Any]
...
class LogisticsShipmentInMemoryRepository(BaseLogisticsRepository[LogisticsShipmentEntity]):
    """In-memory repository for logistics shipment"""

    def __init__(self):
        self._storage: Dict[str, LogisticsShipmentEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: LogisticsShipmentEntity) -> LogisticsShipmentEntity:
    ...
    async def read(self, id: str) -> Optional[LogisticsShipmentEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: LogisticsShipmentEntity) -> LogisticsShipmentEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[LogisticsShipmentEntity]:
    ...
    async def find_by(self, **kwargs) -> List[LogisticsShipmentEntity]:
...
class LogisticsShipmentCachedRepository(BaseLogisticsRepository[LogisticsShipmentEntity]):
    """Cached repository wrapper for logistics shipment"""
...
class LogisticsShipmentUnitOfWork:
    """Unit of Work pattern for logistics shipment"""

    def __init__(self, repository: BaseLogisticsRepository):
        self._repository = repository
        self._new: List[LogisticsShipmentEntity] = []
        self._dirty: List[LogisticsShipmentEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: LogisticsShipmentEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: LogisticsShipmentEntity) -> None:
