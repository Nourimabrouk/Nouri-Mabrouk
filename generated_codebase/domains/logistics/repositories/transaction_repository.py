"""
Logistics Transaction Repository
...
@dataclass
class LogisticsTransactionEntity:
    """Entity for logistics transaction"""
    id: str
    data: Dict[str, Any]
...
class LogisticsTransactionInMemoryRepository(BaseLogisticsRepository[LogisticsTransactionEntity]):
    """In-memory repository for logistics transaction"""

    def __init__(self):
        self._storage: Dict[str, LogisticsTransactionEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: LogisticsTransactionEntity) -> LogisticsTransactionEntity:
    ...
    async def read(self, id: str) -> Optional[LogisticsTransactionEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: LogisticsTransactionEntity) -> LogisticsTransactionEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[LogisticsTransactionEntity]:
    ...
    async def find_by(self, **kwargs) -> List[LogisticsTransactionEntity]:
...
class LogisticsTransactionCachedRepository(BaseLogisticsRepository[LogisticsTransactionEntity]):
    """Cached repository wrapper for logistics transaction"""
...
class LogisticsTransactionUnitOfWork:
    """Unit of Work pattern for logistics transaction"""

    def __init__(self, repository: BaseLogisticsRepository):
        self._repository = repository
        self._new: List[LogisticsTransactionEntity] = []
        self._dirty: List[LogisticsTransactionEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: LogisticsTransactionEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: LogisticsTransactionEntity) -> None:
