...
@dataclass
class LogisticsEmployeeEntity:
    """Entity for logistics employee"""
    id: str
    data: Dict[str, Any]
...
class LogisticsEmployeeInMemoryRepository(BaseLogisticsRepository[LogisticsEmployeeEntity]):
    """In-memory repository for logistics employee"""

    def __init__(self):
        self._storage: Dict[str, LogisticsEmployeeEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: LogisticsEmployeeEntity) -> LogisticsEmployeeEntity:
    ...
    async def read(self, id: str) -> Optional[LogisticsEmployeeEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: LogisticsEmployeeEntity) -> LogisticsEmployeeEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[LogisticsEmployeeEntity]:
    ...
    async def find_by(self, **kwargs) -> List[LogisticsEmployeeEntity]:
...
class LogisticsEmployeeCachedRepository(BaseLogisticsRepository[LogisticsEmployeeEntity]):
    """Cached repository wrapper for logistics employee"""
...
class LogisticsEmployeeUnitOfWork:
    """Unit of Work pattern for logistics employee"""

    def __init__(self, repository: BaseLogisticsRepository):
        self._repository = repository
        self._new: List[LogisticsEmployeeEntity] = []
        self._dirty: List[LogisticsEmployeeEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: LogisticsEmployeeEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: LogisticsEmployeeEntity) -> None:
