...
@dataclass
class LogisticsOrderEntity:
    """Entity for logistics order"""
    id: str
    data: Dict[str, Any]
...
class LogisticsOrderInMemoryRepository(BaseLogisticsRepository[LogisticsOrderEntity]):
    """In-memory repository for logistics order"""

    def __init__(self):
        self._storage: Dict[str, LogisticsOrderEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: LogisticsOrderEntity) -> LogisticsOrderEntity:
    ...
    async def read(self, id: str) -> Optional[LogisticsOrderEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: LogisticsOrderEntity) -> LogisticsOrderEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[LogisticsOrderEntity]:
    ...
    async def find_by(self, **kwargs) -> List[LogisticsOrderEntity]:
...
class LogisticsOrderCachedRepository(BaseLogisticsRepository[LogisticsOrderEntity]):
    """Cached repository wrapper for logistics order"""
...
class LogisticsOrderUnitOfWork:
    """Unit of Work pattern for logistics order"""

    def __init__(self, repository: BaseLogisticsRepository):
        self._repository = repository
        self._new: List[LogisticsOrderEntity] = []
        self._dirty: List[LogisticsOrderEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: LogisticsOrderEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: LogisticsOrderEntity) -> None:
