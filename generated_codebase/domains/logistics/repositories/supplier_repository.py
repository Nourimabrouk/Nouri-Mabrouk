...
@dataclass
class LogisticsSupplierEntity:
    """Entity for logistics supplier"""
    id: str
    data: Dict[str, Any]
...
class LogisticsSupplierInMemoryRepository(BaseLogisticsRepository[LogisticsSupplierEntity]):
    """In-memory repository for logistics supplier"""

    def __init__(self):
        self._storage: Dict[str, LogisticsSupplierEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: LogisticsSupplierEntity) -> LogisticsSupplierEntity:
    ...
    async def read(self, id: str) -> Optional[LogisticsSupplierEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: LogisticsSupplierEntity) -> LogisticsSupplierEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[LogisticsSupplierEntity]:
    ...
    async def find_by(self, **kwargs) -> List[LogisticsSupplierEntity]:
...
class LogisticsSupplierCachedRepository(BaseLogisticsRepository[LogisticsSupplierEntity]):
    """Cached repository wrapper for logistics supplier"""
...
class LogisticsSupplierUnitOfWork:
    """Unit of Work pattern for logistics supplier"""

    def __init__(self, repository: BaseLogisticsRepository):
        self._repository = repository
        self._new: List[LogisticsSupplierEntity] = []
        self._dirty: List[LogisticsSupplierEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: LogisticsSupplierEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: LogisticsSupplierEntity) -> None:
