"""
Logistics Appointment Repository
...
@dataclass
class LogisticsAppointmentEntity:
    """Entity for logistics appointment"""
    id: str
    data: Dict[str, Any]
...
class BaseLogisticsRepository(ABC, Generic[T]):
    """Base repository for logistics"""
...
class LogisticsAppointmentInMemoryRepository(BaseLogisticsRepository[LogisticsAppointmentEntity]):
    """In-memory repository for logistics appointment"""

    def __init__(self):
        self._storage: Dict[str, LogisticsAppointmentEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: LogisticsAppointmentEntity) -> LogisticsAppointmentEntity:
    ...
    async def read(self, id: str) -> Optional[LogisticsAppointmentEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: LogisticsAppointmentEntity) -> LogisticsAppointmentEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[LogisticsAppointmentEntity]:
    ...
    async def find_by(self, **kwargs) -> List[LogisticsAppointmentEntity]:
...
class LogisticsAppointmentCachedRepository(BaseLogisticsRepository[LogisticsAppointmentEntity]):
    """Cached repository wrapper for logistics appointment"""

    def __init__(self, base_repo: BaseLogisticsRepository, cache_ttl: int = 300):
...
class LogisticsAppointmentUnitOfWork:
    """Unit of Work pattern for logistics appointment"""

    def __init__(self, repository: BaseLogisticsRepository):
        self._repository = repository
        self._new: List[LogisticsAppointmentEntity] = []
        self._dirty: List[LogisticsAppointmentEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: LogisticsAppointmentEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: LogisticsAppointmentEntity) -> None:
