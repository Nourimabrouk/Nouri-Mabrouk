...
@dataclass
class LogisticsReportEntity:
    """Entity for logistics report"""
    id: str
    data: Dict[str, Any]
...
class LogisticsReportInMemoryRepository(BaseLogisticsRepository[LogisticsReportEntity]):
    """In-memory repository for logistics report"""

    def __init__(self):
        self._storage: Dict[str, LogisticsReportEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: LogisticsReportEntity) -> LogisticsReportEntity:
    ...
    async def read(self, id: str) -> Optional[LogisticsReportEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: LogisticsReportEntity) -> LogisticsReportEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[LogisticsReportEntity]:
    ...
    async def find_by(self, **kwargs) -> List[LogisticsReportEntity]:
...
class LogisticsReportCachedRepository(BaseLogisticsRepository[LogisticsReportEntity]):
    """Cached repository wrapper for logistics report"""
...
class LogisticsReportUnitOfWork:
    """Unit of Work pattern for logistics report"""

    def __init__(self, repository: BaseLogisticsRepository):
        self._repository = repository
        self._new: List[LogisticsReportEntity] = []
        self._dirty: List[LogisticsReportEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: LogisticsReportEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: LogisticsReportEntity) -> None:
