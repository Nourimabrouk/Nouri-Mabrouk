...
@dataclass
class LogisticsPaymentEntity:
    """Entity for logistics payment"""
    id: str
    data: Dict[str, Any]
...
class LogisticsPaymentInMemoryRepository(BaseLogisticsRepository[LogisticsPaymentEntity]):
    """In-memory repository for logistics payment"""

    def __init__(self):
        self._storage: Dict[str, LogisticsPaymentEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: LogisticsPaymentEntity) -> LogisticsPaymentEntity:
    ...
    async def read(self, id: str) -> Optional[LogisticsPaymentEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: LogisticsPaymentEntity) -> LogisticsPaymentEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[LogisticsPaymentEntity]:
    ...
    async def find_by(self, **kwargs) -> List[LogisticsPaymentEntity]:
...
class LogisticsPaymentCachedRepository(BaseLogisticsRepository[LogisticsPaymentEntity]):
    """Cached repository wrapper for logistics payment"""
...
class LogisticsPaymentUnitOfWork:
    """Unit of Work pattern for logistics payment"""

    def __init__(self, repository: BaseLogisticsRepository):
        self._repository = repository
        self._new: List[LogisticsPaymentEntity] = []
        self._dirty: List[LogisticsPaymentEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: LogisticsPaymentEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: LogisticsPaymentEntity) -> None:
