...
@dataclass
class LogisticsProductEntity:
    """Entity for logistics product"""
    id: str
    data: Dict[str, Any]
...
class LogisticsProductInMemoryRepository(BaseLogisticsRepository[LogisticsProductEntity]):
    """In-memory repository for logistics product"""

    def __init__(self):
        self._storage: Dict[str, LogisticsProductEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: LogisticsProductEntity) -> LogisticsProductEntity:
    ...
    async def read(self, id: str) -> Optional[LogisticsProductEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: LogisticsProductEntity) -> LogisticsProductEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[LogisticsProductEntity]:
    ...
    async def find_by(self, **kwargs) -> List[LogisticsProductEntity]:
...
class LogisticsProductCachedRepository(BaseLogisticsRepository[LogisticsProductEntity]):
    """Cached repository wrapper for logistics product"""
...
class LogisticsProductUnitOfWork:
    """Unit of Work pattern for logistics product"""

    def __init__(self, repository: BaseLogisticsRepository):
        self._repository = repository
        self._new: List[LogisticsProductEntity] = []
        self._dirty: List[LogisticsProductEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: LogisticsProductEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: LogisticsProductEntity) -> None:
