...
@dataclass
class LogisticsContractEntity:
    """Entity for logistics contract"""
    id: str
    data: Dict[str, Any]
...
class LogisticsContractInMemoryRepository(BaseLogisticsRepository[LogisticsContractEntity]):
    """In-memory repository for logistics contract"""

    def __init__(self):
        self._storage: Dict[str, LogisticsContractEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: LogisticsContractEntity) -> LogisticsContractEntity:
    ...
    async def read(self, id: str) -> Optional[LogisticsContractEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: LogisticsContractEntity) -> LogisticsContractEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[LogisticsContractEntity]:
    ...
    async def find_by(self, **kwargs) -> List[LogisticsContractEntity]:
...
class LogisticsContractCachedRepository(BaseLogisticsRepository[LogisticsContractEntity]):
    """Cached repository wrapper for logistics contract"""
...
class LogisticsContractUnitOfWork:
    """Unit of Work pattern for logistics contract"""

    def __init__(self, repository: BaseLogisticsRepository):
        self._repository = repository
        self._new: List[LogisticsContractEntity] = []
        self._dirty: List[LogisticsContractEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: LogisticsContractEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: LogisticsContractEntity) -> None:
