...
@dataclass
class LogisticsTicketEntity:
    """Entity for logistics ticket"""
    id: str
    data: Dict[str, Any]
...
class LogisticsTicketInMemoryRepository(BaseLogisticsRepository[LogisticsTicketEntity]):
    """In-memory repository for logistics ticket"""

    def __init__(self):
        self._storage: Dict[str, LogisticsTicketEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: LogisticsTicketEntity) -> LogisticsTicketEntity:
    ...
    async def read(self, id: str) -> Optional[LogisticsTicketEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: LogisticsTicketEntity) -> LogisticsTicketEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[LogisticsTicketEntity]:
    ...
    async def find_by(self, **kwargs) -> List[LogisticsTicketEntity]:
...
class LogisticsTicketCachedRepository(BaseLogisticsRepository[LogisticsTicketEntity]):
    """Cached repository wrapper for logistics ticket"""
...
class LogisticsTicketUnitOfWork:
    """Unit of Work pattern for logistics ticket"""

    def __init__(self, repository: BaseLogisticsRepository):
        self._repository = repository
        self._new: List[LogisticsTicketEntity] = []
        self._dirty: List[LogisticsTicketEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: LogisticsTicketEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: LogisticsTicketEntity) -> None:
