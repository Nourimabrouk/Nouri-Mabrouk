"""
Hospitality Employee Repository
...
@dataclass
class HospitalityEmployeeEntity:
    """Entity for hospitality employee"""
    id: str
    data: Dict[str, Any]
...
class HospitalityEmployeeInMemoryRepository(BaseHospitalityRepository[HospitalityEmployeeEntity]):
    """In-memory repository for hospitality employee"""

    def __init__(self):
        self._storage: Dict[str, HospitalityEmployeeEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: HospitalityEmployeeEntity) -> HospitalityEmployeeEntity:
    ...
    async def read(self, id: str) -> Optional[HospitalityEmployeeEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: HospitalityEmployeeEntity) -> HospitalityEmployeeEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[HospitalityEmployeeEntity]:
    ...
    async def find_by(self, **kwargs) -> List[HospitalityEmployeeEntity]:
...
class HospitalityEmployeeCachedRepository(BaseHospitalityRepository[HospitalityEmployeeEntity]):
    """Cached repository wrapper for hospitality employee"""
...
class HospitalityEmployeeUnitOfWork:
    """Unit of Work pattern for hospitality employee"""

    def __init__(self, repository: BaseHospitalityRepository):
        self._repository = repository
        self._new: List[HospitalityEmployeeEntity] = []
        self._dirty: List[HospitalityEmployeeEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: HospitalityEmployeeEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: HospitalityEmployeeEntity) -> None:
