"""
Hospitality Appointment Repository
...
@dataclass
class HospitalityAppointmentEntity:
    """Entity for hospitality appointment"""
    id: str
    data: Dict[str, Any]
...
class BaseHospitalityRepository(ABC, Generic[T]):
    """Base repository for hospitality"""
...
class HospitalityAppointmentInMemoryRepository(BaseHospitalityRepository[HospitalityAppointmentEntity]):
    """In-memory repository for hospitality appointment"""

    def __init__(self):
        self._storage: Dict[str, HospitalityAppointmentEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: HospitalityAppointmentEntity) -> HospitalityAppointmentEntity:
    ...
    async def read(self, id: str) -> Optional[HospitalityAppointmentEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: HospitalityAppointmentEntity) -> HospitalityAppointmentEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[HospitalityAppointmentEntity]:
    ...
    async def find_by(self, **kwargs) -> List[HospitalityAppointmentEntity]:
...
class HospitalityAppointmentCachedRepository(BaseHospitalityRepository[HospitalityAppointmentEntity]):
    """Cached repository wrapper for hospitality appointment"""

    def __init__(self, base_repo: BaseHospitalityRepository, cache_ttl: int = 300):
...
class HospitalityAppointmentUnitOfWork:
    """Unit of Work pattern for hospitality appointment"""

    def __init__(self, repository: BaseHospitalityRepository):
        self._repository = repository
        self._new: List[HospitalityAppointmentEntity] = []
        self._dirty: List[HospitalityAppointmentEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: HospitalityAppointmentEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: HospitalityAppointmentEntity) -> None:
