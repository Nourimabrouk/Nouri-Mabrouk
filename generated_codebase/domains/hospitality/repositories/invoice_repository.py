"""
Hospitality Invoice Repository
...
@dataclass
class HospitalityInvoiceEntity:
    """Entity for hospitality invoice"""
    id: str
    data: Dict[str, Any]
...
class HospitalityInvoiceInMemoryRepository(BaseHospitalityRepository[HospitalityInvoiceEntity]):
    """In-memory repository for hospitality invoice"""

    def __init__(self):
        self._storage: Dict[str, HospitalityInvoiceEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: HospitalityInvoiceEntity) -> HospitalityInvoiceEntity:
    ...
    async def read(self, id: str) -> Optional[HospitalityInvoiceEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: HospitalityInvoiceEntity) -> HospitalityInvoiceEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[HospitalityInvoiceEntity]:
    ...
    async def find_by(self, **kwargs) -> List[HospitalityInvoiceEntity]:
...
class HospitalityInvoiceCachedRepository(BaseHospitalityRepository[HospitalityInvoiceEntity]):
    """Cached repository wrapper for hospitality invoice"""
...
class HospitalityInvoiceUnitOfWork:
    """Unit of Work pattern for hospitality invoice"""

    def __init__(self, repository: BaseHospitalityRepository):
        self._repository = repository
        self._new: List[HospitalityInvoiceEntity] = []
        self._dirty: List[HospitalityInvoiceEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: HospitalityInvoiceEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: HospitalityInvoiceEntity) -> None:
