"""
Hospitality Transaction Repository
...
@dataclass
class HospitalityTransactionEntity:
    """Entity for hospitality transaction"""
    id: str
    data: Dict[str, Any]
...
class HospitalityTransactionInMemoryRepository(BaseHospitalityRepository[HospitalityTransactionEntity]):
    """In-memory repository for hospitality transaction"""

    def __init__(self):
        self._storage: Dict[str, HospitalityTransactionEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: HospitalityTransactionEntity) -> HospitalityTransactionEntity:
    ...
    async def read(self, id: str) -> Optional[HospitalityTransactionEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: HospitalityTransactionEntity) -> HospitalityTransactionEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[HospitalityTransactionEntity]:
    ...
    async def find_by(self, **kwargs) -> List[HospitalityTransactionEntity]:
...
class HospitalityTransactionCachedRepository(BaseHospitalityRepository[HospitalityTransactionEntity]):
    """Cached repository wrapper for hospitality transaction"""
...
class HospitalityTransactionUnitOfWork:
    """Unit of Work pattern for hospitality transaction"""

    def __init__(self, repository: BaseHospitalityRepository):
        self._repository = repository
        self._new: List[HospitalityTransactionEntity] = []
        self._dirty: List[HospitalityTransactionEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: HospitalityTransactionEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: HospitalityTransactionEntity) -> None:
