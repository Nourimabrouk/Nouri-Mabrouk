"""
Hospitality Payment Repository
...
@dataclass
class HospitalityPaymentEntity:
    """Entity for hospitality payment"""
    id: str
    data: Dict[str, Any]
...
class HospitalityPaymentInMemoryRepository(BaseHospitalityRepository[HospitalityPaymentEntity]):
    """In-memory repository for hospitality payment"""

    def __init__(self):
        self._storage: Dict[str, HospitalityPaymentEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: HospitalityPaymentEntity) -> HospitalityPaymentEntity:
    ...
    async def read(self, id: str) -> Optional[HospitalityPaymentEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: HospitalityPaymentEntity) -> HospitalityPaymentEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[HospitalityPaymentEntity]:
    ...
    async def find_by(self, **kwargs) -> List[HospitalityPaymentEntity]:
...
class HospitalityPaymentCachedRepository(BaseHospitalityRepository[HospitalityPaymentEntity]):
    """Cached repository wrapper for hospitality payment"""
...
class HospitalityPaymentUnitOfWork:
    """Unit of Work pattern for hospitality payment"""

    def __init__(self, repository: BaseHospitalityRepository):
        self._repository = repository
        self._new: List[HospitalityPaymentEntity] = []
        self._dirty: List[HospitalityPaymentEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: HospitalityPaymentEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: HospitalityPaymentEntity) -> None:
