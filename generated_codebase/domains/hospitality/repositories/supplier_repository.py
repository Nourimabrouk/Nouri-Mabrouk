"""
Hospitality Supplier Repository
...
@dataclass
class HospitalitySupplierEntity:
    """Entity for hospitality supplier"""
    id: str
    data: Dict[str, Any]
...
class HospitalitySupplierInMemoryRepository(BaseHospitalityRepository[HospitalitySupplierEntity]):
    """In-memory repository for hospitality supplier"""

    def __init__(self):
        self._storage: Dict[str, HospitalitySupplierEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: HospitalitySupplierEntity) -> HospitalitySupplierEntity:
    ...
    async def read(self, id: str) -> Optional[HospitalitySupplierEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: HospitalitySupplierEntity) -> HospitalitySupplierEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[HospitalitySupplierEntity]:
    ...
    async def find_by(self, **kwargs) -> List[HospitalitySupplierEntity]:
...
class HospitalitySupplierCachedRepository(BaseHospitalityRepository[HospitalitySupplierEntity]):
    """Cached repository wrapper for hospitality supplier"""
...
class HospitalitySupplierUnitOfWork:
    """Unit of Work pattern for hospitality supplier"""

    def __init__(self, repository: BaseHospitalityRepository):
        self._repository = repository
        self._new: List[HospitalitySupplierEntity] = []
        self._dirty: List[HospitalitySupplierEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: HospitalitySupplierEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: HospitalitySupplierEntity) -> None:
