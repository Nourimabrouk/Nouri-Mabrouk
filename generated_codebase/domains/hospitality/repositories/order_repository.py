...
@dataclass
class HospitalityOrderEntity:
    """Entity for hospitality order"""
    id: str
    data: Dict[str, Any]
...
class HospitalityOrderInMemoryRepository(BaseHospitalityRepository[HospitalityOrderEntity]):
    """In-memory repository for hospitality order"""

    def __init__(self):
        self._storage: Dict[str, HospitalityOrderEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: HospitalityOrderEntity) -> HospitalityOrderEntity:
    ...
    async def read(self, id: str) -> Optional[HospitalityOrderEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: HospitalityOrderEntity) -> HospitalityOrderEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[HospitalityOrderEntity]:
    ...
    async def find_by(self, **kwargs) -> List[HospitalityOrderEntity]:
...
class HospitalityOrderCachedRepository(BaseHospitalityRepository[HospitalityOrderEntity]):
    """Cached repository wrapper for hospitality order"""
...
class HospitalityOrderUnitOfWork:
    """Unit of Work pattern for hospitality order"""

    def __init__(self, repository: BaseHospitalityRepository):
        self._repository = repository
        self._new: List[HospitalityOrderEntity] = []
        self._dirty: List[HospitalityOrderEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: HospitalityOrderEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: HospitalityOrderEntity) -> None:
