"""
Hospitality Product Repository
...
@dataclass
class HospitalityProductEntity:
    """Entity for hospitality product"""
    id: str
    data: Dict[str, Any]
...
class HospitalityProductInMemoryRepository(BaseHospitalityRepository[HospitalityProductEntity]):
    """In-memory repository for hospitality product"""

    def __init__(self):
        self._storage: Dict[str, HospitalityProductEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: HospitalityProductEntity) -> HospitalityProductEntity:
    ...
    async def read(self, id: str) -> Optional[HospitalityProductEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: HospitalityProductEntity) -> HospitalityProductEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[HospitalityProductEntity]:
    ...
    async def find_by(self, **kwargs) -> List[HospitalityProductEntity]:
...
class HospitalityProductCachedRepository(BaseHospitalityRepository[HospitalityProductEntity]):
    """Cached repository wrapper for hospitality product"""
...
class HospitalityProductUnitOfWork:
    """Unit of Work pattern for hospitality product"""

    def __init__(self, repository: BaseHospitalityRepository):
        self._repository = repository
        self._new: List[HospitalityProductEntity] = []
        self._dirty: List[HospitalityProductEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: HospitalityProductEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: HospitalityProductEntity) -> None:
