"""
Hospitality Shipment Repository
...
@dataclass
class HospitalityShipmentEntity:
    """Entity for hospitality shipment"""
    id: str
    data: Dict[str, Any]
...
class HospitalityShipmentInMemoryRepository(BaseHospitalityRepository[HospitalityShipmentEntity]):
    """In-memory repository for hospitality shipment"""

    def __init__(self):
        self._storage: Dict[str, HospitalityShipmentEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: HospitalityShipmentEntity) -> HospitalityShipmentEntity:
    ...
    async def read(self, id: str) -> Optional[HospitalityShipmentEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: HospitalityShipmentEntity) -> HospitalityShipmentEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[HospitalityShipmentEntity]:
    ...
    async def find_by(self, **kwargs) -> List[HospitalityShipmentEntity]:
...
class HospitalityShipmentCachedRepository(BaseHospitalityRepository[HospitalityShipmentEntity]):
    """Cached repository wrapper for hospitality shipment"""
...
class HospitalityShipmentUnitOfWork:
    """Unit of Work pattern for hospitality shipment"""

    def __init__(self, repository: BaseHospitalityRepository):
        self._repository = repository
        self._new: List[HospitalityShipmentEntity] = []
        self._dirty: List[HospitalityShipmentEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: HospitalityShipmentEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: HospitalityShipmentEntity) -> None:
