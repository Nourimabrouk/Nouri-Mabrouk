"""
Hospitality Inventory Repository
...
@dataclass
class HospitalityInventoryEntity:
    """Entity for hospitality inventory"""
    id: str
    data: Dict[str, Any]
...
class HospitalityInventoryInMemoryRepository(BaseHospitalityRepository[HospitalityInventoryEntity]):
    """In-memory repository for hospitality inventory"""

    def __init__(self):
        self._storage: Dict[str, HospitalityInventoryEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: HospitalityInventoryEntity) -> HospitalityInventoryEntity:
    ...
    async def read(self, id: str) -> Optional[HospitalityInventoryEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: HospitalityInventoryEntity) -> HospitalityInventoryEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[HospitalityInventoryEntity]:
    ...
    async def find_by(self, **kwargs) -> List[HospitalityInventoryEntity]:
...
class HospitalityInventoryCachedRepository(BaseHospitalityRepository[HospitalityInventoryEntity]):
    """Cached repository wrapper for hospitality inventory"""
...
class HospitalityInventoryUnitOfWork:
    """Unit of Work pattern for hospitality inventory"""

    def __init__(self, repository: BaseHospitalityRepository):
        self._repository = repository
        self._new: List[HospitalityInventoryEntity] = []
        self._dirty: List[HospitalityInventoryEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: HospitalityInventoryEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: HospitalityInventoryEntity) -> None:
