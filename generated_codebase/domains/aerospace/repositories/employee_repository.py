...
@dataclass
class AerospaceEmployeeEntity:
    """Entity for aerospace employee"""
    id: str
    data: Dict[str, Any]
...
class AerospaceEmployeeInMemoryRepository(BaseAerospaceRepository[AerospaceEmployeeEntity]):
    """In-memory repository for aerospace employee"""

    def __init__(self):
        self._storage: Dict[str, AerospaceEmployeeEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: AerospaceEmployeeEntity) -> AerospaceEmployeeEntity:
    ...
    async def read(self, id: str) -> Optional[AerospaceEmployeeEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: AerospaceEmployeeEntity) -> AerospaceEmployeeEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[AerospaceEmployeeEntity]:
    ...
    async def find_by(self, **kwargs) -> List[AerospaceEmployeeEntity]:
...
class AerospaceEmployeeCachedRepository(BaseAerospaceRepository[AerospaceEmployeeEntity]):
    """Cached repository wrapper for aerospace employee"""
...
class AerospaceEmployeeUnitOfWork:
    """Unit of Work pattern for aerospace employee"""

    def __init__(self, repository: BaseAerospaceRepository):
        self._repository = repository
        self._new: List[AerospaceEmployeeEntity] = []
        self._dirty: List[AerospaceEmployeeEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: AerospaceEmployeeEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: AerospaceEmployeeEntity) -> None:
