"""
Aerospace Transaction Repository
...
@dataclass
class AerospaceTransactionEntity:
    """Entity for aerospace transaction"""
    id: str
    data: Dict[str, Any]
...
class AerospaceTransactionInMemoryRepository(BaseAerospaceRepository[AerospaceTransactionEntity]):
    """In-memory repository for aerospace transaction"""

    def __init__(self):
        self._storage: Dict[str, AerospaceTransactionEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: AerospaceTransactionEntity) -> AerospaceTransactionEntity:
    ...
    async def read(self, id: str) -> Optional[AerospaceTransactionEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: AerospaceTransactionEntity) -> AerospaceTransactionEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[AerospaceTransactionEntity]:
    ...
    async def find_by(self, **kwargs) -> List[AerospaceTransactionEntity]:
...
class AerospaceTransactionCachedRepository(BaseAerospaceRepository[AerospaceTransactionEntity]):
    """Cached repository wrapper for aerospace transaction"""
...
class AerospaceTransactionUnitOfWork:
    """Unit of Work pattern for aerospace transaction"""

    def __init__(self, repository: BaseAerospaceRepository):
        self._repository = repository
        self._new: List[AerospaceTransactionEntity] = []
        self._dirty: List[AerospaceTransactionEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: AerospaceTransactionEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: AerospaceTransactionEntity) -> None:
