...
@dataclass
class AerospacePaymentEntity:
    """Entity for aerospace payment"""
    id: str
    data: Dict[str, Any]
...
class AerospacePaymentInMemoryRepository(BaseAerospaceRepository[AerospacePaymentEntity]):
    """In-memory repository for aerospace payment"""

    def __init__(self):
        self._storage: Dict[str, AerospacePaymentEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: AerospacePaymentEntity) -> AerospacePaymentEntity:
    ...
    async def read(self, id: str) -> Optional[AerospacePaymentEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: AerospacePaymentEntity) -> AerospacePaymentEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[AerospacePaymentEntity]:
    ...
    async def find_by(self, **kwargs) -> List[AerospacePaymentEntity]:
...
class AerospacePaymentCachedRepository(BaseAerospaceRepository[AerospacePaymentEntity]):
    """Cached repository wrapper for aerospace payment"""
...
class AerospacePaymentUnitOfWork:
    """Unit of Work pattern for aerospace payment"""

    def __init__(self, repository: BaseAerospaceRepository):
        self._repository = repository
        self._new: List[AerospacePaymentEntity] = []
        self._dirty: List[AerospacePaymentEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: AerospacePaymentEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: AerospacePaymentEntity) -> None:
