"""
Aerospace Inventory Repository
...
@dataclass
class AerospaceInventoryEntity:
    """Entity for aerospace inventory"""
    id: str
    data: Dict[str, Any]
...
class AerospaceInventoryInMemoryRepository(BaseAerospaceRepository[AerospaceInventoryEntity]):
    """In-memory repository for aerospace inventory"""

    def __init__(self):
        self._storage: Dict[str, AerospaceInventoryEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: AerospaceInventoryEntity) -> AerospaceInventoryEntity:
    ...
    async def read(self, id: str) -> Optional[AerospaceInventoryEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: AerospaceInventoryEntity) -> AerospaceInventoryEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[AerospaceInventoryEntity]:
    ...
    async def find_by(self, **kwargs) -> List[AerospaceInventoryEntity]:
...
class AerospaceInventoryCachedRepository(BaseAerospaceRepository[AerospaceInventoryEntity]):
    """Cached repository wrapper for aerospace inventory"""
...
class AerospaceInventoryUnitOfWork:
    """Unit of Work pattern for aerospace inventory"""

    def __init__(self, repository: BaseAerospaceRepository):
        self._repository = repository
        self._new: List[AerospaceInventoryEntity] = []
        self._dirty: List[AerospaceInventoryEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: AerospaceInventoryEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: AerospaceInventoryEntity) -> None:
