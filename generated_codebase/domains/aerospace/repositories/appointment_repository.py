"""
Aerospace Appointment Repository
...
@dataclass
class AerospaceAppointmentEntity:
    """Entity for aerospace appointment"""
    id: str
    data: Dict[str, Any]
...
class BaseAerospaceRepository(ABC, Generic[T]):
    """Base repository for aerospace"""
...
class AerospaceAppointmentInMemoryRepository(BaseAerospaceRepository[AerospaceAppointmentEntity]):
    """In-memory repository for aerospace appointment"""

    def __init__(self):
        self._storage: Dict[str, AerospaceAppointmentEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: AerospaceAppointmentEntity) -> AerospaceAppointmentEntity:
    ...
    async def read(self, id: str) -> Optional[AerospaceAppointmentEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: AerospaceAppointmentEntity) -> AerospaceAppointmentEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[AerospaceAppointmentEntity]:
    ...
    async def find_by(self, **kwargs) -> List[AerospaceAppointmentEntity]:
...
class AerospaceAppointmentCachedRepository(BaseAerospaceRepository[AerospaceAppointmentEntity]):
    """Cached repository wrapper for aerospace appointment"""

    def __init__(self, base_repo: BaseAerospaceRepository, cache_ttl: int = 300):
...
class AerospaceAppointmentUnitOfWork:
    """Unit of Work pattern for aerospace appointment"""

    def __init__(self, repository: BaseAerospaceRepository):
        self._repository = repository
        self._new: List[AerospaceAppointmentEntity] = []
        self._dirty: List[AerospaceAppointmentEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: AerospaceAppointmentEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: AerospaceAppointmentEntity) -> None:
