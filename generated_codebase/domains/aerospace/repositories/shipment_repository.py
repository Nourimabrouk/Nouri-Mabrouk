...
@dataclass
class AerospaceShipmentEntity:
    """Entity for aerospace shipment"""
    id: str
    data: Dict[str, Any]
...
class AerospaceShipmentInMemoryRepository(BaseAerospaceRepository[AerospaceShipmentEntity]):
    """In-memory repository for aerospace shipment"""

    def __init__(self):
        self._storage: Dict[str, AerospaceShipmentEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: AerospaceShipmentEntity) -> AerospaceShipmentEntity:
    ...
    async def read(self, id: str) -> Optional[AerospaceShipmentEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: AerospaceShipmentEntity) -> AerospaceShipmentEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[AerospaceShipmentEntity]:
    ...
    async def find_by(self, **kwargs) -> List[AerospaceShipmentEntity]:
...
class AerospaceShipmentCachedRepository(BaseAerospaceRepository[AerospaceShipmentEntity]):
    """Cached repository wrapper for aerospace shipment"""
...
class AerospaceShipmentUnitOfWork:
    """Unit of Work pattern for aerospace shipment"""

    def __init__(self, repository: BaseAerospaceRepository):
        self._repository = repository
        self._new: List[AerospaceShipmentEntity] = []
        self._dirty: List[AerospaceShipmentEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: AerospaceShipmentEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: AerospaceShipmentEntity) -> None:
