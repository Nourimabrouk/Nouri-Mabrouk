...
@dataclass
class AerospaceCustomerEntity:
    """Entity for aerospace customer"""
    id: str
    data: Dict[str, Any]
...
class AerospaceCustomerInMemoryRepository(BaseAerospaceRepository[AerospaceCustomerEntity]):
    """In-memory repository for aerospace customer"""

    def __init__(self):
        self._storage: Dict[str, AerospaceCustomerEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: AerospaceCustomerEntity) -> AerospaceCustomerEntity:
    ...
    async def read(self, id: str) -> Optional[AerospaceCustomerEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: AerospaceCustomerEntity) -> AerospaceCustomerEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[AerospaceCustomerEntity]:
    ...
    async def find_by(self, **kwargs) -> List[AerospaceCustomerEntity]:
...
class AerospaceCustomerCachedRepository(BaseAerospaceRepository[AerospaceCustomerEntity]):
    """Cached repository wrapper for aerospace customer"""
...
class AerospaceCustomerUnitOfWork:
    """Unit of Work pattern for aerospace customer"""

    def __init__(self, repository: BaseAerospaceRepository):
        self._repository = repository
        self._new: List[AerospaceCustomerEntity] = []
        self._dirty: List[AerospaceCustomerEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: AerospaceCustomerEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: AerospaceCustomerEntity) -> None:
