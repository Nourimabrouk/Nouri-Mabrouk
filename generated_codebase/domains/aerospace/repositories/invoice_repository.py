...
@dataclass
class AerospaceInvoiceEntity:
    """Entity for aerospace invoice"""
    id: str
    data: Dict[str, Any]
...
class AerospaceInvoiceInMemoryRepository(BaseAerospaceRepository[AerospaceInvoiceEntity]):
    """In-memory repository for aerospace invoice"""

    def __init__(self):
        self._storage: Dict[str, AerospaceInvoiceEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: AerospaceInvoiceEntity) -> AerospaceInvoiceEntity:
    ...
    async def read(self, id: str) -> Optional[AerospaceInvoiceEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: AerospaceInvoiceEntity) -> AerospaceInvoiceEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[AerospaceInvoiceEntity]:
    ...
    async def find_by(self, **kwargs) -> List[AerospaceInvoiceEntity]:
...
class AerospaceInvoiceCachedRepository(BaseAerospaceRepository[AerospaceInvoiceEntity]):
    """Cached repository wrapper for aerospace invoice"""
...
class AerospaceInvoiceUnitOfWork:
    """Unit of Work pattern for aerospace invoice"""

    def __init__(self, repository: BaseAerospaceRepository):
        self._repository = repository
        self._new: List[AerospaceInvoiceEntity] = []
        self._dirty: List[AerospaceInvoiceEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: AerospaceInvoiceEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: AerospaceInvoiceEntity) -> None:
