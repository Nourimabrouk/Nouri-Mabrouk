...
@dataclass
class AerospaceProductEntity:
    """Entity for aerospace product"""
    id: str
    data: Dict[str, Any]
...
class AerospaceProductInMemoryRepository(BaseAerospaceRepository[AerospaceProductEntity]):
    """In-memory repository for aerospace product"""

    def __init__(self):
        self._storage: Dict[str, AerospaceProductEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: AerospaceProductEntity) -> AerospaceProductEntity:
    ...
    async def read(self, id: str) -> Optional[AerospaceProductEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: AerospaceProductEntity) -> AerospaceProductEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[AerospaceProductEntity]:
    ...
    async def find_by(self, **kwargs) -> List[AerospaceProductEntity]:
...
class AerospaceProductCachedRepository(BaseAerospaceRepository[AerospaceProductEntity]):
    """Cached repository wrapper for aerospace product"""
...
class AerospaceProductUnitOfWork:
    """Unit of Work pattern for aerospace product"""

    def __init__(self, repository: BaseAerospaceRepository):
        self._repository = repository
        self._new: List[AerospaceProductEntity] = []
        self._dirty: List[AerospaceProductEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: AerospaceProductEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: AerospaceProductEntity) -> None:
