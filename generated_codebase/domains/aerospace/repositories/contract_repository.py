...
@dataclass
class AerospaceContractEntity:
    """Entity for aerospace contract"""
    id: str
    data: Dict[str, Any]
...
class AerospaceContractInMemoryRepository(BaseAerospaceRepository[AerospaceContractEntity]):
    """In-memory repository for aerospace contract"""

    def __init__(self):
        self._storage: Dict[str, AerospaceContractEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: AerospaceContractEntity) -> AerospaceContractEntity:
    ...
    async def read(self, id: str) -> Optional[AerospaceContractEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: AerospaceContractEntity) -> AerospaceContractEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[AerospaceContractEntity]:
    ...
    async def find_by(self, **kwargs) -> List[AerospaceContractEntity]:
...
class AerospaceContractCachedRepository(BaseAerospaceRepository[AerospaceContractEntity]):
    """Cached repository wrapper for aerospace contract"""
...
class AerospaceContractUnitOfWork:
    """Unit of Work pattern for aerospace contract"""

    def __init__(self, repository: BaseAerospaceRepository):
        self._repository = repository
        self._new: List[AerospaceContractEntity] = []
        self._dirty: List[AerospaceContractEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: AerospaceContractEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: AerospaceContractEntity) -> None:
