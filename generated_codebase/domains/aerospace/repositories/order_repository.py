...
@dataclass
class AerospaceOrderEntity:
    """Entity for aerospace order"""
    id: str
    data: Dict[str, Any]
...
class AerospaceOrderInMemoryRepository(BaseAerospaceRepository[AerospaceOrderEntity]):
    """In-memory repository for aerospace order"""

    def __init__(self):
        self._storage: Dict[str, AerospaceOrderEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: AerospaceOrderEntity) -> AerospaceOrderEntity:
    ...
    async def read(self, id: str) -> Optional[AerospaceOrderEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: AerospaceOrderEntity) -> AerospaceOrderEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[AerospaceOrderEntity]:
    ...
    async def find_by(self, **kwargs) -> List[AerospaceOrderEntity]:
...
class AerospaceOrderCachedRepository(BaseAerospaceRepository[AerospaceOrderEntity]):
    """Cached repository wrapper for aerospace order"""
...
class AerospaceOrderUnitOfWork:
    """Unit of Work pattern for aerospace order"""

    def __init__(self, repository: BaseAerospaceRepository):
        self._repository = repository
        self._new: List[AerospaceOrderEntity] = []
        self._dirty: List[AerospaceOrderEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: AerospaceOrderEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: AerospaceOrderEntity) -> None:
