...
@dataclass
class LegalShipmentEntity:
    """Entity for legal shipment"""
    id: str
    data: Dict[str, Any]
...
class LegalShipmentInMemoryRepository(BaseLegalRepository[LegalShipmentEntity]):
    """In-memory repository for legal shipment"""

    def __init__(self):
        self._storage: Dict[str, LegalShipmentEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: LegalShipmentEntity) -> LegalShipmentEntity:
    ...
    async def read(self, id: str) -> Optional[LegalShipmentEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: LegalShipmentEntity) -> LegalShipmentEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[LegalShipmentEntity]:
    ...
    async def find_by(self, **kwargs) -> List[LegalShipmentEntity]:
...
class LegalShipmentCachedRepository(BaseLegalRepository[LegalShipmentEntity]):
    """Cached repository wrapper for legal shipment"""
...
class LegalShipmentUnitOfWork:
    """Unit of Work pattern for legal shipment"""

    def __init__(self, repository: BaseLegalRepository):
        self._repository = repository
        self._new: List[LegalShipmentEntity] = []
        self._dirty: List[LegalShipmentEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: LegalShipmentEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: LegalShipmentEntity) -> None:
