...
@dataclass
class LegalProductEntity:
    """Entity for legal product"""
    id: str
    data: Dict[str, Any]
...
class LegalProductInMemoryRepository(BaseLegalRepository[LegalProductEntity]):
    """In-memory repository for legal product"""

    def __init__(self):
        self._storage: Dict[str, LegalProductEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: LegalProductEntity) -> LegalProductEntity:
    ...
    async def read(self, id: str) -> Optional[LegalProductEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: LegalProductEntity) -> LegalProductEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[LegalProductEntity]:
    ...
    async def find_by(self, **kwargs) -> List[LegalProductEntity]:
...
class LegalProductCachedRepository(BaseLegalRepository[LegalProductEntity]):
    """Cached repository wrapper for legal product"""
...
class LegalProductUnitOfWork:
    """Unit of Work pattern for legal product"""

    def __init__(self, repository: BaseLegalRepository):
        self._repository = repository
        self._new: List[LegalProductEntity] = []
        self._dirty: List[LegalProductEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: LegalProductEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: LegalProductEntity) -> None:
