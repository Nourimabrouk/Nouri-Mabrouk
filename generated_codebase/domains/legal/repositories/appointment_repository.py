...
@dataclass
class LegalAppointmentEntity:
    """Entity for legal appointment"""
    id: str
    data: Dict[str, Any]
...
class LegalAppointmentInMemoryRepository(BaseLegalRepository[LegalAppointmentEntity]):
    """In-memory repository for legal appointment"""

    def __init__(self):
        self._storage: Dict[str, LegalAppointmentEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: LegalAppointmentEntity) -> LegalAppointmentEntity:
    ...
    async def read(self, id: str) -> Optional[LegalAppointmentEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: LegalAppointmentEntity) -> LegalAppointmentEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[LegalAppointmentEntity]:
    ...
    async def find_by(self, **kwargs) -> List[LegalAppointmentEntity]:
...
class LegalAppointmentCachedRepository(BaseLegalRepository[LegalAppointmentEntity]):
    """Cached repository wrapper for legal appointment"""
...
class LegalAppointmentUnitOfWork:
    """Unit of Work pattern for legal appointment"""

    def __init__(self, repository: BaseLegalRepository):
        self._repository = repository
        self._new: List[LegalAppointmentEntity] = []
        self._dirty: List[LegalAppointmentEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: LegalAppointmentEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: LegalAppointmentEntity) -> None:
