...
@dataclass
class LegalTransactionEntity:
    """Entity for legal transaction"""
    id: str
    data: Dict[str, Any]
...
class BaseLegalRepository(ABC, Generic[T]):
    """Base repository for legal"""
...
class LegalTransactionInMemoryRepository(BaseLegalRepository[LegalTransactionEntity]):
    """In-memory repository for legal transaction"""

    def __init__(self):
        self._storage: Dict[str, LegalTransactionEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: LegalTransactionEntity) -> LegalTransactionEntity:
    ...
    async def read(self, id: str) -> Optional[LegalTransactionEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: LegalTransactionEntity) -> LegalTransactionEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[LegalTransactionEntity]:
    ...
    async def find_by(self, **kwargs) -> List[LegalTransactionEntity]:
...
class LegalTransactionCachedRepository(BaseLegalRepository[LegalTransactionEntity]):
    """Cached repository wrapper for legal transaction"""

    def __init__(self, base_repo: BaseLegalRepository, cache_ttl: int = 300):
...
class LegalTransactionUnitOfWork:
    """Unit of Work pattern for legal transaction"""

    def __init__(self, repository: BaseLegalRepository):
        self._repository = repository
        self._new: List[LegalTransactionEntity] = []
        self._dirty: List[LegalTransactionEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: LegalTransactionEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: LegalTransactionEntity) -> None:
