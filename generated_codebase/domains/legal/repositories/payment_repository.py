...
@dataclass
class LegalPaymentEntity:
    """Entity for legal payment"""
    id: str
    data: Dict[str, Any]
...
class LegalPaymentInMemoryRepository(BaseLegalRepository[LegalPaymentEntity]):
    """In-memory repository for legal payment"""

    def __init__(self):
        self._storage: Dict[str, LegalPaymentEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: LegalPaymentEntity) -> LegalPaymentEntity:
    ...
    async def read(self, id: str) -> Optional[LegalPaymentEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: LegalPaymentEntity) -> LegalPaymentEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[LegalPaymentEntity]:
    ...
    async def find_by(self, **kwargs) -> List[LegalPaymentEntity]:
...
class LegalPaymentCachedRepository(BaseLegalRepository[LegalPaymentEntity]):
    """Cached repository wrapper for legal payment"""
...
class LegalPaymentUnitOfWork:
    """Unit of Work pattern for legal payment"""

    def __init__(self, repository: BaseLegalRepository):
        self._repository = repository
        self._new: List[LegalPaymentEntity] = []
        self._dirty: List[LegalPaymentEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: LegalPaymentEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: LegalPaymentEntity) -> None:
