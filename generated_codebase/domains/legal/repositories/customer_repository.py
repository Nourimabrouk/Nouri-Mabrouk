...
@dataclass
class LegalCustomerEntity:
    """Entity for legal customer"""
    id: str
    data: Dict[str, Any]
...
class LegalCustomerInMemoryRepository(BaseLegalRepository[LegalCustomerEntity]):
    """In-memory repository for legal customer"""

    def __init__(self):
        self._storage: Dict[str, LegalCustomerEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: LegalCustomerEntity) -> LegalCustomerEntity:
    ...
    async def read(self, id: str) -> Optional[LegalCustomerEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: LegalCustomerEntity) -> LegalCustomerEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[LegalCustomerEntity]:
    ...
    async def find_by(self, **kwargs) -> List[LegalCustomerEntity]:
...
class LegalCustomerCachedRepository(BaseLegalRepository[LegalCustomerEntity]):
    """Cached repository wrapper for legal customer"""
...
class LegalCustomerUnitOfWork:
    """Unit of Work pattern for legal customer"""

    def __init__(self, repository: BaseLegalRepository):
        self._repository = repository
        self._new: List[LegalCustomerEntity] = []
        self._dirty: List[LegalCustomerEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: LegalCustomerEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: LegalCustomerEntity) -> None:
