...
@dataclass
class LegalTicketEntity:
    """Entity for legal ticket"""
    id: str
    data: Dict[str, Any]
...
class LegalTicketInMemoryRepository(BaseLegalRepository[LegalTicketEntity]):
    """In-memory repository for legal ticket"""

    def __init__(self):
        self._storage: Dict[str, LegalTicketEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: LegalTicketEntity) -> LegalTicketEntity:
    ...
    async def read(self, id: str) -> Optional[LegalTicketEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: LegalTicketEntity) -> LegalTicketEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[LegalTicketEntity]:
    ...
    async def find_by(self, **kwargs) -> List[LegalTicketEntity]:
...
class LegalTicketCachedRepository(BaseLegalRepository[LegalTicketEntity]):
    """Cached repository wrapper for legal ticket"""
...
class LegalTicketUnitOfWork:
    """Unit of Work pattern for legal ticket"""

    def __init__(self, repository: BaseLegalRepository):
        self._repository = repository
        self._new: List[LegalTicketEntity] = []
        self._dirty: List[LegalTicketEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: LegalTicketEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: LegalTicketEntity) -> None:
