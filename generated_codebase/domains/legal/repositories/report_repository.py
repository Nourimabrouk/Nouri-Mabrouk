...
@dataclass
class LegalReportEntity:
    """Entity for legal report"""
    id: str
    data: Dict[str, Any]
...
class LegalReportInMemoryRepository(BaseLegalRepository[LegalReportEntity]):
    """In-memory repository for legal report"""

    def __init__(self):
        self._storage: Dict[str, LegalReportEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: LegalReportEntity) -> LegalReportEntity:
    ...
    async def read(self, id: str) -> Optional[LegalReportEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: LegalReportEntity) -> LegalReportEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[LegalReportEntity]:
    ...
    async def find_by(self, **kwargs) -> List[LegalReportEntity]:
...
class LegalReportCachedRepository(BaseLegalRepository[LegalReportEntity]):
    """Cached repository wrapper for legal report"""
...
class LegalReportUnitOfWork:
    """Unit of Work pattern for legal report"""

    def __init__(self, repository: BaseLegalRepository):
        self._repository = repository
        self._new: List[LegalReportEntity] = []
        self._dirty: List[LegalReportEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: LegalReportEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: LegalReportEntity) -> None:
