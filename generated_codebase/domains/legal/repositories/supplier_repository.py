...
@dataclass
class LegalSupplierEntity:
    """Entity for legal supplier"""
    id: str
    data: Dict[str, Any]
...
class LegalSupplierInMemoryRepository(BaseLegalRepository[LegalSupplierEntity]):
    """In-memory repository for legal supplier"""

    def __init__(self):
        self._storage: Dict[str, LegalSupplierEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: LegalSupplierEntity) -> LegalSupplierEntity:
    ...
    async def read(self, id: str) -> Optional[LegalSupplierEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: LegalSupplierEntity) -> LegalSupplierEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[LegalSupplierEntity]:
    ...
    async def find_by(self, **kwargs) -> List[LegalSupplierEntity]:
...
class LegalSupplierCachedRepository(BaseLegalRepository[LegalSupplierEntity]):
    """Cached repository wrapper for legal supplier"""
...
class LegalSupplierUnitOfWork:
    """Unit of Work pattern for legal supplier"""

    def __init__(self, repository: BaseLegalRepository):
        self._repository = repository
        self._new: List[LegalSupplierEntity] = []
        self._dirty: List[LegalSupplierEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: LegalSupplierEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: LegalSupplierEntity) -> None:
