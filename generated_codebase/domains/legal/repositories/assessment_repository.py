...
@dataclass
class LegalAssessmentEntity:
    """Entity for legal assessment"""
    id: str
    data: Dict[str, Any]
...
class LegalAssessmentInMemoryRepository(BaseLegalRepository[LegalAssessmentEntity]):
    """In-memory repository for legal assessment"""

    def __init__(self):
        self._storage: Dict[str, LegalAssessmentEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: LegalAssessmentEntity) -> LegalAssessmentEntity:
    ...
    async def read(self, id: str) -> Optional[LegalAssessmentEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: LegalAssessmentEntity) -> LegalAssessmentEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[LegalAssessmentEntity]:
    ...
    async def find_by(self, **kwargs) -> List[LegalAssessmentEntity]:
...
class LegalAssessmentCachedRepository(BaseLegalRepository[LegalAssessmentEntity]):
    """Cached repository wrapper for legal assessment"""
...
class LegalAssessmentUnitOfWork:
    """Unit of Work pattern for legal assessment"""

    def __init__(self, repository: BaseLegalRepository):
        self._repository = repository
        self._new: List[LegalAssessmentEntity] = []
        self._dirty: List[LegalAssessmentEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: LegalAssessmentEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: LegalAssessmentEntity) -> None:
