...
class LegalContractInMemoryRepository(BaseLegalRepository[LegalContractEntity]):
    """In-memory repository for legal contract"""

    def __init__(self):
        self._storage: Dict[str, LegalContractEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: LegalContractEntity) -> LegalContractEntity:
    ...
    async def read(self, id: str) -> Optional[LegalContractEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: LegalContractEntity) -> LegalContractEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[LegalContractEntity]:
    ...
    async def find_by(self, **kwargs) -> List[LegalContractEntity]:
...
class LegalContractCachedRepository(BaseLegalRepository[LegalContractEntity]):
    """Cached repository wrapper for legal contract"""
...
class LegalContractUnitOfWork:
    """Unit of Work pattern for legal contract"""

    def __init__(self, repository: BaseLegalRepository):
        self._repository = repository
        self._new: List[LegalContractEntity] = []
        self._dirty: List[LegalContractEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: LegalContractEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: LegalContractEntity) -> None:
