...
@dataclass
class LegalOrderEntity:
    """Entity for legal order"""
    id: str
    data: Dict[str, Any]
...
class LegalOrderInMemoryRepository(BaseLegalRepository[LegalOrderEntity]):
    """In-memory repository for legal order"""

    def __init__(self):
        self._storage: Dict[str, LegalOrderEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: LegalOrderEntity) -> LegalOrderEntity:
    ...
    async def read(self, id: str) -> Optional[LegalOrderEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: LegalOrderEntity) -> LegalOrderEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[LegalOrderEntity]:
    ...
    async def find_by(self, **kwargs) -> List[LegalOrderEntity]:
...
class LegalOrderCachedRepository(BaseLegalRepository[LegalOrderEntity]):
    """Cached repository wrapper for legal order"""
...
class LegalOrderUnitOfWork:
    """Unit of Work pattern for legal order"""

    def __init__(self, repository: BaseLegalRepository):
        self._repository = repository
        self._new: List[LegalOrderEntity] = []
        self._dirty: List[LegalOrderEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: LegalOrderEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: LegalOrderEntity) -> None:
