...
@dataclass
class LegalInventoryEntity:
    """Entity for legal inventory"""
    id: str
    data: Dict[str, Any]
...
class LegalInventoryInMemoryRepository(BaseLegalRepository[LegalInventoryEntity]):
    """In-memory repository for legal inventory"""

    def __init__(self):
        self._storage: Dict[str, LegalInventoryEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: LegalInventoryEntity) -> LegalInventoryEntity:
    ...
    async def read(self, id: str) -> Optional[LegalInventoryEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: LegalInventoryEntity) -> LegalInventoryEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[LegalInventoryEntity]:
    ...
    async def find_by(self, **kwargs) -> List[LegalInventoryEntity]:
...
class LegalInventoryCachedRepository(BaseLegalRepository[LegalInventoryEntity]):
    """Cached repository wrapper for legal inventory"""
...
class LegalInventoryUnitOfWork:
    """Unit of Work pattern for legal inventory"""

    def __init__(self, repository: BaseLegalRepository):
        self._repository = repository
        self._new: List[LegalInventoryEntity] = []
        self._dirty: List[LegalInventoryEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: LegalInventoryEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: LegalInventoryEntity) -> None:
