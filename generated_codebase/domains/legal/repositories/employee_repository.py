...
@dataclass
class LegalEmployeeEntity:
    """Entity for legal employee"""
    id: str
    data: Dict[str, Any]
...
class LegalEmployeeInMemoryRepository(BaseLegalRepository[LegalEmployeeEntity]):
    """In-memory repository for legal employee"""

    def __init__(self):
        self._storage: Dict[str, LegalEmployeeEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: LegalEmployeeEntity) -> LegalEmployeeEntity:
    ...
    async def read(self, id: str) -> Optional[LegalEmployeeEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: LegalEmployeeEntity) -> LegalEmployeeEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[LegalEmployeeEntity]:
    ...
    async def find_by(self, **kwargs) -> List[LegalEmployeeEntity]:
...
class LegalEmployeeCachedRepository(BaseLegalRepository[LegalEmployeeEntity]):
    """Cached repository wrapper for legal employee"""
...
class LegalEmployeeUnitOfWork:
    """Unit of Work pattern for legal employee"""

    def __init__(self, repository: BaseLegalRepository):
        self._repository = repository
        self._new: List[LegalEmployeeEntity] = []
        self._dirty: List[LegalEmployeeEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: LegalEmployeeEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: LegalEmployeeEntity) -> None:
