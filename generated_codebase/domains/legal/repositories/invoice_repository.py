...
@dataclass
class LegalInvoiceEntity:
    """Entity for legal invoice"""
    id: str
    data: Dict[str, Any]
...
class LegalInvoiceInMemoryRepository(BaseLegalRepository[LegalInvoiceEntity]):
    """In-memory repository for legal invoice"""

    def __init__(self):
        self._storage: Dict[str, LegalInvoiceEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: LegalInvoiceEntity) -> LegalInvoiceEntity:
    ...
    async def read(self, id: str) -> Optional[LegalInvoiceEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: LegalInvoiceEntity) -> LegalInvoiceEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[LegalInvoiceEntity]:
    ...
    async def find_by(self, **kwargs) -> List[LegalInvoiceEntity]:
...
class LegalInvoiceCachedRepository(BaseLegalRepository[LegalInvoiceEntity]):
    """Cached repository wrapper for legal invoice"""
...
class LegalInvoiceUnitOfWork:
    """Unit of Work pattern for legal invoice"""

    def __init__(self, repository: BaseLegalRepository):
        self._repository = repository
        self._new: List[LegalInvoiceEntity] = []
        self._dirty: List[LegalInvoiceEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: LegalInvoiceEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: LegalInvoiceEntity) -> None:
