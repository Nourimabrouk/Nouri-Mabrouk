"""
Automotive Appointment Repository
...
@dataclass
class AutomotiveAppointmentEntity:
    """Entity for automotive appointment"""
    id: str
    data: Dict[str, Any]
...
class BaseAutomotiveRepository(ABC, Generic[T]):
    """Base repository for automotive"""
...
class AutomotiveAppointmentInMemoryRepository(BaseAutomotiveRepository[AutomotiveAppointmentEntity]):
    """In-memory repository for automotive appointment"""

    def __init__(self):
        self._storage: Dict[str, AutomotiveAppointmentEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: AutomotiveAppointmentEntity) -> AutomotiveAppointmentEntity:
    ...
    async def read(self, id: str) -> Optional[AutomotiveAppointmentEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: AutomotiveAppointmentEntity) -> AutomotiveAppointmentEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[AutomotiveAppointmentEntity]:
    ...
    async def find_by(self, **kwargs) -> List[AutomotiveAppointmentEntity]:
...
class AutomotiveAppointmentCachedRepository(BaseAutomotiveRepository[AutomotiveAppointmentEntity]):
    """Cached repository wrapper for automotive appointment"""

    def __init__(self, base_repo: BaseAutomotiveRepository, cache_ttl: int = 300):
...
class AutomotiveAppointmentUnitOfWork:
    """Unit of Work pattern for automotive appointment"""

    def __init__(self, repository: BaseAutomotiveRepository):
        self._repository = repository
        self._new: List[AutomotiveAppointmentEntity] = []
        self._dirty: List[AutomotiveAppointmentEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: AutomotiveAppointmentEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: AutomotiveAppointmentEntity) -> None:
