...
@dataclass
class AutomotiveOrderEntity:
    """Entity for automotive order"""
    id: str
    data: Dict[str, Any]
...
class AutomotiveOrderInMemoryRepository(BaseAutomotiveRepository[AutomotiveOrderEntity]):
    """In-memory repository for automotive order"""

    def __init__(self):
        self._storage: Dict[str, AutomotiveOrderEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: AutomotiveOrderEntity) -> AutomotiveOrderEntity:
    ...
    async def read(self, id: str) -> Optional[AutomotiveOrderEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: AutomotiveOrderEntity) -> AutomotiveOrderEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[AutomotiveOrderEntity]:
    ...
    async def find_by(self, **kwargs) -> List[AutomotiveOrderEntity]:
...
class AutomotiveOrderCachedRepository(BaseAutomotiveRepository[AutomotiveOrderEntity]):
    """Cached repository wrapper for automotive order"""
...
class AutomotiveOrderUnitOfWork:
    """Unit of Work pattern for automotive order"""

    def __init__(self, repository: BaseAutomotiveRepository):
        self._repository = repository
        self._new: List[AutomotiveOrderEntity] = []
        self._dirty: List[AutomotiveOrderEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: AutomotiveOrderEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: AutomotiveOrderEntity) -> None:
