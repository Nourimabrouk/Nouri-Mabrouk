...
@dataclass
class MediaShipmentEntity:
    """Entity for media shipment"""
    id: str
    data: Dict[str, Any]
...
class MediaShipmentInMemoryRepository(BaseMediaRepository[MediaShipmentEntity]):
    """In-memory repository for media shipment"""

    def __init__(self):
        self._storage: Dict[str, MediaShipmentEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: MediaShipmentEntity) -> MediaShipmentEntity:
    ...
    async def read(self, id: str) -> Optional[MediaShipmentEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: MediaShipmentEntity) -> MediaShipmentEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[MediaShipmentEntity]:
    ...
    async def find_by(self, **kwargs) -> List[MediaShipmentEntity]:
...
class MediaShipmentCachedRepository(BaseMediaRepository[MediaShipmentEntity]):
    """Cached repository wrapper for media shipment"""
...
class MediaShipmentUnitOfWork:
    """Unit of Work pattern for media shipment"""

    def __init__(self, repository: BaseMediaRepository):
        self._repository = repository
        self._new: List[MediaShipmentEntity] = []
        self._dirty: List[MediaShipmentEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: MediaShipmentEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: MediaShipmentEntity) -> None:
