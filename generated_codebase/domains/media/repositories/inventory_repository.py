...
@dataclass
class MediaInventoryEntity:
    """Entity for media inventory"""
    id: str
    data: Dict[str, Any]
...
class MediaInventoryInMemoryRepository(BaseMediaRepository[MediaInventoryEntity]):
    """In-memory repository for media inventory"""

    def __init__(self):
        self._storage: Dict[str, MediaInventoryEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: MediaInventoryEntity) -> MediaInventoryEntity:
    ...
    async def read(self, id: str) -> Optional[MediaInventoryEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: MediaInventoryEntity) -> MediaInventoryEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[MediaInventoryEntity]:
    ...
    async def find_by(self, **kwargs) -> List[MediaInventoryEntity]:
...
class MediaInventoryCachedRepository(BaseMediaRepository[MediaInventoryEntity]):
    """Cached repository wrapper for media inventory"""
...
class MediaInventoryUnitOfWork:
    """Unit of Work pattern for media inventory"""

    def __init__(self, repository: BaseMediaRepository):
        self._repository = repository
        self._new: List[MediaInventoryEntity] = []
        self._dirty: List[MediaInventoryEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: MediaInventoryEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: MediaInventoryEntity) -> None:
