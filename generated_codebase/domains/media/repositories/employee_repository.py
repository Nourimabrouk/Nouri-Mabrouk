...
@dataclass
class MediaEmployeeEntity:
    """Entity for media employee"""
    id: str
    data: Dict[str, Any]
...
class MediaEmployeeInMemoryRepository(BaseMediaRepository[MediaEmployeeEntity]):
    """In-memory repository for media employee"""

    def __init__(self):
        self._storage: Dict[str, MediaEmployeeEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: MediaEmployeeEntity) -> MediaEmployeeEntity:
    ...
    async def read(self, id: str) -> Optional[MediaEmployeeEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: MediaEmployeeEntity) -> MediaEmployeeEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[MediaEmployeeEntity]:
    ...
    async def find_by(self, **kwargs) -> List[MediaEmployeeEntity]:
...
class MediaEmployeeCachedRepository(BaseMediaRepository[MediaEmployeeEntity]):
    """Cached repository wrapper for media employee"""
...
class MediaEmployeeUnitOfWork:
    """Unit of Work pattern for media employee"""

    def __init__(self, repository: BaseMediaRepository):
        self._repository = repository
        self._new: List[MediaEmployeeEntity] = []
        self._dirty: List[MediaEmployeeEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: MediaEmployeeEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: MediaEmployeeEntity) -> None:
