...
@dataclass
class MediaTransactionEntity:
    """Entity for media transaction"""
    id: str
    data: Dict[str, Any]
...
class MediaTransactionInMemoryRepository(BaseMediaRepository[MediaTransactionEntity]):
    """In-memory repository for media transaction"""

    def __init__(self):
        self._storage: Dict[str, MediaTransactionEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: MediaTransactionEntity) -> MediaTransactionEntity:
    ...
    async def read(self, id: str) -> Optional[MediaTransactionEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: MediaTransactionEntity) -> MediaTransactionEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[MediaTransactionEntity]:
    ...
    async def find_by(self, **kwargs) -> List[MediaTransactionEntity]:
...
class MediaTransactionCachedRepository(BaseMediaRepository[MediaTransactionEntity]):
    """Cached repository wrapper for media transaction"""
...
class MediaTransactionUnitOfWork:
    """Unit of Work pattern for media transaction"""

    def __init__(self, repository: BaseMediaRepository):
        self._repository = repository
        self._new: List[MediaTransactionEntity] = []
        self._dirty: List[MediaTransactionEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: MediaTransactionEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: MediaTransactionEntity) -> None:
