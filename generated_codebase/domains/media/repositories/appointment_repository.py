...
@dataclass
class MediaAppointmentEntity:
    """Entity for media appointment"""
    id: str
    data: Dict[str, Any]
...
class MediaAppointmentInMemoryRepository(BaseMediaRepository[MediaAppointmentEntity]):
    """In-memory repository for media appointment"""

    def __init__(self):
        self._storage: Dict[str, MediaAppointmentEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: MediaAppointmentEntity) -> MediaAppointmentEntity:
    ...
    async def read(self, id: str) -> Optional[MediaAppointmentEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: MediaAppointmentEntity) -> MediaAppointmentEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[MediaAppointmentEntity]:
    ...
    async def find_by(self, **kwargs) -> List[MediaAppointmentEntity]:
...
class MediaAppointmentCachedRepository(BaseMediaRepository[MediaAppointmentEntity]):
    """Cached repository wrapper for media appointment"""

    def __init__(self, base_repo: BaseMediaRepository, cache_ttl: int = 300):
...
class MediaAppointmentUnitOfWork:
    """Unit of Work pattern for media appointment"""

    def __init__(self, repository: BaseMediaRepository):
        self._repository = repository
        self._new: List[MediaAppointmentEntity] = []
        self._dirty: List[MediaAppointmentEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: MediaAppointmentEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: MediaAppointmentEntity) -> None:
