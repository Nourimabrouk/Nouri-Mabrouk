...
@dataclass
class MediaTicketEntity:
    """Entity for media ticket"""
    id: str
    data: Dict[str, Any]
...
class MediaTicketInMemoryRepository(BaseMediaRepository[MediaTicketEntity]):
    """In-memory repository for media ticket"""

    def __init__(self):
        self._storage: Dict[str, MediaTicketEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: MediaTicketEntity) -> MediaTicketEntity:
    ...
    async def read(self, id: str) -> Optional[MediaTicketEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: MediaTicketEntity) -> MediaTicketEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[MediaTicketEntity]:
    ...
    async def find_by(self, **kwargs) -> List[MediaTicketEntity]:
...
class MediaTicketCachedRepository(BaseMediaRepository[MediaTicketEntity]):
    """Cached repository wrapper for media ticket"""
...
class MediaTicketUnitOfWork:
    """Unit of Work pattern for media ticket"""

    def __init__(self, repository: BaseMediaRepository):
        self._repository = repository
        self._new: List[MediaTicketEntity] = []
        self._dirty: List[MediaTicketEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: MediaTicketEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: MediaTicketEntity) -> None:
