...
@dataclass
class MediaAssessmentEntity:
    """Entity for media assessment"""
    id: str
    data: Dict[str, Any]
...
class MediaAssessmentInMemoryRepository(BaseMediaRepository[MediaAssessmentEntity]):
    """In-memory repository for media assessment"""

    def __init__(self):
        self._storage: Dict[str, MediaAssessmentEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: MediaAssessmentEntity) -> MediaAssessmentEntity:
    ...
    async def read(self, id: str) -> Optional[MediaAssessmentEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: MediaAssessmentEntity) -> MediaAssessmentEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[MediaAssessmentEntity]:
    ...
    async def find_by(self, **kwargs) -> List[MediaAssessmentEntity]:
...
class MediaAssessmentCachedRepository(BaseMediaRepository[MediaAssessmentEntity]):
    """Cached repository wrapper for media assessment"""
...
class MediaAssessmentUnitOfWork:
    """Unit of Work pattern for media assessment"""

    def __init__(self, repository: BaseMediaRepository):
        self._repository = repository
        self._new: List[MediaAssessmentEntity] = []
        self._dirty: List[MediaAssessmentEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: MediaAssessmentEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: MediaAssessmentEntity) -> None:
