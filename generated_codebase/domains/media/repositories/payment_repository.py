...
@dataclass
class MediaPaymentEntity:
    """Entity for media payment"""
    id: str
    data: Dict[str, Any]
...
class MediaPaymentInMemoryRepository(BaseMediaRepository[MediaPaymentEntity]):
    """In-memory repository for media payment"""

    def __init__(self):
        self._storage: Dict[str, MediaPaymentEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: MediaPaymentEntity) -> MediaPaymentEntity:
    ...
    async def read(self, id: str) -> Optional[MediaPaymentEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: MediaPaymentEntity) -> MediaPaymentEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[MediaPaymentEntity]:
    ...
    async def find_by(self, **kwargs) -> List[MediaPaymentEntity]:
...
class MediaPaymentCachedRepository(BaseMediaRepository[MediaPaymentEntity]):
    """Cached repository wrapper for media payment"""
...
class MediaPaymentUnitOfWork:
    """Unit of Work pattern for media payment"""

    def __init__(self, repository: BaseMediaRepository):
        self._repository = repository
        self._new: List[MediaPaymentEntity] = []
        self._dirty: List[MediaPaymentEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: MediaPaymentEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: MediaPaymentEntity) -> None:
