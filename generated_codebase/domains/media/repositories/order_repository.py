...
class MediaOrderInMemoryRepository(BaseMediaRepository[MediaOrderEntity]):
    """In-memory repository for media order"""

    def __init__(self):
        self._storage: Dict[str, MediaOrderEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: MediaOrderEntity) -> MediaOrderEntity:
    ...
    async def read(self, id: str) -> Optional[MediaOrderEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: MediaOrderEntity) -> MediaOrderEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[MediaOrderEntity]:
    ...
    async def find_by(self, **kwargs) -> List[MediaOrderEntity]:
...
class MediaOrderCachedRepository(BaseMediaRepository[MediaOrderEntity]):
    """Cached repository wrapper for media order"""
...
class MediaOrderUnitOfWork:
    """Unit of Work pattern for media order"""

    def __init__(self, repository: BaseMediaRepository):
        self._repository = repository
        self._new: List[MediaOrderEntity] = []
        self._dirty: List[MediaOrderEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: MediaOrderEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: MediaOrderEntity) -> None:
