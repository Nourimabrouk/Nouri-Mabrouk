...
@dataclass
class MediaReportEntity:
    """Entity for media report"""
    id: str
    data: Dict[str, Any]
...
class MediaReportInMemoryRepository(BaseMediaRepository[MediaReportEntity]):
    """In-memory repository for media report"""

    def __init__(self):
        self._storage: Dict[str, MediaReportEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: MediaReportEntity) -> MediaReportEntity:
    ...
    async def read(self, id: str) -> Optional[MediaReportEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: MediaReportEntity) -> MediaReportEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[MediaReportEntity]:
    ...
    async def find_by(self, **kwargs) -> List[MediaReportEntity]:
...
class MediaReportCachedRepository(BaseMediaRepository[MediaReportEntity]):
    """Cached repository wrapper for media report"""
...
class MediaReportUnitOfWork:
    """Unit of Work pattern for media report"""

    def __init__(self, repository: BaseMediaRepository):
        self._repository = repository
        self._new: List[MediaReportEntity] = []
        self._dirty: List[MediaReportEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: MediaReportEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: MediaReportEntity) -> None:
