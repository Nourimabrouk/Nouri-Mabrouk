...
@dataclass
class MediaCustomerEntity:
    """Entity for media customer"""
    id: str
    data: Dict[str, Any]
...
class MediaCustomerInMemoryRepository(BaseMediaRepository[MediaCustomerEntity]):
    """In-memory repository for media customer"""

    def __init__(self):
        self._storage: Dict[str, MediaCustomerEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: MediaCustomerEntity) -> MediaCustomerEntity:
    ...
    async def read(self, id: str) -> Optional[MediaCustomerEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: MediaCustomerEntity) -> MediaCustomerEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[MediaCustomerEntity]:
    ...
    async def find_by(self, **kwargs) -> List[MediaCustomerEntity]:
...
class MediaCustomerCachedRepository(BaseMediaRepository[MediaCustomerEntity]):
    """Cached repository wrapper for media customer"""
...
class MediaCustomerUnitOfWork:
    """Unit of Work pattern for media customer"""

    def __init__(self, repository: BaseMediaRepository):
        self._repository = repository
        self._new: List[MediaCustomerEntity] = []
        self._dirty: List[MediaCustomerEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: MediaCustomerEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: MediaCustomerEntity) -> None:
