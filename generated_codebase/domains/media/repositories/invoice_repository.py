...
@dataclass
class MediaInvoiceEntity:
    """Entity for media invoice"""
    id: str
    data: Dict[str, Any]
...
class MediaInvoiceInMemoryRepository(BaseMediaRepository[MediaInvoiceEntity]):
    """In-memory repository for media invoice"""

    def __init__(self):
        self._storage: Dict[str, MediaInvoiceEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: MediaInvoiceEntity) -> MediaInvoiceEntity:
    ...
    async def read(self, id: str) -> Optional[MediaInvoiceEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: MediaInvoiceEntity) -> MediaInvoiceEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[MediaInvoiceEntity]:
    ...
    async def find_by(self, **kwargs) -> List[MediaInvoiceEntity]:
...
class MediaInvoiceCachedRepository(BaseMediaRepository[MediaInvoiceEntity]):
    """Cached repository wrapper for media invoice"""
...
class MediaInvoiceUnitOfWork:
    """Unit of Work pattern for media invoice"""

    def __init__(self, repository: BaseMediaRepository):
        self._repository = repository
        self._new: List[MediaInvoiceEntity] = []
        self._dirty: List[MediaInvoiceEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: MediaInvoiceEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: MediaInvoiceEntity) -> None:
