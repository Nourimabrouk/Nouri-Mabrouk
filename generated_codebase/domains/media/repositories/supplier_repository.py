...
@dataclass
class MediaSupplierEntity:
    """Entity for media supplier"""
    id: str
    data: Dict[str, Any]
...
class MediaSupplierInMemoryRepository(BaseMediaRepository[MediaSupplierEntity]):
    """In-memory repository for media supplier"""

    def __init__(self):
        self._storage: Dict[str, MediaSupplierEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: MediaSupplierEntity) -> MediaSupplierEntity:
    ...
    async def read(self, id: str) -> Optional[MediaSupplierEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: MediaSupplierEntity) -> MediaSupplierEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[MediaSupplierEntity]:
    ...
    async def find_by(self, **kwargs) -> List[MediaSupplierEntity]:
...
class MediaSupplierCachedRepository(BaseMediaRepository[MediaSupplierEntity]):
    """Cached repository wrapper for media supplier"""
...
class MediaSupplierUnitOfWork:
    """Unit of Work pattern for media supplier"""

    def __init__(self, repository: BaseMediaRepository):
        self._repository = repository
        self._new: List[MediaSupplierEntity] = []
        self._dirty: List[MediaSupplierEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: MediaSupplierEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: MediaSupplierEntity) -> None:
