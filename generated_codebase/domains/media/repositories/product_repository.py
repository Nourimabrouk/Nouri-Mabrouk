...
@dataclass
class MediaProductEntity:
    """Entity for media product"""
    id: str
    data: Dict[str, Any]
...
class MediaProductInMemoryRepository(BaseMediaRepository[MediaProductEntity]):
    """In-memory repository for media product"""

    def __init__(self):
        self._storage: Dict[str, MediaProductEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: MediaProductEntity) -> MediaProductEntity:
    ...
    async def read(self, id: str) -> Optional[MediaProductEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: MediaProductEntity) -> MediaProductEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[MediaProductEntity]:
    ...
    async def find_by(self, **kwargs) -> List[MediaProductEntity]:
...
class MediaProductCachedRepository(BaseMediaRepository[MediaProductEntity]):
    """Cached repository wrapper for media product"""
...
class MediaProductUnitOfWork:
    """Unit of Work pattern for media product"""

    def __init__(self, repository: BaseMediaRepository):
        self._repository = repository
        self._new: List[MediaProductEntity] = []
        self._dirty: List[MediaProductEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: MediaProductEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: MediaProductEntity) -> None:
