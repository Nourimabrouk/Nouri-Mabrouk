...
@dataclass
class MediaContractEntity:
    """Entity for media contract"""
    id: str
    data: Dict[str, Any]
...
class MediaContractInMemoryRepository(BaseMediaRepository[MediaContractEntity]):
    """In-memory repository for media contract"""

    def __init__(self):
        self._storage: Dict[str, MediaContractEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: MediaContractEntity) -> MediaContractEntity:
    ...
    async def read(self, id: str) -> Optional[MediaContractEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: MediaContractEntity) -> MediaContractEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[MediaContractEntity]:
    ...
    async def find_by(self, **kwargs) -> List[MediaContractEntity]:
...
class MediaContractCachedRepository(BaseMediaRepository[MediaContractEntity]):
    """Cached repository wrapper for media contract"""
...
class MediaContractUnitOfWork:
    """Unit of Work pattern for media contract"""

    def __init__(self, repository: BaseMediaRepository):
        self._repository = repository
        self._new: List[MediaContractEntity] = []
        self._dirty: List[MediaContractEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: MediaContractEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: MediaContractEntity) -> None:
