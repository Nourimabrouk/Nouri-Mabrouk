"""
Cybersecurity Invoice Repository
...
@dataclass
class CybersecurityInvoiceEntity:
    """Entity for cybersecurity invoice"""
    id: str
    data: Dict[str, Any]
...
class CybersecurityInvoiceInMemoryRepository(BaseCybersecurityRepository[CybersecurityInvoiceEntity]):
    """In-memory repository for cybersecurity invoice"""

    def __init__(self):
        self._storage: Dict[str, CybersecurityInvoiceEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: CybersecurityInvoiceEntity) -> CybersecurityInvoiceEntity:
    ...
    async def read(self, id: str) -> Optional[CybersecurityInvoiceEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: CybersecurityInvoiceEntity) -> CybersecurityInvoiceEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[CybersecurityInvoiceEntity]:
    ...
    async def find_by(self, **kwargs) -> List[CybersecurityInvoiceEntity]:
...
class CybersecurityInvoiceCachedRepository(BaseCybersecurityRepository[CybersecurityInvoiceEntity]):
    """Cached repository wrapper for cybersecurity invoice"""
...
class CybersecurityInvoiceUnitOfWork:
    """Unit of Work pattern for cybersecurity invoice"""

    def __init__(self, repository: BaseCybersecurityRepository):
        self._repository = repository
        self._new: List[CybersecurityInvoiceEntity] = []
        self._dirty: List[CybersecurityInvoiceEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: CybersecurityInvoiceEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: CybersecurityInvoiceEntity) -> None:
