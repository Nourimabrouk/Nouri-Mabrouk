"""
Cybersecurity Order Repository
...
@dataclass
class CybersecurityOrderEntity:
    """Entity for cybersecurity order"""
    id: str
    data: Dict[str, Any]
...
class CybersecurityOrderInMemoryRepository(BaseCybersecurityRepository[CybersecurityOrderEntity]):
    """In-memory repository for cybersecurity order"""

    def __init__(self):
        self._storage: Dict[str, CybersecurityOrderEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: CybersecurityOrderEntity) -> CybersecurityOrderEntity:
    ...
    async def read(self, id: str) -> Optional[CybersecurityOrderEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: CybersecurityOrderEntity) -> CybersecurityOrderEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[CybersecurityOrderEntity]:
    ...
    async def find_by(self, **kwargs) -> List[CybersecurityOrderEntity]:
...
class CybersecurityOrderCachedRepository(BaseCybersecurityRepository[CybersecurityOrderEntity]):
    """Cached repository wrapper for cybersecurity order"""
...
class CybersecurityOrderUnitOfWork:
    """Unit of Work pattern for cybersecurity order"""

    def __init__(self, repository: BaseCybersecurityRepository):
        self._repository = repository
        self._new: List[CybersecurityOrderEntity] = []
        self._dirty: List[CybersecurityOrderEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: CybersecurityOrderEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: CybersecurityOrderEntity) -> None:
