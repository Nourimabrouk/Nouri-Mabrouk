"""
Agriculture Employee Repository
...
@dataclass
class AgricultureEmployeeEntity:
    """Entity for agriculture employee"""
    id: str
    data: Dict[str, Any]
...
class AgricultureEmployeeInMemoryRepository(BaseAgricultureRepository[AgricultureEmployeeEntity]):
    """In-memory repository for agriculture employee"""

    def __init__(self):
        self._storage: Dict[str, AgricultureEmployeeEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: AgricultureEmployeeEntity) -> AgricultureEmployeeEntity:
    ...
    async def read(self, id: str) -> Optional[AgricultureEmployeeEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: AgricultureEmployeeEntity) -> AgricultureEmployeeEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[AgricultureEmployeeEntity]:
    ...
    async def find_by(self, **kwargs) -> List[AgricultureEmployeeEntity]:
...
class AgricultureEmployeeCachedRepository(BaseAgricultureRepository[AgricultureEmployeeEntity]):
    """Cached repository wrapper for agriculture employee"""
...
class AgricultureEmployeeUnitOfWork:
    """Unit of Work pattern for agriculture employee"""

    def __init__(self, repository: BaseAgricultureRepository):
        self._repository = repository
        self._new: List[AgricultureEmployeeEntity] = []
        self._dirty: List[AgricultureEmployeeEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: AgricultureEmployeeEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: AgricultureEmployeeEntity) -> None:
