...
@dataclass
class AgricultureReportEntity:
    """Entity for agriculture report"""
    id: str
    data: Dict[str, Any]
...
class AgricultureReportInMemoryRepository(BaseAgricultureRepository[AgricultureReportEntity]):
    """In-memory repository for agriculture report"""

    def __init__(self):
        self._storage: Dict[str, AgricultureReportEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: AgricultureReportEntity) -> AgricultureReportEntity:
    ...
    async def read(self, id: str) -> Optional[AgricultureReportEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: AgricultureReportEntity) -> AgricultureReportEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[AgricultureReportEntity]:
    ...
    async def find_by(self, **kwargs) -> List[AgricultureReportEntity]:
...
class AgricultureReportCachedRepository(BaseAgricultureRepository[AgricultureReportEntity]):
    """Cached repository wrapper for agriculture report"""
...
class AgricultureReportUnitOfWork:
    """Unit of Work pattern for agriculture report"""

    def __init__(self, repository: BaseAgricultureRepository):
        self._repository = repository
        self._new: List[AgricultureReportEntity] = []
        self._dirty: List[AgricultureReportEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: AgricultureReportEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: AgricultureReportEntity) -> None:
