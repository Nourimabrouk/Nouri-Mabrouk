...
@dataclass
class AgricultureTicketEntity:
    """Entity for agriculture ticket"""
    id: str
    data: Dict[str, Any]
...
class AgricultureTicketInMemoryRepository(BaseAgricultureRepository[AgricultureTicketEntity]):
    """In-memory repository for agriculture ticket"""

    def __init__(self):
        self._storage: Dict[str, AgricultureTicketEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: AgricultureTicketEntity) -> AgricultureTicketEntity:
    ...
    async def read(self, id: str) -> Optional[AgricultureTicketEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: AgricultureTicketEntity) -> AgricultureTicketEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[AgricultureTicketEntity]:
    ...
    async def find_by(self, **kwargs) -> List[AgricultureTicketEntity]:
...
class AgricultureTicketCachedRepository(BaseAgricultureRepository[AgricultureTicketEntity]):
    """Cached repository wrapper for agriculture ticket"""
...
class AgricultureTicketUnitOfWork:
    """Unit of Work pattern for agriculture ticket"""

    def __init__(self, repository: BaseAgricultureRepository):
        self._repository = repository
        self._new: List[AgricultureTicketEntity] = []
        self._dirty: List[AgricultureTicketEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: AgricultureTicketEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: AgricultureTicketEntity) -> None:
