"""
Agriculture Supplier Repository
...
@dataclass
class AgricultureSupplierEntity:
    """Entity for agriculture supplier"""
    id: str
    data: Dict[str, Any]
...
class AgricultureSupplierInMemoryRepository(BaseAgricultureRepository[AgricultureSupplierEntity]):
    """In-memory repository for agriculture supplier"""

    def __init__(self):
        self._storage: Dict[str, AgricultureSupplierEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: AgricultureSupplierEntity) -> AgricultureSupplierEntity:
    ...
    async def read(self, id: str) -> Optional[AgricultureSupplierEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: AgricultureSupplierEntity) -> AgricultureSupplierEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[AgricultureSupplierEntity]:
    ...
    async def find_by(self, **kwargs) -> List[AgricultureSupplierEntity]:
...
class AgricultureSupplierCachedRepository(BaseAgricultureRepository[AgricultureSupplierEntity]):
    """Cached repository wrapper for agriculture supplier"""
...
class AgricultureSupplierUnitOfWork:
    """Unit of Work pattern for agriculture supplier"""

    def __init__(self, repository: BaseAgricultureRepository):
        self._repository = repository
        self._new: List[AgricultureSupplierEntity] = []
        self._dirty: List[AgricultureSupplierEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: AgricultureSupplierEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: AgricultureSupplierEntity) -> None:
