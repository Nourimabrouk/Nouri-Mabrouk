...
@dataclass
class AgricultureOrderEntity:
    """Entity for agriculture order"""
    id: str
    data: Dict[str, Any]
...
class AgricultureOrderInMemoryRepository(BaseAgricultureRepository[AgricultureOrderEntity]):
    """In-memory repository for agriculture order"""

    def __init__(self):
        self._storage: Dict[str, AgricultureOrderEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: AgricultureOrderEntity) -> AgricultureOrderEntity:
    ...
    async def read(self, id: str) -> Optional[AgricultureOrderEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: AgricultureOrderEntity) -> AgricultureOrderEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[AgricultureOrderEntity]:
    ...
    async def find_by(self, **kwargs) -> List[AgricultureOrderEntity]:
...
class AgricultureOrderCachedRepository(BaseAgricultureRepository[AgricultureOrderEntity]):
    """Cached repository wrapper for agriculture order"""
...
class AgricultureOrderUnitOfWork:
    """Unit of Work pattern for agriculture order"""

    def __init__(self, repository: BaseAgricultureRepository):
        self._repository = repository
        self._new: List[AgricultureOrderEntity] = []
        self._dirty: List[AgricultureOrderEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: AgricultureOrderEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: AgricultureOrderEntity) -> None:
