"""
Agriculture Payment Repository
...
@dataclass
class AgriculturePaymentEntity:
    """Entity for agriculture payment"""
    id: str
    data: Dict[str, Any]
...
class AgriculturePaymentInMemoryRepository(BaseAgricultureRepository[AgriculturePaymentEntity]):
    """In-memory repository for agriculture payment"""

    def __init__(self):
        self._storage: Dict[str, AgriculturePaymentEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: AgriculturePaymentEntity) -> AgriculturePaymentEntity:
    ...
    async def read(self, id: str) -> Optional[AgriculturePaymentEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: AgriculturePaymentEntity) -> AgriculturePaymentEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[AgriculturePaymentEntity]:
    ...
    async def find_by(self, **kwargs) -> List[AgriculturePaymentEntity]:
...
class AgriculturePaymentCachedRepository(BaseAgricultureRepository[AgriculturePaymentEntity]):
    """Cached repository wrapper for agriculture payment"""
...
class AgriculturePaymentUnitOfWork:
    """Unit of Work pattern for agriculture payment"""

    def __init__(self, repository: BaseAgricultureRepository):
        self._repository = repository
        self._new: List[AgriculturePaymentEntity] = []
        self._dirty: List[AgriculturePaymentEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: AgriculturePaymentEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: AgriculturePaymentEntity) -> None:
