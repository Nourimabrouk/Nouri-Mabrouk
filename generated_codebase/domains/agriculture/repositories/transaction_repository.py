"""
Agriculture Transaction Repository
...
@dataclass
class AgricultureTransactionEntity:
    """Entity for agriculture transaction"""
    id: str
    data: Dict[str, Any]
...
class BaseAgricultureRepository(ABC, Generic[T]):
    """Base repository for agriculture"""
...
class AgricultureTransactionInMemoryRepository(BaseAgricultureRepository[AgricultureTransactionEntity]):
    """In-memory repository for agriculture transaction"""

    def __init__(self):
        self._storage: Dict[str, AgricultureTransactionEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: AgricultureTransactionEntity) -> AgricultureTransactionEntity:
    ...
    async def read(self, id: str) -> Optional[AgricultureTransactionEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: AgricultureTransactionEntity) -> AgricultureTransactionEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[AgricultureTransactionEntity]:
    ...
    async def find_by(self, **kwargs) -> List[AgricultureTransactionEntity]:
...
class AgricultureTransactionCachedRepository(BaseAgricultureRepository[AgricultureTransactionEntity]):
    """Cached repository wrapper for agriculture transaction"""

    def __init__(self, base_repo: BaseAgricultureRepository, cache_ttl: int = 300):
...
class AgricultureTransactionUnitOfWork:
    """Unit of Work pattern for agriculture transaction"""

    def __init__(self, repository: BaseAgricultureRepository):
        self._repository = repository
        self._new: List[AgricultureTransactionEntity] = []
        self._dirty: List[AgricultureTransactionEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: AgricultureTransactionEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: AgricultureTransactionEntity) -> None:
