"""
Agriculture Shipment Repository
...
@dataclass
class AgricultureShipmentEntity:
    """Entity for agriculture shipment"""
    id: str
    data: Dict[str, Any]
...
class AgricultureShipmentInMemoryRepository(BaseAgricultureRepository[AgricultureShipmentEntity]):
    """In-memory repository for agriculture shipment"""

    def __init__(self):
        self._storage: Dict[str, AgricultureShipmentEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: AgricultureShipmentEntity) -> AgricultureShipmentEntity:
    ...
    async def read(self, id: str) -> Optional[AgricultureShipmentEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: AgricultureShipmentEntity) -> AgricultureShipmentEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[AgricultureShipmentEntity]:
    ...
    async def find_by(self, **kwargs) -> List[AgricultureShipmentEntity]:
...
class AgricultureShipmentCachedRepository(BaseAgricultureRepository[AgricultureShipmentEntity]):
    """Cached repository wrapper for agriculture shipment"""
...
class AgricultureShipmentUnitOfWork:
    """Unit of Work pattern for agriculture shipment"""

    def __init__(self, repository: BaseAgricultureRepository):
        self._repository = repository
        self._new: List[AgricultureShipmentEntity] = []
        self._dirty: List[AgricultureShipmentEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: AgricultureShipmentEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: AgricultureShipmentEntity) -> None:
