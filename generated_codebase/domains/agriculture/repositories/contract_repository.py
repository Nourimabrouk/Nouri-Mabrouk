"""
Agriculture Contract Repository
...
@dataclass
class AgricultureContractEntity:
    """Entity for agriculture contract"""
    id: str
    data: Dict[str, Any]
...
class AgricultureContractInMemoryRepository(BaseAgricultureRepository[AgricultureContractEntity]):
    """In-memory repository for agriculture contract"""

    def __init__(self):
        self._storage: Dict[str, AgricultureContractEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: AgricultureContractEntity) -> AgricultureContractEntity:
    ...
    async def read(self, id: str) -> Optional[AgricultureContractEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: AgricultureContractEntity) -> AgricultureContractEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[AgricultureContractEntity]:
    ...
    async def find_by(self, **kwargs) -> List[AgricultureContractEntity]:
...
class AgricultureContractCachedRepository(BaseAgricultureRepository[AgricultureContractEntity]):
    """Cached repository wrapper for agriculture contract"""
...
class AgricultureContractUnitOfWork:
    """Unit of Work pattern for agriculture contract"""

    def __init__(self, repository: BaseAgricultureRepository):
        self._repository = repository
        self._new: List[AgricultureContractEntity] = []
        self._dirty: List[AgricultureContractEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: AgricultureContractEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: AgricultureContractEntity) -> None:
