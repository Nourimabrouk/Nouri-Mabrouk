"""
Agriculture Invoice Repository
...
@dataclass
class AgricultureInvoiceEntity:
    """Entity for agriculture invoice"""
    id: str
    data: Dict[str, Any]
...
class AgricultureInvoiceInMemoryRepository(BaseAgricultureRepository[AgricultureInvoiceEntity]):
    """In-memory repository for agriculture invoice"""

    def __init__(self):
        self._storage: Dict[str, AgricultureInvoiceEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: AgricultureInvoiceEntity) -> AgricultureInvoiceEntity:
    ...
    async def read(self, id: str) -> Optional[AgricultureInvoiceEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: AgricultureInvoiceEntity) -> AgricultureInvoiceEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[AgricultureInvoiceEntity]:
    ...
    async def find_by(self, **kwargs) -> List[AgricultureInvoiceEntity]:
...
class AgricultureInvoiceCachedRepository(BaseAgricultureRepository[AgricultureInvoiceEntity]):
    """Cached repository wrapper for agriculture invoice"""
...
class AgricultureInvoiceUnitOfWork:
    """Unit of Work pattern for agriculture invoice"""

    def __init__(self, repository: BaseAgricultureRepository):
        self._repository = repository
        self._new: List[AgricultureInvoiceEntity] = []
        self._dirty: List[AgricultureInvoiceEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: AgricultureInvoiceEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: AgricultureInvoiceEntity) -> None:
