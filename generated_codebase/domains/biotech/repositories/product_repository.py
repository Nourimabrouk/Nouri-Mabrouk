...
@dataclass
class BiotechProductEntity:
    """Entity for biotech product"""
    id: str
    data: Dict[str, Any]
...
class BiotechProductInMemoryRepository(BaseBiotechRepository[BiotechProductEntity]):
    """In-memory repository for biotech product"""

    def __init__(self):
        self._storage: Dict[str, BiotechProductEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: BiotechProductEntity) -> BiotechProductEntity:
    ...
    async def read(self, id: str) -> Optional[BiotechProductEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: BiotechProductEntity) -> BiotechProductEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[BiotechProductEntity]:
    ...
    async def find_by(self, **kwargs) -> List[BiotechProductEntity]:
...
class BiotechProductCachedRepository(BaseBiotechRepository[BiotechProductEntity]):
    """Cached repository wrapper for biotech product"""
...
class BiotechProductUnitOfWork:
    """Unit of Work pattern for biotech product"""

    def __init__(self, repository: BaseBiotechRepository):
        self._repository = repository
        self._new: List[BiotechProductEntity] = []
        self._dirty: List[BiotechProductEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: BiotechProductEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: BiotechProductEntity) -> None:
