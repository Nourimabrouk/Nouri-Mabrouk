...
@dataclass
class BiotechContractEntity:
    """Entity for biotech contract"""
    id: str
    data: Dict[str, Any]
...
class BiotechContractInMemoryRepository(BaseBiotechRepository[BiotechContractEntity]):
    """In-memory repository for biotech contract"""

    def __init__(self):
        self._storage: Dict[str, BiotechContractEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: BiotechContractEntity) -> BiotechContractEntity:
    ...
    async def read(self, id: str) -> Optional[BiotechContractEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: BiotechContractEntity) -> BiotechContractEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[BiotechContractEntity]:
    ...
    async def find_by(self, **kwargs) -> List[BiotechContractEntity]:
...
class BiotechContractCachedRepository(BaseBiotechRepository[BiotechContractEntity]):
    """Cached repository wrapper for biotech contract"""
...
class BiotechContractUnitOfWork:
    """Unit of Work pattern for biotech contract"""

    def __init__(self, repository: BaseBiotechRepository):
        self._repository = repository
        self._new: List[BiotechContractEntity] = []
        self._dirty: List[BiotechContractEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: BiotechContractEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: BiotechContractEntity) -> None:
