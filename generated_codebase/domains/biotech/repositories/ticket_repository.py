...
@dataclass
class BiotechTicketEntity:
    """Entity for biotech ticket"""
    id: str
    data: Dict[str, Any]
...
class BiotechTicketInMemoryRepository(BaseBiotechRepository[BiotechTicketEntity]):
    """In-memory repository for biotech ticket"""

    def __init__(self):
        self._storage: Dict[str, BiotechTicketEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: BiotechTicketEntity) -> BiotechTicketEntity:
    ...
    async def read(self, id: str) -> Optional[BiotechTicketEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: BiotechTicketEntity) -> BiotechTicketEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[BiotechTicketEntity]:
    ...
    async def find_by(self, **kwargs) -> List[BiotechTicketEntity]:
...
class BiotechTicketCachedRepository(BaseBiotechRepository[BiotechTicketEntity]):
    """Cached repository wrapper for biotech ticket"""
...
class BiotechTicketUnitOfWork:
    """Unit of Work pattern for biotech ticket"""

    def __init__(self, repository: BaseBiotechRepository):
        self._repository = repository
        self._new: List[BiotechTicketEntity] = []
        self._dirty: List[BiotechTicketEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: BiotechTicketEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: BiotechTicketEntity) -> None:
