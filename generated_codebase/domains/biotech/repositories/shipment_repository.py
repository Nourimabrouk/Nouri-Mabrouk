...
@dataclass
class BiotechShipmentEntity:
    """Entity for biotech shipment"""
    id: str
    data: Dict[str, Any]
...
class BiotechShipmentInMemoryRepository(BaseBiotechRepository[BiotechShipmentEntity]):
    """In-memory repository for biotech shipment"""

    def __init__(self):
        self._storage: Dict[str, BiotechShipmentEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: BiotechShipmentEntity) -> BiotechShipmentEntity:
    ...
    async def read(self, id: str) -> Optional[BiotechShipmentEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: BiotechShipmentEntity) -> BiotechShipmentEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[BiotechShipmentEntity]:
    ...
    async def find_by(self, **kwargs) -> List[BiotechShipmentEntity]:
...
class BiotechShipmentCachedRepository(BaseBiotechRepository[BiotechShipmentEntity]):
    """Cached repository wrapper for biotech shipment"""
...
class BiotechShipmentUnitOfWork:
    """Unit of Work pattern for biotech shipment"""

    def __init__(self, repository: BaseBiotechRepository):
        self._repository = repository
        self._new: List[BiotechShipmentEntity] = []
        self._dirty: List[BiotechShipmentEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: BiotechShipmentEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: BiotechShipmentEntity) -> None:
