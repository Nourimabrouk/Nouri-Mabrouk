...
@dataclass
class BiotechCustomerEntity:
    """Entity for biotech customer"""
    id: str
    data: Dict[str, Any]
...
class BiotechCustomerInMemoryRepository(BaseBiotechRepository[BiotechCustomerEntity]):
    """In-memory repository for biotech customer"""

    def __init__(self):
        self._storage: Dict[str, BiotechCustomerEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: BiotechCustomerEntity) -> BiotechCustomerEntity:
    ...
    async def read(self, id: str) -> Optional[BiotechCustomerEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: BiotechCustomerEntity) -> BiotechCustomerEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[BiotechCustomerEntity]:
    ...
    async def find_by(self, **kwargs) -> List[BiotechCustomerEntity]:
...
class BiotechCustomerCachedRepository(BaseBiotechRepository[BiotechCustomerEntity]):
    """Cached repository wrapper for biotech customer"""
...
class BiotechCustomerUnitOfWork:
    """Unit of Work pattern for biotech customer"""

    def __init__(self, repository: BaseBiotechRepository):
        self._repository = repository
        self._new: List[BiotechCustomerEntity] = []
        self._dirty: List[BiotechCustomerEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: BiotechCustomerEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: BiotechCustomerEntity) -> None:
