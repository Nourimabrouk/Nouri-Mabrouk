...
@dataclass
class BiotechInvoiceEntity:
    """Entity for biotech invoice"""
    id: str
    data: Dict[str, Any]
...
class BiotechInvoiceInMemoryRepository(BaseBiotechRepository[BiotechInvoiceEntity]):
    """In-memory repository for biotech invoice"""

    def __init__(self):
        self._storage: Dict[str, BiotechInvoiceEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: BiotechInvoiceEntity) -> BiotechInvoiceEntity:
    ...
    async def read(self, id: str) -> Optional[BiotechInvoiceEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: BiotechInvoiceEntity) -> BiotechInvoiceEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[BiotechInvoiceEntity]:
    ...
    async def find_by(self, **kwargs) -> List[BiotechInvoiceEntity]:
...
class BiotechInvoiceCachedRepository(BaseBiotechRepository[BiotechInvoiceEntity]):
    """Cached repository wrapper for biotech invoice"""
...
class BiotechInvoiceUnitOfWork:
    """Unit of Work pattern for biotech invoice"""

    def __init__(self, repository: BaseBiotechRepository):
        self._repository = repository
        self._new: List[BiotechInvoiceEntity] = []
        self._dirty: List[BiotechInvoiceEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: BiotechInvoiceEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: BiotechInvoiceEntity) -> None:
