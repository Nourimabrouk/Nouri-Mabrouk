...
@dataclass
class BiotechReportEntity:
    """Entity for biotech report"""
    id: str
    data: Dict[str, Any]
...
class BiotechReportInMemoryRepository(BaseBiotechRepository[BiotechReportEntity]):
    """In-memory repository for biotech report"""

    def __init__(self):
        self._storage: Dict[str, BiotechReportEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: BiotechReportEntity) -> BiotechReportEntity:
    ...
    async def read(self, id: str) -> Optional[BiotechReportEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: BiotechReportEntity) -> BiotechReportEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[BiotechReportEntity]:
    ...
    async def find_by(self, **kwargs) -> List[BiotechReportEntity]:
...
class BiotechReportCachedRepository(BaseBiotechRepository[BiotechReportEntity]):
    """Cached repository wrapper for biotech report"""
...
class BiotechReportUnitOfWork:
    """Unit of Work pattern for biotech report"""

    def __init__(self, repository: BaseBiotechRepository):
        self._repository = repository
        self._new: List[BiotechReportEntity] = []
        self._dirty: List[BiotechReportEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: BiotechReportEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: BiotechReportEntity) -> None:
