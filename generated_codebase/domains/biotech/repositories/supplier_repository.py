...
@dataclass
class BiotechSupplierEntity:
    """Entity for biotech supplier"""
    id: str
    data: Dict[str, Any]
...
class BiotechSupplierInMemoryRepository(BaseBiotechRepository[BiotechSupplierEntity]):
    """In-memory repository for biotech supplier"""

    def __init__(self):
        self._storage: Dict[str, BiotechSupplierEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: BiotechSupplierEntity) -> BiotechSupplierEntity:
    ...
    async def read(self, id: str) -> Optional[BiotechSupplierEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: BiotechSupplierEntity) -> BiotechSupplierEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[BiotechSupplierEntity]:
    ...
    async def find_by(self, **kwargs) -> List[BiotechSupplierEntity]:
...
class BiotechSupplierCachedRepository(BaseBiotechRepository[BiotechSupplierEntity]):
    """Cached repository wrapper for biotech supplier"""
...
class BiotechSupplierUnitOfWork:
    """Unit of Work pattern for biotech supplier"""

    def __init__(self, repository: BaseBiotechRepository):
        self._repository = repository
        self._new: List[BiotechSupplierEntity] = []
        self._dirty: List[BiotechSupplierEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: BiotechSupplierEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: BiotechSupplierEntity) -> None:
