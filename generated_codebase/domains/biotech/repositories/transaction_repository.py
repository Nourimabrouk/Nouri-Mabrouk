"""
Biotech Transaction Repository
...
@dataclass
class BiotechTransactionEntity:
    """Entity for biotech transaction"""
    id: str
    data: Dict[str, Any]
...
class BaseBiotechRepository(ABC, Generic[T]):
    """Base repository for biotech"""
...
class BiotechTransactionInMemoryRepository(BaseBiotechRepository[BiotechTransactionEntity]):
    """In-memory repository for biotech transaction"""

    def __init__(self):
        self._storage: Dict[str, BiotechTransactionEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: BiotechTransactionEntity) -> BiotechTransactionEntity:
    ...
    async def read(self, id: str) -> Optional[BiotechTransactionEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: BiotechTransactionEntity) -> BiotechTransactionEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[BiotechTransactionEntity]:
    ...
    async def find_by(self, **kwargs) -> List[BiotechTransactionEntity]:
...
class BiotechTransactionCachedRepository(BaseBiotechRepository[BiotechTransactionEntity]):
    """Cached repository wrapper for biotech transaction"""

    def __init__(self, base_repo: BaseBiotechRepository, cache_ttl: int = 300):
...
class BiotechTransactionUnitOfWork:
    """Unit of Work pattern for biotech transaction"""

    def __init__(self, repository: BaseBiotechRepository):
        self._repository = repository
        self._new: List[BiotechTransactionEntity] = []
        self._dirty: List[BiotechTransactionEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: BiotechTransactionEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: BiotechTransactionEntity) -> None:
