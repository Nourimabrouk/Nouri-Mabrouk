...
@dataclass
class BiotechOrderEntity:
    """Entity for biotech order"""
    id: str
    data: Dict[str, Any]
...
class BiotechOrderInMemoryRepository(BaseBiotechRepository[BiotechOrderEntity]):
    """In-memory repository for biotech order"""

    def __init__(self):
        self._storage: Dict[str, BiotechOrderEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: BiotechOrderEntity) -> BiotechOrderEntity:
    ...
    async def read(self, id: str) -> Optional[BiotechOrderEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: BiotechOrderEntity) -> BiotechOrderEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[BiotechOrderEntity]:
    ...
    async def find_by(self, **kwargs) -> List[BiotechOrderEntity]:
...
class BiotechOrderCachedRepository(BaseBiotechRepository[BiotechOrderEntity]):
    """Cached repository wrapper for biotech order"""
...
class BiotechOrderUnitOfWork:
    """Unit of Work pattern for biotech order"""

    def __init__(self, repository: BaseBiotechRepository):
        self._repository = repository
        self._new: List[BiotechOrderEntity] = []
        self._dirty: List[BiotechOrderEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: BiotechOrderEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: BiotechOrderEntity) -> None:
