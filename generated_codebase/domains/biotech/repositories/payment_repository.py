...
@dataclass
class BiotechPaymentEntity:
    """Entity for biotech payment"""
    id: str
    data: Dict[str, Any]
...
class BiotechPaymentInMemoryRepository(BaseBiotechRepository[BiotechPaymentEntity]):
    """In-memory repository for biotech payment"""

    def __init__(self):
        self._storage: Dict[str, BiotechPaymentEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: BiotechPaymentEntity) -> BiotechPaymentEntity:
    ...
    async def read(self, id: str) -> Optional[BiotechPaymentEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: BiotechPaymentEntity) -> BiotechPaymentEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[BiotechPaymentEntity]:
    ...
    async def find_by(self, **kwargs) -> List[BiotechPaymentEntity]:
...
class BiotechPaymentCachedRepository(BaseBiotechRepository[BiotechPaymentEntity]):
    """Cached repository wrapper for biotech payment"""
...
class BiotechPaymentUnitOfWork:
    """Unit of Work pattern for biotech payment"""

    def __init__(self, repository: BaseBiotechRepository):
        self._repository = repository
        self._new: List[BiotechPaymentEntity] = []
        self._dirty: List[BiotechPaymentEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: BiotechPaymentEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: BiotechPaymentEntity) -> None:
