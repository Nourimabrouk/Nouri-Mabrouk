...
@dataclass
class BiotechEmployeeEntity:
    """Entity for biotech employee"""
    id: str
    data: Dict[str, Any]
...
class BiotechEmployeeInMemoryRepository(BaseBiotechRepository[BiotechEmployeeEntity]):
    """In-memory repository for biotech employee"""

    def __init__(self):
        self._storage: Dict[str, BiotechEmployeeEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: BiotechEmployeeEntity) -> BiotechEmployeeEntity:
    ...
    async def read(self, id: str) -> Optional[BiotechEmployeeEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: BiotechEmployeeEntity) -> BiotechEmployeeEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[BiotechEmployeeEntity]:
    ...
    async def find_by(self, **kwargs) -> List[BiotechEmployeeEntity]:
...
class BiotechEmployeeCachedRepository(BaseBiotechRepository[BiotechEmployeeEntity]):
    """Cached repository wrapper for biotech employee"""
...
class BiotechEmployeeUnitOfWork:
    """Unit of Work pattern for biotech employee"""

    def __init__(self, repository: BaseBiotechRepository):
        self._repository = repository
        self._new: List[BiotechEmployeeEntity] = []
        self._dirty: List[BiotechEmployeeEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: BiotechEmployeeEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: BiotechEmployeeEntity) -> None:
