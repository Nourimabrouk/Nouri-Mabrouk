...
@dataclass
class EducationContractEntity:
    """Entity for education contract"""
    id: str
    data: Dict[str, Any]
...
class EducationContractInMemoryRepository(BaseEducationRepository[EducationContractEntity]):
    """In-memory repository for education contract"""

    def __init__(self):
        self._storage: Dict[str, EducationContractEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: EducationContractEntity) -> EducationContractEntity:
    ...
    async def read(self, id: str) -> Optional[EducationContractEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: EducationContractEntity) -> EducationContractEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[EducationContractEntity]:
    ...
    async def find_by(self, **kwargs) -> List[EducationContractEntity]:
...
class EducationContractCachedRepository(BaseEducationRepository[EducationContractEntity]):
    """Cached repository wrapper for education contract"""
...
class EducationContractUnitOfWork:
    """Unit of Work pattern for education contract"""

    def __init__(self, repository: BaseEducationRepository):
        self._repository = repository
        self._new: List[EducationContractEntity] = []
        self._dirty: List[EducationContractEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: EducationContractEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: EducationContractEntity) -> None:
