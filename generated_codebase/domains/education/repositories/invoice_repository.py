...
@dataclass
class EducationInvoiceEntity:
    """Entity for education invoice"""
    id: str
    data: Dict[str, Any]
...
class EducationInvoiceInMemoryRepository(BaseEducationRepository[EducationInvoiceEntity]):
    """In-memory repository for education invoice"""

    def __init__(self):
        self._storage: Dict[str, EducationInvoiceEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: EducationInvoiceEntity) -> EducationInvoiceEntity:
    ...
    async def read(self, id: str) -> Optional[EducationInvoiceEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: EducationInvoiceEntity) -> EducationInvoiceEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[EducationInvoiceEntity]:
    ...
    async def find_by(self, **kwargs) -> List[EducationInvoiceEntity]:
...
class EducationInvoiceCachedRepository(BaseEducationRepository[EducationInvoiceEntity]):
    """Cached repository wrapper for education invoice"""
...
class EducationInvoiceUnitOfWork:
    """Unit of Work pattern for education invoice"""

    def __init__(self, repository: BaseEducationRepository):
        self._repository = repository
        self._new: List[EducationInvoiceEntity] = []
        self._dirty: List[EducationInvoiceEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: EducationInvoiceEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: EducationInvoiceEntity) -> None:
