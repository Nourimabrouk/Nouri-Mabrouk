...
@dataclass
class EducationTicketEntity:
    """Entity for education ticket"""
    id: str
    data: Dict[str, Any]
...
class EducationTicketInMemoryRepository(BaseEducationRepository[EducationTicketEntity]):
    """In-memory repository for education ticket"""

    def __init__(self):
        self._storage: Dict[str, EducationTicketEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: EducationTicketEntity) -> EducationTicketEntity:
    ...
    async def read(self, id: str) -> Optional[EducationTicketEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: EducationTicketEntity) -> EducationTicketEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[EducationTicketEntity]:
    ...
    async def find_by(self, **kwargs) -> List[EducationTicketEntity]:
...
class EducationTicketCachedRepository(BaseEducationRepository[EducationTicketEntity]):
    """Cached repository wrapper for education ticket"""
...
class EducationTicketUnitOfWork:
    """Unit of Work pattern for education ticket"""

    def __init__(self, repository: BaseEducationRepository):
        self._repository = repository
        self._new: List[EducationTicketEntity] = []
        self._dirty: List[EducationTicketEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: EducationTicketEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: EducationTicketEntity) -> None:
