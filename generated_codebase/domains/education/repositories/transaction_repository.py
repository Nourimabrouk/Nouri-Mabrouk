"""
Education Transaction Repository
...
@dataclass
class EducationTransactionEntity:
    """Entity for education transaction"""
    id: str
    data: Dict[str, Any]
...
class BaseEducationRepository(ABC, Generic[T]):
    """Base repository for education"""
...
class EducationTransactionInMemoryRepository(BaseEducationRepository[EducationTransactionEntity]):
    """In-memory repository for education transaction"""

    def __init__(self):
        self._storage: Dict[str, EducationTransactionEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: EducationTransactionEntity) -> EducationTransactionEntity:
    ...
    async def read(self, id: str) -> Optional[EducationTransactionEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: EducationTransactionEntity) -> EducationTransactionEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[EducationTransactionEntity]:
    ...
    async def find_by(self, **kwargs) -> List[EducationTransactionEntity]:
...
class EducationTransactionCachedRepository(BaseEducationRepository[EducationTransactionEntity]):
    """Cached repository wrapper for education transaction"""

    def __init__(self, base_repo: BaseEducationRepository, cache_ttl: int = 300):
...
class EducationTransactionUnitOfWork:
    """Unit of Work pattern for education transaction"""

    def __init__(self, repository: BaseEducationRepository):
        self._repository = repository
        self._new: List[EducationTransactionEntity] = []
        self._dirty: List[EducationTransactionEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: EducationTransactionEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: EducationTransactionEntity) -> None:
