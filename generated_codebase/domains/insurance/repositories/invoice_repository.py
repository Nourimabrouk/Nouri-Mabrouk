...
@dataclass
class InsuranceInvoiceEntity:
    """Entity for insurance invoice"""
    id: str
    data: Dict[str, Any]
...
class InsuranceInvoiceInMemoryRepository(BaseInsuranceRepository[InsuranceInvoiceEntity]):
    """In-memory repository for insurance invoice"""

    def __init__(self):
        self._storage: Dict[str, InsuranceInvoiceEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: InsuranceInvoiceEntity) -> InsuranceInvoiceEntity:
    ...
    async def read(self, id: str) -> Optional[InsuranceInvoiceEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: InsuranceInvoiceEntity) -> InsuranceInvoiceEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[InsuranceInvoiceEntity]:
    ...
    async def find_by(self, **kwargs) -> List[InsuranceInvoiceEntity]:
...
class InsuranceInvoiceCachedRepository(BaseInsuranceRepository[InsuranceInvoiceEntity]):
    """Cached repository wrapper for insurance invoice"""
...
class InsuranceInvoiceUnitOfWork:
    """Unit of Work pattern for insurance invoice"""

    def __init__(self, repository: BaseInsuranceRepository):
        self._repository = repository
        self._new: List[InsuranceInvoiceEntity] = []
        self._dirty: List[InsuranceInvoiceEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: InsuranceInvoiceEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: InsuranceInvoiceEntity) -> None:
