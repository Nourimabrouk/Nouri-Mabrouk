...
@dataclass
class InsurancePaymentEntity:
    """Entity for insurance payment"""
    id: str
    data: Dict[str, Any]
...
class InsurancePaymentInMemoryRepository(BaseInsuranceRepository[InsurancePaymentEntity]):
    """In-memory repository for insurance payment"""

    def __init__(self):
        self._storage: Dict[str, InsurancePaymentEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: InsurancePaymentEntity) -> InsurancePaymentEntity:
    ...
    async def read(self, id: str) -> Optional[InsurancePaymentEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: InsurancePaymentEntity) -> InsurancePaymentEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[InsurancePaymentEntity]:
    ...
    async def find_by(self, **kwargs) -> List[InsurancePaymentEntity]:
...
class InsurancePaymentCachedRepository(BaseInsuranceRepository[InsurancePaymentEntity]):
    """Cached repository wrapper for insurance payment"""
...
class InsurancePaymentUnitOfWork:
    """Unit of Work pattern for insurance payment"""

    def __init__(self, repository: BaseInsuranceRepository):
        self._repository = repository
        self._new: List[InsurancePaymentEntity] = []
        self._dirty: List[InsurancePaymentEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: InsurancePaymentEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: InsurancePaymentEntity) -> None:
