...
class InsuranceProductInMemoryRepository(BaseInsuranceRepository[InsuranceProductEntity]):
    """In-memory repository for insurance product"""

    def __init__(self):
        self._storage: Dict[str, InsuranceProductEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: InsuranceProductEntity) -> InsuranceProductEntity:
    ...
    async def read(self, id: str) -> Optional[InsuranceProductEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: InsuranceProductEntity) -> InsuranceProductEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[InsuranceProductEntity]:
    ...
    async def find_by(self, **kwargs) -> List[InsuranceProductEntity]:
...
class InsuranceProductCachedRepository(BaseInsuranceRepository[InsuranceProductEntity]):
    """Cached repository wrapper for insurance product"""
...
class InsuranceProductUnitOfWork:
    """Unit of Work pattern for insurance product"""

    def __init__(self, repository: BaseInsuranceRepository):
        self._repository = repository
        self._new: List[InsuranceProductEntity] = []
        self._dirty: List[InsuranceProductEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: InsuranceProductEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: InsuranceProductEntity) -> None:
