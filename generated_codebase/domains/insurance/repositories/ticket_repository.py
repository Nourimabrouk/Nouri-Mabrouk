...
@dataclass
class InsuranceTicketEntity:
    """Entity for insurance ticket"""
    id: str
    data: Dict[str, Any]
...
class InsuranceTicketInMemoryRepository(BaseInsuranceRepository[InsuranceTicketEntity]):
    """In-memory repository for insurance ticket"""

    def __init__(self):
        self._storage: Dict[str, InsuranceTicketEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: InsuranceTicketEntity) -> InsuranceTicketEntity:
    ...
    async def read(self, id: str) -> Optional[InsuranceTicketEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: InsuranceTicketEntity) -> InsuranceTicketEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[InsuranceTicketEntity]:
    ...
    async def find_by(self, **kwargs) -> List[InsuranceTicketEntity]:
...
class InsuranceTicketCachedRepository(BaseInsuranceRepository[InsuranceTicketEntity]):
    """Cached repository wrapper for insurance ticket"""
...
class InsuranceTicketUnitOfWork:
    """Unit of Work pattern for insurance ticket"""

    def __init__(self, repository: BaseInsuranceRepository):
        self._repository = repository
        self._new: List[InsuranceTicketEntity] = []
        self._dirty: List[InsuranceTicketEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: InsuranceTicketEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: InsuranceTicketEntity) -> None:
