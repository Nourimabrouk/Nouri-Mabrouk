...
@dataclass
class InsuranceContractEntity:
    """Entity for insurance contract"""
    id: str
    data: Dict[str, Any]
...
class InsuranceContractInMemoryRepository(BaseInsuranceRepository[InsuranceContractEntity]):
    """In-memory repository for insurance contract"""

    def __init__(self):
        self._storage: Dict[str, InsuranceContractEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: InsuranceContractEntity) -> InsuranceContractEntity:
    ...
    async def read(self, id: str) -> Optional[InsuranceContractEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: InsuranceContractEntity) -> InsuranceContractEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[InsuranceContractEntity]:
    ...
    async def find_by(self, **kwargs) -> List[InsuranceContractEntity]:
...
class InsuranceContractCachedRepository(BaseInsuranceRepository[InsuranceContractEntity]):
    """Cached repository wrapper for insurance contract"""
...
class InsuranceContractUnitOfWork:
    """Unit of Work pattern for insurance contract"""

    def __init__(self, repository: BaseInsuranceRepository):
        self._repository = repository
        self._new: List[InsuranceContractEntity] = []
        self._dirty: List[InsuranceContractEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: InsuranceContractEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: InsuranceContractEntity) -> None:
