...
@dataclass
class InsuranceOrderEntity:
    """Entity for insurance order"""
    id: str
    data: Dict[str, Any]
...
class InsuranceOrderInMemoryRepository(BaseInsuranceRepository[InsuranceOrderEntity]):
    """In-memory repository for insurance order"""

    def __init__(self):
        self._storage: Dict[str, InsuranceOrderEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: InsuranceOrderEntity) -> InsuranceOrderEntity:
    ...
    async def read(self, id: str) -> Optional[InsuranceOrderEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: InsuranceOrderEntity) -> InsuranceOrderEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[InsuranceOrderEntity]:
    ...
    async def find_by(self, **kwargs) -> List[InsuranceOrderEntity]:
...
class InsuranceOrderCachedRepository(BaseInsuranceRepository[InsuranceOrderEntity]):
    """Cached repository wrapper for insurance order"""
...
class InsuranceOrderUnitOfWork:
    """Unit of Work pattern for insurance order"""

    def __init__(self, repository: BaseInsuranceRepository):
        self._repository = repository
        self._new: List[InsuranceOrderEntity] = []
        self._dirty: List[InsuranceOrderEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: InsuranceOrderEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: InsuranceOrderEntity) -> None:
