"""
Insurance Transaction Repository
...
@dataclass
class InsuranceTransactionEntity:
    """Entity for insurance transaction"""
    id: str
    data: Dict[str, Any]
...
class BaseInsuranceRepository(ABC, Generic[T]):
    """Base repository for insurance"""
...
class InsuranceTransactionInMemoryRepository(BaseInsuranceRepository[InsuranceTransactionEntity]):
    """In-memory repository for insurance transaction"""

    def __init__(self):
        self._storage: Dict[str, InsuranceTransactionEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: InsuranceTransactionEntity) -> InsuranceTransactionEntity:
    ...
    async def read(self, id: str) -> Optional[InsuranceTransactionEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: InsuranceTransactionEntity) -> InsuranceTransactionEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[InsuranceTransactionEntity]:
    ...
    async def find_by(self, **kwargs) -> List[InsuranceTransactionEntity]:
...
class InsuranceTransactionCachedRepository(BaseInsuranceRepository[InsuranceTransactionEntity]):
    """Cached repository wrapper for insurance transaction"""

    def __init__(self, base_repo: BaseInsuranceRepository, cache_ttl: int = 300):
...
class InsuranceTransactionUnitOfWork:
    """Unit of Work pattern for insurance transaction"""

    def __init__(self, repository: BaseInsuranceRepository):
        self._repository = repository
        self._new: List[InsuranceTransactionEntity] = []
        self._dirty: List[InsuranceTransactionEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: InsuranceTransactionEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: InsuranceTransactionEntity) -> None:
