...
@dataclass
class InsuranceShipmentEntity:
    """Entity for insurance shipment"""
    id: str
    data: Dict[str, Any]
...
class InsuranceShipmentInMemoryRepository(BaseInsuranceRepository[InsuranceShipmentEntity]):
    """In-memory repository for insurance shipment"""

    def __init__(self):
        self._storage: Dict[str, InsuranceShipmentEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: InsuranceShipmentEntity) -> InsuranceShipmentEntity:
    ...
    async def read(self, id: str) -> Optional[InsuranceShipmentEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: InsuranceShipmentEntity) -> InsuranceShipmentEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[InsuranceShipmentEntity]:
    ...
    async def find_by(self, **kwargs) -> List[InsuranceShipmentEntity]:
...
class InsuranceShipmentCachedRepository(BaseInsuranceRepository[InsuranceShipmentEntity]):
    """Cached repository wrapper for insurance shipment"""
...
class InsuranceShipmentUnitOfWork:
    """Unit of Work pattern for insurance shipment"""

    def __init__(self, repository: BaseInsuranceRepository):
        self._repository = repository
        self._new: List[InsuranceShipmentEntity] = []
        self._dirty: List[InsuranceShipmentEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: InsuranceShipmentEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: InsuranceShipmentEntity) -> None:
