...
@dataclass
class InsuranceReportEntity:
    """Entity for insurance report"""
    id: str
    data: Dict[str, Any]
...
class InsuranceReportInMemoryRepository(BaseInsuranceRepository[InsuranceReportEntity]):
    """In-memory repository for insurance report"""

    def __init__(self):
        self._storage: Dict[str, InsuranceReportEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: InsuranceReportEntity) -> InsuranceReportEntity:
    ...
    async def read(self, id: str) -> Optional[InsuranceReportEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: InsuranceReportEntity) -> InsuranceReportEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[InsuranceReportEntity]:
    ...
    async def find_by(self, **kwargs) -> List[InsuranceReportEntity]:
...
class InsuranceReportCachedRepository(BaseInsuranceRepository[InsuranceReportEntity]):
    """Cached repository wrapper for insurance report"""
...
class InsuranceReportUnitOfWork:
    """Unit of Work pattern for insurance report"""

    def __init__(self, repository: BaseInsuranceRepository):
        self._repository = repository
        self._new: List[InsuranceReportEntity] = []
        self._dirty: List[InsuranceReportEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: InsuranceReportEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: InsuranceReportEntity) -> None:
