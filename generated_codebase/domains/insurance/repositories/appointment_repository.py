"""
Insurance Appointment Repository
...
@dataclass
class InsuranceAppointmentEntity:
    """Entity for insurance appointment"""
    id: str
    data: Dict[str, Any]
...
class InsuranceAppointmentInMemoryRepository(BaseInsuranceRepository[InsuranceAppointmentEntity]):
    """In-memory repository for insurance appointment"""

    def __init__(self):
        self._storage: Dict[str, InsuranceAppointmentEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: InsuranceAppointmentEntity) -> InsuranceAppointmentEntity:
    ...
    async def read(self, id: str) -> Optional[InsuranceAppointmentEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: InsuranceAppointmentEntity) -> InsuranceAppointmentEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[InsuranceAppointmentEntity]:
    ...
    async def find_by(self, **kwargs) -> List[InsuranceAppointmentEntity]:
...
class InsuranceAppointmentCachedRepository(BaseInsuranceRepository[InsuranceAppointmentEntity]):
    """Cached repository wrapper for insurance appointment"""
...
class InsuranceAppointmentUnitOfWork:
    """Unit of Work pattern for insurance appointment"""

    def __init__(self, repository: BaseInsuranceRepository):
        self._repository = repository
        self._new: List[InsuranceAppointmentEntity] = []
        self._dirty: List[InsuranceAppointmentEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: InsuranceAppointmentEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: InsuranceAppointmentEntity) -> None:
