...
@dataclass
class RetailSupplierEntity:
    """Entity for retail supplier"""
    id: str
    data: Dict[str, Any]
...
class RetailSupplierInMemoryRepository(BaseRetailRepository[RetailSupplierEntity]):
    """In-memory repository for retail supplier"""

    def __init__(self):
        self._storage: Dict[str, RetailSupplierEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: RetailSupplierEntity) -> RetailSupplierEntity:
    ...
    async def read(self, id: str) -> Optional[RetailSupplierEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: RetailSupplierEntity) -> RetailSupplierEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[RetailSupplierEntity]:
    ...
    async def find_by(self, **kwargs) -> List[RetailSupplierEntity]:
...
class RetailSupplierCachedRepository(BaseRetailRepository[RetailSupplierEntity]):
    """Cached repository wrapper for retail supplier"""
...
class RetailSupplierUnitOfWork:
    """Unit of Work pattern for retail supplier"""

    def __init__(self, repository: BaseRetailRepository):
        self._repository = repository
        self._new: List[RetailSupplierEntity] = []
        self._dirty: List[RetailSupplierEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: RetailSupplierEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: RetailSupplierEntity) -> None:
