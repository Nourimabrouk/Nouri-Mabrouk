...
@dataclass
class RetailInvoiceEntity:
    """Entity for retail invoice"""
    id: str
    data: Dict[str, Any]
...
class RetailInvoiceInMemoryRepository(BaseRetailRepository[RetailInvoiceEntity]):
    """In-memory repository for retail invoice"""

    def __init__(self):
        self._storage: Dict[str, RetailInvoiceEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: RetailInvoiceEntity) -> RetailInvoiceEntity:
    ...
    async def read(self, id: str) -> Optional[RetailInvoiceEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: RetailInvoiceEntity) -> RetailInvoiceEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[RetailInvoiceEntity]:
    ...
    async def find_by(self, **kwargs) -> List[RetailInvoiceEntity]:
...
class RetailInvoiceCachedRepository(BaseRetailRepository[RetailInvoiceEntity]):
    """Cached repository wrapper for retail invoice"""
...
class RetailInvoiceUnitOfWork:
    """Unit of Work pattern for retail invoice"""

    def __init__(self, repository: BaseRetailRepository):
        self._repository = repository
        self._new: List[RetailInvoiceEntity] = []
        self._dirty: List[RetailInvoiceEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: RetailInvoiceEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: RetailInvoiceEntity) -> None:
