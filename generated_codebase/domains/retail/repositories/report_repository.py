...
@dataclass
class RetailReportEntity:
    """Entity for retail report"""
    id: str
    data: Dict[str, Any]
...
class RetailReportInMemoryRepository(BaseRetailRepository[RetailReportEntity]):
    """In-memory repository for retail report"""

    def __init__(self):
        self._storage: Dict[str, RetailReportEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: RetailReportEntity) -> RetailReportEntity:
    ...
    async def read(self, id: str) -> Optional[RetailReportEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: RetailReportEntity) -> RetailReportEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[RetailReportEntity]:
    ...
    async def find_by(self, **kwargs) -> List[RetailReportEntity]:
...
class RetailReportCachedRepository(BaseRetailRepository[RetailReportEntity]):
    """Cached repository wrapper for retail report"""
...
class RetailReportUnitOfWork:
    """Unit of Work pattern for retail report"""

    def __init__(self, repository: BaseRetailRepository):
        self._repository = repository
        self._new: List[RetailReportEntity] = []
        self._dirty: List[RetailReportEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: RetailReportEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: RetailReportEntity) -> None:
