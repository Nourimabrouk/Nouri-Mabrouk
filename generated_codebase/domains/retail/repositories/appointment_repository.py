...
@dataclass
class RetailAppointmentEntity:
    """Entity for retail appointment"""
    id: str
    data: Dict[str, Any]
...
class RetailAppointmentInMemoryRepository(BaseRetailRepository[RetailAppointmentEntity]):
    """In-memory repository for retail appointment"""

    def __init__(self):
        self._storage: Dict[str, RetailAppointmentEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: RetailAppointmentEntity) -> RetailAppointmentEntity:
    ...
    async def read(self, id: str) -> Optional[RetailAppointmentEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: RetailAppointmentEntity) -> RetailAppointmentEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[RetailAppointmentEntity]:
    ...
    async def find_by(self, **kwargs) -> List[RetailAppointmentEntity]:
...
class RetailAppointmentCachedRepository(BaseRetailRepository[RetailAppointmentEntity]):
    """Cached repository wrapper for retail appointment"""
...
class RetailAppointmentUnitOfWork:
    """Unit of Work pattern for retail appointment"""

    def __init__(self, repository: BaseRetailRepository):
        self._repository = repository
        self._new: List[RetailAppointmentEntity] = []
        self._dirty: List[RetailAppointmentEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: RetailAppointmentEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: RetailAppointmentEntity) -> None:
