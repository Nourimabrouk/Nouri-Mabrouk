...
@dataclass
class RetailInventoryEntity:
    """Entity for retail inventory"""
    id: str
    data: Dict[str, Any]
...
class RetailInventoryInMemoryRepository(BaseRetailRepository[RetailInventoryEntity]):
    """In-memory repository for retail inventory"""

    def __init__(self):
        self._storage: Dict[str, RetailInventoryEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: RetailInventoryEntity) -> RetailInventoryEntity:
    ...
    async def read(self, id: str) -> Optional[RetailInventoryEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: RetailInventoryEntity) -> RetailInventoryEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[RetailInventoryEntity]:
    ...
    async def find_by(self, **kwargs) -> List[RetailInventoryEntity]:
...
class RetailInventoryCachedRepository(BaseRetailRepository[RetailInventoryEntity]):
    """Cached repository wrapper for retail inventory"""
...
class RetailInventoryUnitOfWork:
    """Unit of Work pattern for retail inventory"""

    def __init__(self, repository: BaseRetailRepository):
        self._repository = repository
        self._new: List[RetailInventoryEntity] = []
        self._dirty: List[RetailInventoryEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: RetailInventoryEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: RetailInventoryEntity) -> None:
