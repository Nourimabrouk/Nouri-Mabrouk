...
@dataclass
class RetailTicketEntity:
    """Entity for retail ticket"""
    id: str
    data: Dict[str, Any]
...
class RetailTicketInMemoryRepository(BaseRetailRepository[RetailTicketEntity]):
    """In-memory repository for retail ticket"""

    def __init__(self):
        self._storage: Dict[str, RetailTicketEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: RetailTicketEntity) -> RetailTicketEntity:
    ...
    async def read(self, id: str) -> Optional[RetailTicketEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: RetailTicketEntity) -> RetailTicketEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[RetailTicketEntity]:
    ...
    async def find_by(self, **kwargs) -> List[RetailTicketEntity]:
...
class RetailTicketCachedRepository(BaseRetailRepository[RetailTicketEntity]):
    """Cached repository wrapper for retail ticket"""
...
class RetailTicketUnitOfWork:
    """Unit of Work pattern for retail ticket"""

    def __init__(self, repository: BaseRetailRepository):
        self._repository = repository
        self._new: List[RetailTicketEntity] = []
        self._dirty: List[RetailTicketEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: RetailTicketEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: RetailTicketEntity) -> None:
