...
@dataclass
class RetailPaymentEntity:
    """Entity for retail payment"""
    id: str
    data: Dict[str, Any]
...
class RetailPaymentInMemoryRepository(BaseRetailRepository[RetailPaymentEntity]):
    """In-memory repository for retail payment"""

    def __init__(self):
        self._storage: Dict[str, RetailPaymentEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: RetailPaymentEntity) -> RetailPaymentEntity:
    ...
    async def read(self, id: str) -> Optional[RetailPaymentEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: RetailPaymentEntity) -> RetailPaymentEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[RetailPaymentEntity]:
    ...
    async def find_by(self, **kwargs) -> List[RetailPaymentEntity]:
...
class RetailPaymentCachedRepository(BaseRetailRepository[RetailPaymentEntity]):
    """Cached repository wrapper for retail payment"""
...
class RetailPaymentUnitOfWork:
    """Unit of Work pattern for retail payment"""

    def __init__(self, repository: BaseRetailRepository):
        self._repository = repository
        self._new: List[RetailPaymentEntity] = []
        self._dirty: List[RetailPaymentEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: RetailPaymentEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: RetailPaymentEntity) -> None:
