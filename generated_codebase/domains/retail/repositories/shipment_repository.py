...
@dataclass
class RetailShipmentEntity:
    """Entity for retail shipment"""
    id: str
    data: Dict[str, Any]
...
class RetailShipmentInMemoryRepository(BaseRetailRepository[RetailShipmentEntity]):
    """In-memory repository for retail shipment"""

    def __init__(self):
        self._storage: Dict[str, RetailShipmentEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: RetailShipmentEntity) -> RetailShipmentEntity:
    ...
    async def read(self, id: str) -> Optional[RetailShipmentEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: RetailShipmentEntity) -> RetailShipmentEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[RetailShipmentEntity]:
    ...
    async def find_by(self, **kwargs) -> List[RetailShipmentEntity]:
...
class RetailShipmentCachedRepository(BaseRetailRepository[RetailShipmentEntity]):
    """Cached repository wrapper for retail shipment"""
...
class RetailShipmentUnitOfWork:
    """Unit of Work pattern for retail shipment"""

    def __init__(self, repository: BaseRetailRepository):
        self._repository = repository
        self._new: List[RetailShipmentEntity] = []
        self._dirty: List[RetailShipmentEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: RetailShipmentEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: RetailShipmentEntity) -> None:
