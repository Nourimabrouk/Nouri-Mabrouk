...
@dataclass
class RetailContractEntity:
    """Entity for retail contract"""
    id: str
    data: Dict[str, Any]
...
class RetailContractInMemoryRepository(BaseRetailRepository[RetailContractEntity]):
    """In-memory repository for retail contract"""

    def __init__(self):
        self._storage: Dict[str, RetailContractEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: RetailContractEntity) -> RetailContractEntity:
    ...
    async def read(self, id: str) -> Optional[RetailContractEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: RetailContractEntity) -> RetailContractEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[RetailContractEntity]:
    ...
    async def find_by(self, **kwargs) -> List[RetailContractEntity]:
...
class RetailContractCachedRepository(BaseRetailRepository[RetailContractEntity]):
    """Cached repository wrapper for retail contract"""
...
class RetailContractUnitOfWork:
    """Unit of Work pattern for retail contract"""

    def __init__(self, repository: BaseRetailRepository):
        self._repository = repository
        self._new: List[RetailContractEntity] = []
        self._dirty: List[RetailContractEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: RetailContractEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: RetailContractEntity) -> None:
