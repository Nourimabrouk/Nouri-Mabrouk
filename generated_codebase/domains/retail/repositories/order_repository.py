...
@dataclass
class RetailOrderEntity:
    """Entity for retail order"""
    id: str
    data: Dict[str, Any]
...
class RetailOrderInMemoryRepository(BaseRetailRepository[RetailOrderEntity]):
    """In-memory repository for retail order"""

    def __init__(self):
        self._storage: Dict[str, RetailOrderEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: RetailOrderEntity) -> RetailOrderEntity:
    ...
    async def read(self, id: str) -> Optional[RetailOrderEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: RetailOrderEntity) -> RetailOrderEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[RetailOrderEntity]:
    ...
    async def find_by(self, **kwargs) -> List[RetailOrderEntity]:
...
class RetailOrderCachedRepository(BaseRetailRepository[RetailOrderEntity]):
    """Cached repository wrapper for retail order"""
...
class RetailOrderUnitOfWork:
    """Unit of Work pattern for retail order"""

    def __init__(self, repository: BaseRetailRepository):
        self._repository = repository
        self._new: List[RetailOrderEntity] = []
        self._dirty: List[RetailOrderEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: RetailOrderEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: RetailOrderEntity) -> None:
