...
@dataclass
class RetailEmployeeEntity:
    """Entity for retail employee"""
    id: str
    data: Dict[str, Any]
...
class RetailEmployeeInMemoryRepository(BaseRetailRepository[RetailEmployeeEntity]):
    """In-memory repository for retail employee"""

    def __init__(self):
        self._storage: Dict[str, RetailEmployeeEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: RetailEmployeeEntity) -> RetailEmployeeEntity:
    ...
    async def read(self, id: str) -> Optional[RetailEmployeeEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: RetailEmployeeEntity) -> RetailEmployeeEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[RetailEmployeeEntity]:
    ...
    async def find_by(self, **kwargs) -> List[RetailEmployeeEntity]:
...
class RetailEmployeeCachedRepository(BaseRetailRepository[RetailEmployeeEntity]):
    """Cached repository wrapper for retail employee"""
...
class RetailEmployeeUnitOfWork:
    """Unit of Work pattern for retail employee"""

    def __init__(self, repository: BaseRetailRepository):
        self._repository = repository
        self._new: List[RetailEmployeeEntity] = []
        self._dirty: List[RetailEmployeeEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: RetailEmployeeEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: RetailEmployeeEntity) -> None:
