...
@dataclass
class RetailTransactionEntity:
    """Entity for retail transaction"""
    id: str
    data: Dict[str, Any]
...
class BaseRetailRepository(ABC, Generic[T]):
    """Base repository for retail"""
...
class RetailTransactionInMemoryRepository(BaseRetailRepository[RetailTransactionEntity]):
    """In-memory repository for retail transaction"""

    def __init__(self):
        self._storage: Dict[str, RetailTransactionEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: RetailTransactionEntity) -> RetailTransactionEntity:
    ...
    async def read(self, id: str) -> Optional[RetailTransactionEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: RetailTransactionEntity) -> RetailTransactionEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[RetailTransactionEntity]:
    ...
    async def find_by(self, **kwargs) -> List[RetailTransactionEntity]:
...
class RetailTransactionCachedRepository(BaseRetailRepository[RetailTransactionEntity]):
    """Cached repository wrapper for retail transaction"""

    def __init__(self, base_repo: BaseRetailRepository, cache_ttl: int = 300):
...
class RetailTransactionUnitOfWork:
    """Unit of Work pattern for retail transaction"""

    def __init__(self, repository: BaseRetailRepository):
        self._repository = repository
        self._new: List[RetailTransactionEntity] = []
        self._dirty: List[RetailTransactionEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: RetailTransactionEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: RetailTransactionEntity) -> None:
