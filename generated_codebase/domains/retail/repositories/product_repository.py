...
@dataclass
class RetailProductEntity:
    """Entity for retail product"""
    id: str
    data: Dict[str, Any]
...
class RetailProductInMemoryRepository(BaseRetailRepository[RetailProductEntity]):
    """In-memory repository for retail product"""

    def __init__(self):
        self._storage: Dict[str, RetailProductEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: RetailProductEntity) -> RetailProductEntity:
    ...
    async def read(self, id: str) -> Optional[RetailProductEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: RetailProductEntity) -> RetailProductEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[RetailProductEntity]:
    ...
    async def find_by(self, **kwargs) -> List[RetailProductEntity]:
...
class RetailProductCachedRepository(BaseRetailRepository[RetailProductEntity]):
    """Cached repository wrapper for retail product"""
...
class RetailProductUnitOfWork:
    """Unit of Work pattern for retail product"""

    def __init__(self, repository: BaseRetailRepository):
        self._repository = repository
        self._new: List[RetailProductEntity] = []
        self._dirty: List[RetailProductEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: RetailProductEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: RetailProductEntity) -> None:
