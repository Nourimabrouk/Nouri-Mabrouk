...
@dataclass
class RetailCustomerEntity:
    """Entity for retail customer"""
    id: str
    data: Dict[str, Any]
...
class RetailCustomerInMemoryRepository(BaseRetailRepository[RetailCustomerEntity]):
    """In-memory repository for retail customer"""

    def __init__(self):
        self._storage: Dict[str, RetailCustomerEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: RetailCustomerEntity) -> RetailCustomerEntity:
    ...
    async def read(self, id: str) -> Optional[RetailCustomerEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: RetailCustomerEntity) -> RetailCustomerEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[RetailCustomerEntity]:
    ...
    async def find_by(self, **kwargs) -> List[RetailCustomerEntity]:
...
class RetailCustomerCachedRepository(BaseRetailRepository[RetailCustomerEntity]):
    """Cached repository wrapper for retail customer"""
...
class RetailCustomerUnitOfWork:
    """Unit of Work pattern for retail customer"""

    def __init__(self, repository: BaseRetailRepository):
        self._repository = repository
        self._new: List[RetailCustomerEntity] = []
        self._dirty: List[RetailCustomerEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: RetailCustomerEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: RetailCustomerEntity) -> None:
