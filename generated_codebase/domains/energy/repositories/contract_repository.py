...
@dataclass
class EnergyContractEntity:
    """Entity for energy contract"""
    id: str
    data: Dict[str, Any]
...
class EnergyContractInMemoryRepository(BaseEnergyRepository[EnergyContractEntity]):
    """In-memory repository for energy contract"""

    def __init__(self):
        self._storage: Dict[str, EnergyContractEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: EnergyContractEntity) -> EnergyContractEntity:
    ...
    async def read(self, id: str) -> Optional[EnergyContractEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: EnergyContractEntity) -> EnergyContractEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[EnergyContractEntity]:
    ...
    async def find_by(self, **kwargs) -> List[EnergyContractEntity]:
...
class EnergyContractCachedRepository(BaseEnergyRepository[EnergyContractEntity]):
    """Cached repository wrapper for energy contract"""
...
class EnergyContractUnitOfWork:
    """Unit of Work pattern for energy contract"""

    def __init__(self, repository: BaseEnergyRepository):
        self._repository = repository
        self._new: List[EnergyContractEntity] = []
        self._dirty: List[EnergyContractEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: EnergyContractEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: EnergyContractEntity) -> None:
