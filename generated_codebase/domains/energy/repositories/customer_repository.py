...
@dataclass
class EnergyCustomerEntity:
    """Entity for energy customer"""
    id: str
    data: Dict[str, Any]
...
class EnergyCustomerInMemoryRepository(BaseEnergyRepository[EnergyCustomerEntity]):
    """In-memory repository for energy customer"""

    def __init__(self):
        self._storage: Dict[str, EnergyCustomerEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: EnergyCustomerEntity) -> EnergyCustomerEntity:
    ...
    async def read(self, id: str) -> Optional[EnergyCustomerEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: EnergyCustomerEntity) -> EnergyCustomerEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[EnergyCustomerEntity]:
    ...
    async def find_by(self, **kwargs) -> List[EnergyCustomerEntity]:
...
class EnergyCustomerCachedRepository(BaseEnergyRepository[EnergyCustomerEntity]):
    """Cached repository wrapper for energy customer"""
...
class EnergyCustomerUnitOfWork:
    """Unit of Work pattern for energy customer"""

    def __init__(self, repository: BaseEnergyRepository):
        self._repository = repository
        self._new: List[EnergyCustomerEntity] = []
        self._dirty: List[EnergyCustomerEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: EnergyCustomerEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: EnergyCustomerEntity) -> None:
