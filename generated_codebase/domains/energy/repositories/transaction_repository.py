...
class EnergyTransactionInMemoryRepository(BaseEnergyRepository[EnergyTransactionEntity]):
    """In-memory repository for energy transaction"""

    def __init__(self):
        self._storage: Dict[str, EnergyTransactionEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: EnergyTransactionEntity) -> EnergyTransactionEntity:
    ...
    async def read(self, id: str) -> Optional[EnergyTransactionEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: EnergyTransactionEntity) -> EnergyTransactionEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[EnergyTransactionEntity]:
    ...
    async def find_by(self, **kwargs) -> List[EnergyTransactionEntity]:
...
class EnergyTransactionCachedRepository(BaseEnergyRepository[EnergyTransactionEntity]):
    """Cached repository wrapper for energy transaction"""
...
class EnergyTransactionUnitOfWork:
    """Unit of Work pattern for energy transaction"""

    def __init__(self, repository: BaseEnergyRepository):
        self._repository = repository
        self._new: List[EnergyTransactionEntity] = []
        self._dirty: List[EnergyTransactionEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: EnergyTransactionEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: EnergyTransactionEntity) -> None:
