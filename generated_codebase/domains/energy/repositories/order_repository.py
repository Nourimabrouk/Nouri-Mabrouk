...
@dataclass
class EnergyOrderEntity:
    """Entity for energy order"""
    id: str
    data: Dict[str, Any]
...
class EnergyOrderInMemoryRepository(BaseEnergyRepository[EnergyOrderEntity]):
    """In-memory repository for energy order"""

    def __init__(self):
        self._storage: Dict[str, EnergyOrderEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: EnergyOrderEntity) -> EnergyOrderEntity:
    ...
    async def read(self, id: str) -> Optional[EnergyOrderEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: EnergyOrderEntity) -> EnergyOrderEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[EnergyOrderEntity]:
    ...
    async def find_by(self, **kwargs) -> List[EnergyOrderEntity]:
...
class EnergyOrderCachedRepository(BaseEnergyRepository[EnergyOrderEntity]):
    """Cached repository wrapper for energy order"""
...
class EnergyOrderUnitOfWork:
    """Unit of Work pattern for energy order"""

    def __init__(self, repository: BaseEnergyRepository):
        self._repository = repository
        self._new: List[EnergyOrderEntity] = []
        self._dirty: List[EnergyOrderEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: EnergyOrderEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: EnergyOrderEntity) -> None:
