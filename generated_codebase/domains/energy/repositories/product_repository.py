...
@dataclass
class EnergyProductEntity:
    """Entity for energy product"""
    id: str
    data: Dict[str, Any]
...
class EnergyProductInMemoryRepository(BaseEnergyRepository[EnergyProductEntity]):
    """In-memory repository for energy product"""

    def __init__(self):
        self._storage: Dict[str, EnergyProductEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: EnergyProductEntity) -> EnergyProductEntity:
    ...
    async def read(self, id: str) -> Optional[EnergyProductEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: EnergyProductEntity) -> EnergyProductEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[EnergyProductEntity]:
    ...
    async def find_by(self, **kwargs) -> List[EnergyProductEntity]:
...
class EnergyProductCachedRepository(BaseEnergyRepository[EnergyProductEntity]):
    """Cached repository wrapper for energy product"""
...
class EnergyProductUnitOfWork:
    """Unit of Work pattern for energy product"""

    def __init__(self, repository: BaseEnergyRepository):
        self._repository = repository
        self._new: List[EnergyProductEntity] = []
        self._dirty: List[EnergyProductEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: EnergyProductEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: EnergyProductEntity) -> None:
