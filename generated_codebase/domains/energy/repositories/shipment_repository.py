...
@dataclass
class EnergyShipmentEntity:
    """Entity for energy shipment"""
    id: str
    data: Dict[str, Any]
...
class EnergyShipmentInMemoryRepository(BaseEnergyRepository[EnergyShipmentEntity]):
    """In-memory repository for energy shipment"""

    def __init__(self):
        self._storage: Dict[str, EnergyShipmentEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: EnergyShipmentEntity) -> EnergyShipmentEntity:
    ...
    async def read(self, id: str) -> Optional[EnergyShipmentEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: EnergyShipmentEntity) -> EnergyShipmentEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[EnergyShipmentEntity]:
    ...
    async def find_by(self, **kwargs) -> List[EnergyShipmentEntity]:
...
class EnergyShipmentCachedRepository(BaseEnergyRepository[EnergyShipmentEntity]):
    """Cached repository wrapper for energy shipment"""
...
class EnergyShipmentUnitOfWork:
    """Unit of Work pattern for energy shipment"""

    def __init__(self, repository: BaseEnergyRepository):
        self._repository = repository
        self._new: List[EnergyShipmentEntity] = []
        self._dirty: List[EnergyShipmentEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: EnergyShipmentEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: EnergyShipmentEntity) -> None:
