...
@dataclass
class EnergyInventoryEntity:
    """Entity for energy inventory"""
    id: str
    data: Dict[str, Any]
...
class EnergyInventoryInMemoryRepository(BaseEnergyRepository[EnergyInventoryEntity]):
    """In-memory repository for energy inventory"""

    def __init__(self):
        self._storage: Dict[str, EnergyInventoryEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: EnergyInventoryEntity) -> EnergyInventoryEntity:
    ...
    async def read(self, id: str) -> Optional[EnergyInventoryEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: EnergyInventoryEntity) -> EnergyInventoryEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[EnergyInventoryEntity]:
    ...
    async def find_by(self, **kwargs) -> List[EnergyInventoryEntity]:
...
class EnergyInventoryCachedRepository(BaseEnergyRepository[EnergyInventoryEntity]):
    """Cached repository wrapper for energy inventory"""
...
class EnergyInventoryUnitOfWork:
    """Unit of Work pattern for energy inventory"""

    def __init__(self, repository: BaseEnergyRepository):
        self._repository = repository
        self._new: List[EnergyInventoryEntity] = []
        self._dirty: List[EnergyInventoryEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: EnergyInventoryEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: EnergyInventoryEntity) -> None:
