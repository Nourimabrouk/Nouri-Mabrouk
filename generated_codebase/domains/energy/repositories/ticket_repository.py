...
@dataclass
class EnergyTicketEntity:
    """Entity for energy ticket"""
    id: str
    data: Dict[str, Any]
...
class EnergyTicketInMemoryRepository(BaseEnergyRepository[EnergyTicketEntity]):
    """In-memory repository for energy ticket"""

    def __init__(self):
        self._storage: Dict[str, EnergyTicketEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: EnergyTicketEntity) -> EnergyTicketEntity:
    ...
    async def read(self, id: str) -> Optional[EnergyTicketEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: EnergyTicketEntity) -> EnergyTicketEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[EnergyTicketEntity]:
    ...
    async def find_by(self, **kwargs) -> List[EnergyTicketEntity]:
...
class EnergyTicketCachedRepository(BaseEnergyRepository[EnergyTicketEntity]):
    """Cached repository wrapper for energy ticket"""
...
class EnergyTicketUnitOfWork:
    """Unit of Work pattern for energy ticket"""

    def __init__(self, repository: BaseEnergyRepository):
        self._repository = repository
        self._new: List[EnergyTicketEntity] = []
        self._dirty: List[EnergyTicketEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: EnergyTicketEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: EnergyTicketEntity) -> None:
