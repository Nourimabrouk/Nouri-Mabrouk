...
@dataclass
class EnergyInvoiceEntity:
    """Entity for energy invoice"""
    id: str
    data: Dict[str, Any]
...
class EnergyInvoiceInMemoryRepository(BaseEnergyRepository[EnergyInvoiceEntity]):
    """In-memory repository for energy invoice"""

    def __init__(self):
        self._storage: Dict[str, EnergyInvoiceEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: EnergyInvoiceEntity) -> EnergyInvoiceEntity:
    ...
    async def read(self, id: str) -> Optional[EnergyInvoiceEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: EnergyInvoiceEntity) -> EnergyInvoiceEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[EnergyInvoiceEntity]:
    ...
    async def find_by(self, **kwargs) -> List[EnergyInvoiceEntity]:
...
class EnergyInvoiceCachedRepository(BaseEnergyRepository[EnergyInvoiceEntity]):
    """Cached repository wrapper for energy invoice"""
...
class EnergyInvoiceUnitOfWork:
    """Unit of Work pattern for energy invoice"""

    def __init__(self, repository: BaseEnergyRepository):
        self._repository = repository
        self._new: List[EnergyInvoiceEntity] = []
        self._dirty: List[EnergyInvoiceEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: EnergyInvoiceEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: EnergyInvoiceEntity) -> None:
