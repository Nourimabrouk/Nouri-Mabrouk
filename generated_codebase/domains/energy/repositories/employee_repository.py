...
@dataclass
class EnergyEmployeeEntity:
    """Entity for energy employee"""
    id: str
    data: Dict[str, Any]
...
class EnergyEmployeeInMemoryRepository(BaseEnergyRepository[EnergyEmployeeEntity]):
    """In-memory repository for energy employee"""

    def __init__(self):
        self._storage: Dict[str, EnergyEmployeeEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: EnergyEmployeeEntity) -> EnergyEmployeeEntity:
    ...
    async def read(self, id: str) -> Optional[EnergyEmployeeEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: EnergyEmployeeEntity) -> EnergyEmployeeEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[EnergyEmployeeEntity]:
    ...
    async def find_by(self, **kwargs) -> List[EnergyEmployeeEntity]:
...
class EnergyEmployeeCachedRepository(BaseEnergyRepository[EnergyEmployeeEntity]):
    """Cached repository wrapper for energy employee"""
...
class EnergyEmployeeUnitOfWork:
    """Unit of Work pattern for energy employee"""

    def __init__(self, repository: BaseEnergyRepository):
        self._repository = repository
        self._new: List[EnergyEmployeeEntity] = []
        self._dirty: List[EnergyEmployeeEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: EnergyEmployeeEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: EnergyEmployeeEntity) -> None:
