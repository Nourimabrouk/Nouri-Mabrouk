...
@dataclass
class EnergyPaymentEntity:
    """Entity for energy payment"""
    id: str
    data: Dict[str, Any]
...
class EnergyPaymentInMemoryRepository(BaseEnergyRepository[EnergyPaymentEntity]):
    """In-memory repository for energy payment"""

    def __init__(self):
        self._storage: Dict[str, EnergyPaymentEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: EnergyPaymentEntity) -> EnergyPaymentEntity:
    ...
    async def read(self, id: str) -> Optional[EnergyPaymentEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: EnergyPaymentEntity) -> EnergyPaymentEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[EnergyPaymentEntity]:
    ...
    async def find_by(self, **kwargs) -> List[EnergyPaymentEntity]:
...
class EnergyPaymentCachedRepository(BaseEnergyRepository[EnergyPaymentEntity]):
    """Cached repository wrapper for energy payment"""
...
class EnergyPaymentUnitOfWork:
    """Unit of Work pattern for energy payment"""

    def __init__(self, repository: BaseEnergyRepository):
        self._repository = repository
        self._new: List[EnergyPaymentEntity] = []
        self._dirty: List[EnergyPaymentEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: EnergyPaymentEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: EnergyPaymentEntity) -> None:
