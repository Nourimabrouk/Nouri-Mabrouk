...
@dataclass
class EnergySupplierEntity:
    """Entity for energy supplier"""
    id: str
    data: Dict[str, Any]
...
class EnergySupplierInMemoryRepository(BaseEnergyRepository[EnergySupplierEntity]):
    """In-memory repository for energy supplier"""

    def __init__(self):
        self._storage: Dict[str, EnergySupplierEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: EnergySupplierEntity) -> EnergySupplierEntity:
    ...
    async def read(self, id: str) -> Optional[EnergySupplierEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: EnergySupplierEntity) -> EnergySupplierEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[EnergySupplierEntity]:
    ...
    async def find_by(self, **kwargs) -> List[EnergySupplierEntity]:
...
class EnergySupplierCachedRepository(BaseEnergyRepository[EnergySupplierEntity]):
    """Cached repository wrapper for energy supplier"""
...
class EnergySupplierUnitOfWork:
    """Unit of Work pattern for energy supplier"""

    def __init__(self, repository: BaseEnergyRepository):
        self._repository = repository
        self._new: List[EnergySupplierEntity] = []
        self._dirty: List[EnergySupplierEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: EnergySupplierEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: EnergySupplierEntity) -> None:
