...
@dataclass
class EnergyReportEntity:
    """Entity for energy report"""
    id: str
    data: Dict[str, Any]
...
class EnergyReportInMemoryRepository(BaseEnergyRepository[EnergyReportEntity]):
    """In-memory repository for energy report"""

    def __init__(self):
        self._storage: Dict[str, EnergyReportEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: EnergyReportEntity) -> EnergyReportEntity:
    ...
    async def read(self, id: str) -> Optional[EnergyReportEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: EnergyReportEntity) -> EnergyReportEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[EnergyReportEntity]:
    ...
    async def find_by(self, **kwargs) -> List[EnergyReportEntity]:
...
class EnergyReportCachedRepository(BaseEnergyRepository[EnergyReportEntity]):
    """Cached repository wrapper for energy report"""
...
class EnergyReportUnitOfWork:
    """Unit of Work pattern for energy report"""

    def __init__(self, repository: BaseEnergyRepository):
        self._repository = repository
        self._new: List[EnergyReportEntity] = []
        self._dirty: List[EnergyReportEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: EnergyReportEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: EnergyReportEntity) -> None:
