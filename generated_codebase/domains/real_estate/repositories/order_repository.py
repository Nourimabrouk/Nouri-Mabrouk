...
@dataclass
class Real_EstateOrderEntity:
    """Entity for real_estate order"""
    id: str
    data: Dict[str, Any]
...
class Real_EstateOrderInMemoryRepository(BaseReal_EstateRepository[Real_EstateOrderEntity]):
    """In-memory repository for real_estate order"""

    def __init__(self):
        self._storage: Dict[str, Real_EstateOrderEntity] = {}
        self._lock = asyncio.Lock()

    async def create(self, entity: Real_EstateOrderEntity) -> Real_EstateOrderEntity:
    ...
    async def read(self, id: str) -> Optional[Real_EstateOrderEntity]:
        entity = self._storage.get(id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def update(self, entity: Real_EstateOrderEntity) -> Real_EstateOrderEntity:
        async with self._lock:
    ...
    async def list(self, filters: Dict[str, Any] = None, limit: int = 100, offset: int = 0) -> List[Real_EstateOrderEntity]:
    ...
    async def find_by(self, **kwargs) -> List[Real_EstateOrderEntity]:
...
class Real_EstateOrderCachedRepository(BaseReal_EstateRepository[Real_EstateOrderEntity]):
    """Cached repository wrapper for real_estate order"""
...
class Real_EstateOrderUnitOfWork:
    """Unit of Work pattern for real_estate order"""

    def __init__(self, repository: BaseReal_EstateRepository):
        self._repository = repository
        self._new: List[Real_EstateOrderEntity] = []
        self._dirty: List[Real_EstateOrderEntity] = []
        self._deleted: List[str] = []

    def register_new(self, entity: Real_EstateOrderEntity) -> None:
        self._new.append(entity)

    def register_dirty(self, entity: Real_EstateOrderEntity) -> None:
